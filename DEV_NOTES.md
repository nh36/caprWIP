...
- **OE consonant innovations:** add the missing PGmc→OE consonant changes (palatalisation in OE contexts, rhotic prep, targeted lexical replacements) so stage outputs align with `COUNTERPART` without using ME/RP rules.
  - Note: `*dagăz` still yields `dāga` rather than `dæġ` because early brightening is reversed by back‑mutation from the following weak‑tail vowel. Decide whether to adjust back‑mutation triggers or handle weak‑tail earlier so `dæġ` is reachable without breaking a‑back logic.
