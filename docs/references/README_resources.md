# OCRed reference resources (quick index)

This file lists OCRed reference texts in `docs/references/` with quick hints for searching.

## Old English / Germanic lexica

- `aconciseanglosa01hallgoog.txt`  
  Hall, *A Concise Anglo‑Saxon Dictionary* (2nd ed.).  
  Quick search: `rg -n -i "cnop|cnoppa|wullknoppa" docs/references/aconciseanglosa01hallgoog.txt`

- `anglosaxondictio00tolluoft.txt`  
  Bosworth‑Toller (dictionary + supplement scans combined).  
  Quick search: `rg -n -i "cnop|cnoppa|wullknoppa" docs/references/anglosaxondictio00tolluoft.txt`

- `anglosaxonoldeng00wrig.txt`  
  Wright & Wülcker, *Anglo‑Saxon and Old English Vocabularies*.  
  Quick search: `rg -n -i "cnop|wullknoppa|ballationes" docs/references/anglosaxonoldeng00wrig.txt`

- `aneightcenturyl00librgoog.txt`  
  Hessels / early glossary material (Eighth‑Century Latin‑Anglo‑Saxon Glossary).  
  Quick search: `rg -n -i "cnop|ballationes" docs/references/aneightcenturyl00librgoog.txt`

- `etymological_dictionary_of_proto_germanic_kroonen.txt`  
  Kroonen, *Etymological Dictionary of Proto‑Germanic*.  
  Quick search: `rg -n -i "knappr|knappa|knab/ppan|cnæp|knob" docs/references/etymological_dictionary_of_proto_germanic_kroonen.txt`

## Other references

- `hogg_vol1.txt`  
  Hogg, *A Grammar of Old English*, Vol. 1.

