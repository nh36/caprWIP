...
#### Germanic/docs/DEV_NOTES.md:1770 (exact COUNTERPART)
...
1769:   - 2026-01-10 tracing follow-up:
1770:     - `*kewwăną → ċēowan`: `OldEnglishEwLongDiphthong` only sees single `{*w}`; extend the rule to promote `{eww}` to `{ēow}` so duplicated glides still trigger the long diphthong tier.
...
#### Germanic/docs/DEV_NOTES.md:3628 (exact COUNTERPART)

- Nearby heading: ## OEAwLongDiphthong: PGmc *aw → OE ēaw before vowels (Campbell §272)
...
- Nearby heading: ### Potential Other Affected Forms

```text
16345: R/T mentions only two clear examples of the `*dw/*zw → *ww` change:
16346: - `*fedwor` 'four' — the only OE example in our TSV
16347: - `*izwiz` 'you (dat.pl.)' — OE ēow (not in TSV but derivable)
16348: 
16349: ### Chronological Placement
```

#### Germanic/docs/DEV_NOTES.md:16768 (exact COUNTERPART)

- Nearby heading: ### Stiles 1985-6 on the Numeral 'four': Research Summary (2026-04-10)

```text
16766: 
16767: The assimilation `*-ðw- → *-ww-` also applied to `*-zw- → *-ww-`, as shown by
16768: the pronoun: Go. izwis → OE ēow (from `*iwwar-`).
16769: 
16770: Thus for 'four': PGmc `*feðwōr` → WGmc `*fewwar` → OE `feower`.
```

#### Germanic/docs/DEV_NOTES.md:42774 (exact COUNTERPART)

- Nearby heading: ## §17.50 — *néwun → nigon* mismatch dossier (2026-04-XX)

```text
42772: EwLongDiphthong + medial u-lowering); the attested OE form is `nigon`.
42773: Trace shows nothing remarkable until late: `*n*é*w*u*n` carries through
42774: all early stages, then receives breaking `*éw → *ēow` and medial
42775: `*u → *o` to yield `nēowon`.
42776: 
```
...
#### Germanic/docs/DEV_NOTES.md:2033 (concept name)

- Nearby heading: # Daily Hand-off Notes

```text
2031: # Daily Hand-off Notes
2032: 
2033: Add a new dated section (reverse chronological) each time you pause work.
2034: Include:
2035: - Services you touched (Docker/Caddy, datasets loaded).
```

#### Germanic/docs/DEV_NOTES.md:2035 (concept name)
...
2034: Include:
2035: - Services you touched (Docker/Caddy, datasets loaded).
2036: - Regression harness status and notable warnings.
2037: - Next tasks or blockers.
```

#### Germanic/docs/DEV_NOTES.md:2197 (concept name)

- Nearby heading: ### Tips

```text
2195: - To inspect board titles in the UI, open the dev console and check `window.loaded.boards` after loading data.
2196: 
2197: See you tomorrow!
2198: 
2199: - Enabled brace-star tokens at the proto layer by removing `RemoveStars` from `GermanProtoInput`; rewrote `GermanEwChain`, `GermanAuMonophth`, and `GermanLongVowelRules` to operate on `{*…}` symbols.
```

#### Germanic/docs/DEV_NOTES.md:4328 (concept name)

- Nearby heading: ### Research on Option 2: 3sg present pathway (2026-03-09 continued)

```text
4326: R/T vol.2 p.25 explicitly give the parallel derivation for *habban*:
4327: 
4328: > "PGmc pres. \*habaisi 'you have', \*habaiþi '(s)he has' (Goth. habais, habaiþ) →
4329: > \*habēs, \*habēþ → OE (North.) hæfes, **hefed**"
4330: 
```

#### Germanic/docs/DEV_NOTES.md:16262 (concept name)

- Nearby heading: ### Source Analysis

```text
16260: > been backformed to fiordo 'fourth', etc. (Stiles 1985-6, NOWELE 6: 91-2);
16261: >
16262: > PGmc `*izwiz` 'you (dat. pl.)' (Goth. izwis) > `*iwwi` > PWGmc `*iuwi` ~ `*iuw`
16263: > (see 3.1.4) > OE iow, OF it, OS, OHG iu;
16264: >
```
...
#### Germanic/docs/analysis/notable_findings.md:540 (concept name)

- Nearby heading: ## 3. PWGmc \*j-related sound changes: formalization of under-specified rules

```text
539: leaves vague. The questions this raises — about rule generality, about
540: unifying structurally parallel changes — are questions that only arise when you
541: try to implement the sound changes as formal rules rather than prose
```

#### Germanic/docs/dossiers/g-palatalisation-conditioning.md:299 (concept name)

- Nearby heading: ### 2.8 Sources NOT consulted in detail (named but not used as authority here)

```text
298: 
299: If you want any of these covered next, point me at the file (or a scan).
300: 
```

#### Germanic/docs/dossiers/g-palatalisation-conditioning.md:408 (concept name)

- Nearby heading: ### 6.2 Equivalent four-clause enumeration (more explicit, easier to audit)

```text
407: 
408: If you prefer the rule to spell out each environment that handbook §6.4.1
409: rule 4 / Campbell §428–429 license:
```
...
| SieversBrunner1965 | single available key for Brunner |
