# Evidence packet — 2252 thousand / þūsend
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 2252 | thousand | þūsend | *θūs-undī | *θūs-èndi | early_analogy | Kroonen *θūsundī- f. 'thousand' → OE þūsend; medial -e- is analogical (Vorbild: ærende < *arundiiu); preserved ū (not ȳ) proves -ī lost before i-umlaut (double umlaut would give *þȳsend); OS/OHG retain -u- | - |
...
#### Germanic/docs/DEV_NOTES.md:27619 (row ID)

- Nearby heading: ### §16.6.4 Scope in our data (OE rows only)

```text
27617: |---|---|---|---|
27618: | 2148 | `*régnă-bugô` | `reġnboga` | match (via `OECompoundLinkingSyncope`) |
27619: | 2252 | `*θūs-endi` | `þūsend` | match |
27620: | 2302 | `*wír-aldu` | `weorold` | match (via transponent *u) |
27621: 
```
...
#### Germanic/docs/DEV_NOTES.md:16818 (exact COUNTERPART)

- Nearby heading: ### Compound Words: *regnă-bugô → reġnboga 'rainbow' (2026-04-11)

```text
16816: | `*regnă-bugô` | `reġnafoga` | `reġnboga` |
16817: | `*wiră-aldiz` | `weraield` | `weorold` |
16818: | `*θūs-undī` | `þūsynde` | `þūsend` |
16819: 
16820: For `*regnă-bugô → reġnafoga`, the issues are:
```

#### Germanic/docs/DEV_NOTES.md:17936 (exact COUNTERPART)

- Nearby heading: ## §14.7 OE þūsend 'thousand': Compound Analysis (2026-04-12)

```text
17934: ---
17935: 
17936: ## §14.7 OE þūsend 'thousand': Compound Analysis (2026-04-12)
17937: 
17938: ### Initial Problem Statement
```

#### Germanic/docs/DEV_NOTES.md:17942 (exact COUNTERPART)

- Nearby heading: ### Initial Problem Statement

```text
17940: | Proto-Form | FST Output | Expected OE |
17941: |------------|------------|-------------|
17942: | `*θūs-undī` | `þūsynde` | `þūsend` |
17943: 
17944: The FST produces `þūsynde` but the expected OE form is `þūsend`. Two issues:
```

#### Germanic/docs/DEV_NOTES.md:17944 (exact COUNTERPART)
...
17944: The FST produces `þūsynde` but the expected OE form is `þūsend`. Two issues:
17945: 
17946: 1. **Vowel quality**: FST gives `y` (from i-umlaut `*u → *y`), but OE has `e`
```

#### Germanic/docs/DEV_NOTES.md:17969 (exact COUNTERPART)

- Nearby heading: ### Preliminary Observations

```text
17967: - PGmc `*þūsundī` (ī/jō-stem, nom.sg.)
17968: - PWGmc `*þūsundi` (with SHORT final `-i`, not long `-ī`)
17969: - OE `þūsend`
17970: 
17971: **Cross-Germanic comparison:**
```

#### Germanic/docs/DEV_NOTES.md:17974 (exact COUNTERPART)

- Nearby heading: ### Preliminary Observations

```text
17972: - OS `thūsund` — keeps `u`
17973: - OD `thūsunt` — keeps `u`
17974: - OE `þūsend` — has `e`, not `y`
17975: 
17976: The OE `e` (not `y`) is unexpected. Standard i-umlaut `*u → *y` should apply.
```

#### Germanic/docs/DEV_NOTES.md:27747 (exact PROTOFORM)

- Nearby heading: ### §17.11.0 Motivation

```text
27745: 
27746: The OE analogues now use grave-accent Nebenton
27747: (`*régna-bùgô`, `*wír-àldu`, `*θūs-èndi`; §16.6), and the
27748: §16.6 migration also dropped the breve from the linking vowel
27749: of row 2148 (`*régnă-bugô → *régna-bùgô`) on the principle
```
...
| :--- | :--- | :--- | :--- | :--- |
| thousand | þūsend | inh | template:inh | thousand |
...
#### Germanic/docs/DEV_NOTES.md:18021 (concept name)

- Nearby heading: #### Source 2: Kroonen, *EDPG* (p.554)

```text
18019: #### Source 2: Kroonen, *EDPG* (p.554)
18020: 
18021: > **\*þūsundī-** f. 'thousand' — Go. þūsundi f. 'id.', ON þúsund f. 'id.', 
18022: > ... **OE þūsend num. 'id.'**, E thousand...
18023: 
```

#### Germanic/docs/DEV_NOTES.md:18022 (concept name)
...
18022: > ... **OE þūsend num. 'id.'**, E thousand...
18023: 
18024: Kroonen reconstructs PGmc as `*þūsundī-` (feminine jō-stem), but notes 
```

#### Germanic/docs/DEV_NOTES.md:18030 (concept name)

- Nearby heading: #### Source 3: Campbell, *Old English Grammar* (§689, §203)

```text
18028: #### Source 3: Campbell, *Old English Grammar* (§689, §203)
18029: 
18030: §689: "Þūsend thousand, is a **neut. noun**, g.s. -es, d.s. -e, n.a.p. 
18031: unchanged or -u (-o, -a), g.p. -a (rarely -ra), d.p. -um."
18032: 
```

#### Germanic/docs/DEV_NOTES.md:18176 (concept name)

- Nearby heading: #### Hypothesis 2: Unstressed u → e in Certain Positions

```text
18174: However, **Luick §492** (p. 610) groups `þūsend` with `ærende`:
18175: 
18176: > "und den nach diesen Mustern umgebildeten Formen thousand 
18177: > 'tausend', erand(e) 'Botschaft' (ae. pūsend, ærende)"
18178: 
```

#### Germanic/docs/DEV_NOTES.md:18331 (concept name)

- Nearby heading: #### 1. Viredaz 2025 "Germanic, Slavic and Baltic 'thousand' once more"

```text
18329: OE þūsend problem:
18330: 
18331: #### 1. Viredaz 2025 "Germanic, Slavic and Baltic 'thousand' once more"
18332: 
18333: Key finding (§2.1.4):
```
