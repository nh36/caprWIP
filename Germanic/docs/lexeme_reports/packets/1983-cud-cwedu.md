# Evidence packet — 1983 cud / cwedu
...
#### Germanic/docs/DEV_NOTES.md:10412 (exact pair)
...
#### Germanic/docs/DEV_NOTES.md:28409 (row ID)

- Nearby heading: ### §17.14.1 The mismatch

```text
28407: | ID | PROTOFORM | FST output | COUNTERPART (target) |
28408: |----|-----------|------------|----------------------|
28409: | 1983 | `*kwíθuz` | `cwiþu` | `cwedu` |
28410: 
28411: showed two surface divergences: *i* vs *e* in the stem vowel, and *þ* vs *d*
```

#### Germanic/docs/DEV_NOTES.md:28509 (row ID)

- Nearby heading: ### §17.14.5 TSV change

```text
28507: ### §17.14.5 TSV change
28508: 
28509: Row 1983, PROTOFORM: `*kwíθuz` → `*kwéðuz`.
28510: 
28511: The COUNTERPART (`cwedu`), TOKENS (`c u d u`), PROTO (`*kwéduz`, Kroonen
```
...
#### Germanic/docs/DEV_NOTES.md:6009 (exact COUNTERPART)

- Nearby heading: ## OE cwedu/cwidu/cudu 'cud, resin': Fix proto-form *kwedu- (2026-03-10)

```text
6007: ---
6008: 
6009: ## OE cwedu/cwidu/cudu 'cud, resin': Fix proto-form *kwedu- (2026-03-10)
6010: 
6011: ### The problem
```

#### Germanic/docs/DEV_NOTES.md:6074 (exact COUNTERPART)

- Nearby heading: ### The fixes

```text
6072: 
6073: 1. Changed PROTO from `*kwiθuz` → `*kweduz` (u-stem nominative)
6074: 2. Changed COUNTERPART from `cudu` → `cwedu` (preserving etymological `cw-`)
6075: 
6076: The form `cwedu` (with `e` from ablaut variant) is attested alongside `cwidu`.
```

#### Germanic/docs/DEV_NOTES.md:6076 (exact COUNTERPART)
...
6075: 
6076: The form `cwedu` (with `e` from ablaut variant) is attested alongside `cwidu`.
6077: The w-less form `cudu` is a later/dialectal simplification.
6078: 
```

#### Germanic/docs/DEV_NOTES.md:17438 (exact COUNTERPART)

- Nearby heading: #### 12. Fulk vs. Our Implementation of `*i → *e` (2026-04-12)

```text
17436: **Forms to investigate:**
17437: 
17438: 1. `*kwiθuz` → OE `cwidu/cwedu/cudu` — coda *θ is coronal, so lowering should apply → `*e`
17439:    But the attested form has *i* (`cwidu`). Is this analogical restoration?
17440: 
```

#### Germanic/docs/DEV_NOTES.md:17477 (exact COUNTERPART)

- Nearby heading: #### 12. Fulk vs. Our Implementation of `*i → *e` (2026-04-12)

```text
17475: yet `cwidu` shows *i. This may reflect:
17476: - Analogical restoration of *i from related forms (cf. `cwic` 'alive')
17477: - Dialectal variation (some glosses show `cwedu`, `cweodu`)
17478: - A phonological factor we haven't identified
17479: 
```
...
#### Germanic/docs/DEV_NOTES.md:6020 (concept name)

- Nearby heading: #### Kroonen (2013) p.316, s.v. `*kwedu- 2 m. 'resin'`

```text
6018: #### Kroonen (2013) p.316, s.v. `*kwedu- 2 m. 'resin'`
6019: 
6020: > "*kwedu- 2 m. 'resin' — OE cwidu, cweodu, c(w)udu m. 'cud, mastix', E cud,
6021: > quid 'ruminated substance; wad of tobacco', OHG quiti, kuti m. 'resin', MHG
6022: > kite, küt m. 'id.', G Kitt m. 'putty, cement' > *gʷet-u- (IE) — Skt játu- n.
```

#### Germanic/docs/DEV_NOTES.md:6033 (concept name)

- Nearby heading: #### Ringe & Taylor vol.2 p.42 (dw > ww discussion)

```text
6031: #### Ringe & Taylor vol.2 p.42 (dw > ww discussion)
6032: 
6033: > "The OE neuter cwidu, c(w)udu, gen. cwidwes 'gum, cud' could also have been
6034: > a u-stem originally (Stiles 1985-6, NOWELE 6: 93 with references); note that
6035: > its only certain cognate, OHG quiti, chuti 'putty, glue' is apparently an
```

#### Germanic/docs/DEV_NOTES.md:6046 (concept name)

- Nearby heading: #### Campbell (1959) §218

```text
6044: 
6045: Campbell discusses `cwudu` in the context of combinative back umlaut:
6046: > "Cp. -cudu cud, although Ep. has some forms without the change, e.g.
6047: > uuidu-, uuiloc-, -quidu..."
6048: 
```

#### Germanic/docs/DEV_NOTES.md:6056 (concept name)

- Nearby heading: #### Hall (1916) s.v. `cudu`

```text
6054: #### Hall (1916) s.v. `cudu`
6055: 
6056: > "cwudu (o, i) n. what is chewed, cud"
6057: 
6058: Lists variants with `w` (cwudu) and without (cudu), plus `i` variant.
```

#### Germanic/docs/DEV_NOTES.md:28401 (concept name)

- Nearby heading: ## §17.14 — OE `cwedu` 'cud': PROTOFORM correction (2026-04-24)

```text
28399: 
28400: 
28401: ## §17.14 — OE `cwedu` 'cud': PROTOFORM correction (2026-04-24)
28402: 
28403: ### §17.14.1 The mismatch
```
...
#### Germanic/docs/dossiers/widuwe-u-preservation.md:700 (concept name)

- Nearby heading: ### B.2 Campbell, Old English Grammar sec. 218 (and 217, 219, 365, 373, 470)

```text
699: > texts, e.g. BH Derauuda d.s. (after -wudu), Ep. uudu- 'wood',
700: > Cp. -cudu 'cud', although Ep. has some forms without the change,
701: > e.g. uuidu-, uuiloc-, -quidu, and Cp. has also forms which show
```

#### Germanic/docs/dossiers/widuwe-u-preservation.md:707 (concept name)

- Nearby heading: ### B.2 Campbell, Old English Grammar sec. 218 (and 217, 219, 365, 373, 470)

```text
706: > liquids and labials, e.g. **wudu** wood, **wuduwe** widow, **wucu**
707: > week, **swutol** evident, **cwoudu** cud, **swugian** be silent,
708: > **(w)uton** let us; but there occur also in many cases forms with
```

#### Germanic/docs/dossiers/widuwe-u-preservation.md:730 (concept name)

- Nearby heading: ### B.2 Campbell, Old English Grammar sec. 218 (and 217, 219, 365, 373, 470)

```text
729: > irregularly between another consonant and u. Examples are: (1)
730: > with combinative back umlaut (sec. 218): cucu alive, cudu cud,
731: > sugian be silent, sutol clear, beside cwucu, cwudu, swugian,
```
