...
Clark Hall gives `fearn` as an Old English noun, and Bosworth-Toller records `fearn` with inflected forms such as `fearnes`, `fearna`, and `fearne` [@ClarkHall1960; @BosworthToller1898, 219]. Kroonen also records `fern`, but the local lexical sources give stronger support to `fearn` as the citation target [@Kroonen2013].
