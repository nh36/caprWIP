...
| :--- | :--- |
| lexical item | rust |
| citation reconstruction / lexeme label | *rústō |
...
uniformly. Orel cites [rustaz]{.recon} 'rust' (sb.m./f.) with Old English [`rust`]{.iv lang=oe sort=rust role=comparison_form} 'rust' and Old Saxon
...
The Old English noun is attested, not reconstructed. Clark Hall gives [`rūst`]{.iv lang=oe sort=rust role=comparison_form} 'rust' (m.)
[@ClarkHall1960, 245], and Bosworth-Toller records [`rúst`]{.iv lang=oe sort=rúst role=comparison_form} 'rust' (also `rust`)
[@BosworthToller1898, 677]. The form is normalized here as [`rust`]{.iv lang=oe sort=rust role=comparison_form} 'rust' from that attested
record.
...
vowel, the citation-form input gives `rost` 'rust', not [`rust`]{.iv lang=oe sort=rust role=comparison_form} 'rust' [@Campbell1959, §115].
The same lowering would also affect comparative citation-form reconstructions
such as [rustaz]{.recon} ‘rust’.

A high-vowel comparator such as instrumental-type [rústu]{.recon} ‘rust’ would yield [`rust`]{.iv lang=oe sort=rust role=comparison_form} 'rust'
regularly, but that does not explain the attested citation form of the noun. No
accepted regular pathway from the citation form to attested [`rust`]{.iv lang=oe sort=rust role=comparison_form} 'rust' has been
established.
