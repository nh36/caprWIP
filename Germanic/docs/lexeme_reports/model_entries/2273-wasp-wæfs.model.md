...
The Proto-Germanic form `*wábsaz` reaches Old English without any special
...
Fulk presents the Old English forms together as `wæfs` with variants `wæsp` and
`wæps` [@Fulk2018, §6.5]. Bülbring and Brunner then make the chronology more
...
The earliest directly cited Old English form is **`wæfs`**, written `waefs` in
...
From `*wábsaz`, the regular Old English path passes through loss of final `z`,
Anglo-Frisian fronting, and the allophonic development of `b` to a fricative
before `s`, yielding `wæfs`.

The later forms `wæps` and `wæsp / wasp` belong to subsequent, lexically
...
| :--- | :--- | :--- |
| `wæfs` | earliest attested OE form; regular trace output | selected target |
| `wæps` | later attested metathesized variant | genuine OE doublet, but secondary |
| `wæsp / wasp` | later West Saxon metathesized variant | genuine OE doublet, but not the selected form |
