...
Kroonen treats the cognate set under `*skellinga- ~ *skillinga-` and connects it with `*skeld-linga-`, while Orel likewise gives the coin word with OE [`scilling`]{.iv lang=oe sort=scilling role=comparison_form} 'shilling' among the reflexes [@Kroonen2013; @Orel2003]. The selected input [skíllingaz]{.recon} ‘shilling’ is the nominative-style form used here to represent that inherited `*-ing-` derivative.
