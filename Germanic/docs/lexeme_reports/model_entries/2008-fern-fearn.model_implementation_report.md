# Model entry implementation report — 2008 fern / fearn

## Files inspected

- `Germanic/docs/lexeme_reports/packets/2008-fern-fearn.md`
- `Germanic/docs/lexeme_reports/dev_notes_slices/2008-fern-fearn.md`
- `Germanic/docs/lexeme_reports/research_memos/2008-fern-fearn.md`
- `Germanic/data/germanic-aligned-final.tsv`
- `Germanic/docs/debug_snapshots/oe_derivation_class_trace_report.compact.md`
- `Germanic/docs/analysis/arestoration_r_l_research.md`
- `Germanic/docs/lexeme_reports/model_entries/1934-bake-bacan.model.md`
- `Germanic/docs/lexeme_reports/model_entries/1942-beech-bōc.model.md`
- `Germanic/docs/lexeme_reports/model_entries/1943-begin-beġinnan.model.md`
- `Germanic/docs/lexeme_reports/model_entries/1954-bone-bān.model.md`
- `Germanic/docs/lexeme_reports/model_entries/1987-deed-dǣd.model.md`
- `Germanic/docs/lexeme_reports/model_entries/production_batch_07_report.md`
- `Germanic/docs/lexeme_reports/writing_skill/README.md`
- `Germanic/docs/lexeme_reports/writing_skill/source_ledger_template.md`
- `Germanic/docs/lexeme_reports/writing_skill/book_entry_template.md`
- `Germanic/docs/lexeme_reports/writing_skill/reviewer_checklist.md`
- `Germanic/docs/lexeme_reports/writing_skill/anti_patterns.md`
- `docs/refs.bib`
- local reference files for Kroonen, Orel, Clark Hall, Bosworth-Toller, Campbell, and Ringe & Taylor

## Files created

- `Germanic/docs/lexeme_reports/model_entries/2008-fern-fearn.source_ledger.md`
- `Germanic/docs/lexeme_reports/model_entries/2008-fern-fearn.model.md`
- `Germanic/docs/lexeme_reports/model_entries/2008-fern-fearn.reviewer_checklist.md`
- `Germanic/docs/lexeme_reports/model_entries/2008-fern-fearn.model_implementation_report.md`

## Notes on this pass

- Kept the entry compact and centered on attested `fearn`.
- Preserved the comparative stem/gender variation only as brief philological background.
- Omitted a paradigm comparison because the row is already a stable regular breaking derivation.

## Citation-key check

Checked against `docs/refs.bib`:

- `BosworthToller1898`
- `Campbell1959`
- `ClarkHall1960`
- `Kroonen2013`
- `Orel2003`
- `RingeTaylor2014`

## Unresolved points

- Later editorial review should keep the comparative `*farna-` / `*farnan` labels distinct from selected input `*fárnaz` and from attested OE `fearn`.

## OCR and source-transcription checks

- No suspicious OCR form required corrective rewriting in the final prose.
- The Kroonen, Orel, Clark Hall, Bosworth-Toller, Campbell, and Ringe-Taylor passages used here were legible in the local files.
- No Google Vision fallback beyond the ordinary local vision-backed reference files was needed.

## Scope confirmation

- No TSV, FST, manifest, packet, dev-note slice, research memo, bibliography file, derivation trace, writing-skill file, or existing model entry was changed.

## Citation-locator full-corpus high-confidence pass

- Added page-specific locators for `Orel2003, 133`.
- This pass was limited to high-confidence sources (`Kroonen2013`, `Orel2003`, `ClarkHall1960`, `RingeTaylor2014`, `Fulk2018`, `Seebold1970`, `BrightCassidyRingler1971`).
- Existing citations to conditional or unresolved locator sources were left unchanged.

## Citation-locator conditional-source pass

- Added page-specific locators for `BosworthToller1898, 219`.
- This pass was limited to claim-by-claim localization for `Campbell1959`, `BosworthToller1898`, and `Luick1914`.
- `KlugeSeebold2011` remained unchanged because the local text still does not preserve a reliable page marker.
