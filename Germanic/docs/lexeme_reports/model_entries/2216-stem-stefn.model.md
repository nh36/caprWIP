...
| selected input form | *stébnō |
| Old English target | stefn |
| classification | early_analogy |
| documented output | *stébnō -> stefn |

### Reconstruction and comparative evidence

The source tradition behind **[`stefn` 'stem']{.iv lang=oe sort=stefn role=target_form}** is not the same as the comparative label
...
Gothic [`stibna` 'stem']{.iv lang=goth sort=stibna role=comparison_form} and Old English `stebn > stefn > stemn` [@RingeTaylor2014,
...
**[`stemn` 'stem']{.iv lang=oe sort=stemn role=comparison_form}** to the same word [@ClarkHall1960, 276]. Ringe and Taylor give the OE
...
| :--- | :--- | :--- | :--- |
| [`*stámnaz`]{.iv lang=pgmc sort=stamnaz role=source_protoform} | comparative citation label for the broader stem/trunk family | does not itself control the `stefn` derivation discussed here | broader lexical label only |
| [`*stébnō`]{.iv lang=pgmc sort=stebno role=selected_input} | voice-noun transponent | trace output: [`stefn`]{.iv lang=oe sort=stefn role=regular_output} | selected OE-facing input |
| [`stemn`]{.iv lang=oe sort=stemn role=comparison_form} | later attested West Saxon doublet | secondary form from `stefn` by `fn > mn` | real OE variant, but not the selected comparator |
