...
Old Frisian [`stevene`]{.iv lang=ofris sort=stevene role=comparison_form} fem. 'id.',
...
Clark Hall records [`stefn`]{.iv lang=oe sort=stefn role=target_form} III m. 'stem, trunk,
root, prow, foundation', the weak n-stem
[`stefna`]{.iv lang=oe sort=stefna role=comparison_form} m. 'prow or stern of a ship',
and the related [`stofn`]{.iv lang=oe sort=stofn role=comparison_form} m. 'trunk, stem,
branch, shoot' [@ClarkHall1960, 276, 341].

Luick §211 explicitly distinguishes `stefn/stemn` 'Stamm (trunk)' — noting cognates with
Old Saxon `stamn` and Middle English `stam` — from the `stefn/stemn` 'voice' word treated
in §75 [@Luick1914, §211]. Brunner §205 lists `stefn, stemn Stamm` among words showing
the `fn/mn` alternation [@SieversBrunner1965, §205].

The primary comparison form used here is [`stefn`]{.iv lang=oe sort=stefn role=target_form}
...
[`stafn`]{.iv lang=on sort=stafn role=comparison_form} and Old Saxon
[`stamn`]{.iv lang=os sort=stamn role=comparison_form} preserve the `fn`/`mn` variants
...
`*heβonų → heofon`) handled by the `NWGmcMnDissimilation` rule; it is currently
...
1. **`stefn/stemn` 'voice, sound'**: from PGmc `*stebnō` (Ringe-Taylor, Orel); via `bn → fn` (b-allophony), giving `stefn`. This is a **different row** from 2216.
