...
The selected input [sápą]{.iv .recon lang=pgmc sort=sapa role=selected_input} 'sap' therefore does not replace those comparative labels.
...
From [sápą]{.iv .recon lang=pgmc sort=sapa role=selected_input} 'sap', Anglo-Frisian brightening yields `sæ` 'sap', and heavy-syllable nasal
...
n-stem notation [sapōn]{.iv .recon lang=pgmc sort=sapon role=source_protoform} 'sap' yields [`sape`]{.iv lang=oe sort=sape role=comparison_form} 'sap', while an i-stem continuation from the
