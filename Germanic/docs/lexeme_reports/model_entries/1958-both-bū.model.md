...
gives OE [`bezen`]{.iv lang=oe sort=bezen role=comparison_form} 'both' (< [bō-jenō]{.recon} 'both') beside ON [`báðir`]{.iv lang=on sort=badir role=comparison_form} 'both', OFris [`bēthe`]{.iv lang=ofris sort=bethe role=comparison_form} 'both', OS [`be-thia`]{.iv lang=os sort=bethia role=comparison_form} 'both',
