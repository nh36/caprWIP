# Model entry implementation report — 2314 learn (3sg) / liornaþ

## Files inspected

- `Germanic/docs/lexeme_reports/packets/2314-learn-(3sg)-liornaþ.md`
- `Germanic/docs/lexeme_reports/dev_notes_slices/2314-learn-(3sg)-liornaþ.md`
- `Germanic/docs/lexeme_reports/research_memos/2314-learn-(3sg)-liornaþ.md`
- `Germanic/docs/lexeme_reports/research_memos/batch_12_summary.md`
- `Germanic/data/germanic-aligned-final.tsv`
- `Germanic/docs/debug_snapshots/oe_derivation_class_trace_report.compact.md`
- `Germanic/docs/lexeme_reports/model_entries/2183-shoulder-sċuldrum.model.md`
- `Germanic/docs/lexeme_reports/model_entries/1958-both-bū.model.md`
- `Germanic/docs/lexeme_reports/model_entries/2258-timber-timber.model.md`
- `Germanic/docs/lexeme_reports/model_entries/2309-make-(iptv.2sg)-maca.model.md`
- `Germanic/docs/lexeme_reports/model_entries/2095-learn-liornian.model.md`
- `Germanic/docs/lexeme_reports/writing_skill/README.md`
- `Germanic/docs/lexeme_reports/writing_skill/source_ledger_template.md`
- `Germanic/docs/lexeme_reports/writing_skill/book_entry_template.md`
- `Germanic/docs/lexeme_reports/writing_skill/reviewer_checklist.md`
- `Germanic/docs/lexeme_reports/writing_skill/anti_patterns.md`
- `docs/refs.bib`
- local reference files for Kroonen, Ringe & Taylor, Campbell, Clark Hall, Brunner, Fulk, Kilday, and Bright

## Files created

- `Germanic/docs/lexeme_reports/model_entries/2314-learn-(3sg)-liornaþ.source_ledger.md`
- `Germanic/docs/lexeme_reports/model_entries/2314-learn-(3sg)-liornaþ.model.md`
- `Germanic/docs/lexeme_reports/model_entries/2314-learn-(3sg)-liornaþ.reviewer_checklist.md`
- `Germanic/docs/lexeme_reports/model_entries/2314-learn-(3sg)-liornaþ.model_implementation_report.md`

## Citation-key check

Checked against `docs/refs.bib`:

- `Campbell1959`
- `ClarkHall1960`
- `Kroonen2013`
- `RingeTaylor2014`
- `SieversBrunner1965`

## Unresolved points

- The ordinary dictionary headword remains `leornian`, so `liornaþ` must stay clearly marked as a normalized 3sg form.
- The local evidence directly supports Northumbrian finite forms in `liorn-`, but not a separate dictionary-style headword entry for `liornaþ` itself.

## OCR and source-transcription checks

- Brunner's plain-text OCR is noisy around `liornes` and related finite forms, so the relevant passages were cross-checked in `brunner_1965_altenglische_grammatik.vision.txt`.
- No corrupt Brunner form was reproduced in the final prose.
- The Campbell, Ringe-Taylor, Kroonen, and Clark Hall passages used here were legible in local text or vision files.

## Scope confirmation

- No TSV, FST, manifest, packet, dev-note slice, research memo, bibliography file, derivation trace, writing-skill file, or existing model entry was changed.
