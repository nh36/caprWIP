...
| :--- | :--- | :--- | :--- |
| regular inherited noun path | [`*búkkaz`]{.iv lang=pgmc sort=bukkaz role=selected_input} | compact-trace output: [`bocc`]{.iv lang=oe sort=bocc role=regular_output} | regular output, but not the target |
| attested OE target | — | [`bucc`]{.iv lang=oe sort=bucc role=target_form} | genuine target form, but unexplained in the present classification |
| parallel OE lexical background | — | [`bucca`]{.iv lang=oe sort=bucca role=comparison_form} | related n-stem form, not the present target |
