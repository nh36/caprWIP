...
Every printable occurrence has a stable **occurrence ID** (`occurrence_id`)
of the form `<source_ref>:<ordinal>`, for example:
...
identical. For the vast majority of occurrences the ordinal is `:1`.
...
forms.tsv
    ...
    → book_main.tsv               assembled-book subset (1 865 rows)
...
**Book-specific** (`book_main.tsv`): the subset of `print_main` whose source
material is represented in the current `capr_book_draft_alpha_01`. 233 rows are
absent because their source entries are not yet in the assembled lexical volume.
...
- `Germanic/docs/book/index_verborum_book_main.tsv` is the **book-specific**
  subset: occurrences whose source material is currently assembled into the book.
...
- `Germanic/docs/book/index_verborum_emission_table.tsv` is a machine-generated
  table mapping every print_main row to its emission path, site, and canonical
  index command. Generated by `index_verborum_emission.py`.
...
- `Germanic/docs/book/index_verborum_explicit_allow_sortkey.tsv` is a fallback
  allowlist for the Lua filter, keyed by (language, role, sort_key, occurrence_id,
  variety). Used to handle Unicode normalization mismatches (U+1E2F → ī+combining
  acute) that can cause the primary form-based match to fail.
...
1. **Occurrence-level (Markdown)**: `check_book_occ_id_parity.py` scans
   the assembled `capr_book_draft_alpha_01.md` for `occ_id=` attributes and
   proves that every `book_main` explicit_tag occurrence_id appears exactly
   once. This is the canonical proof that no explicit occurrence is missing.
