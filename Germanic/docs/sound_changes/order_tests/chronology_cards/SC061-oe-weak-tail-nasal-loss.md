...
- crossed stage: `SC023` NWGmc N Stem N Loss
...
- interpretation: SC061 can move earlier safely through order `24`, but it cannot move earlier across SC023. Pulling OE Weak Tail Nasal Loss ahead of NWGmc N Stem N Loss collapses the `do` derivation instead of preserving the live OE form.
...
Current first-break evidence places SC061 after `SC023` NWGmc N Stem N Loss. If OE Weak Tail Nasal Loss is moved before that stage, PGmc `*dōną` no longer yields expected OE `dōn`, and the variant row records no output at all (`+?`), showing that the live weak-tail sequence has been disrupted. The later direction found no real break through order `86`, so this run does **not** identify any later historical boundary for SC061.
