...
assembled_md="${script_dir}/reader_facing_local_section_10.md"
assembled_pdf="${script_dir}/reader_facing_local_section_10.pdf"
...
out = root / "reader_facing_local_section_10.md"
chapter_files = [
...
docker run --rm --platform "${platform}" --entrypoint /bin/sh \
  -v "${repo_root}":/data -w /data "${image}" -c "
    ...
    pandoc Germanic/docs/sound_changes/reader_facing/reader_facing_local_section_10.md \
      --standalone \
      ...
      --citeproc \
      --pdf-engine=xelatex \
      -o Germanic/docs/sound_changes/reader_facing/reader_facing_local_section_10.pdf
  "
