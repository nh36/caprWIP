# Reader-facing manifest coverage 03

## Inputs

1. `Germanic/docs/sound_changes/change_reports/report_manifest.tsv`
2. `Germanic/docs/sound_changes/reader_facing/build_reader_facing_local_section_15_docker.sh`
...
## Expected gaps in the manifest-backed sequence after SC014
...
2. `SC002` is absent because `report_manifest.tsv` does not include it.
3. The remaining early SC numbers outside the current manifest-backed programme are `SC001`, `SC004`, `SC005`, `SC006`, `SC007`, `SC008`, `SC009`, `SC010`, `SC011`, `SC012`, `SC013`.
4. The manifest-backed sequence now opens with `SC003` and then resumes at `SC014-SC015`.
