# Reader-facing manifest coverage 04

## Inputs

1. `Germanic/docs/sound_changes/change_reports/report_manifest.tsv`
2. `Germanic/docs/sound_changes/reader_facing/build_reader_facing_local_section_16_docker.sh`
...
4. `SC005` is absent because `report_manifest.tsv` does not include it.
5. Coverage from `SC014` through `SC087` remains intact.
6. The remaining early SC numbers outside the current manifest-backed programme are `SC001`, `SC002`, `SC007`, `SC008`, `SC009`, `SC010`, `SC011`, `SC012`, `SC013`.
7. The manifest-backed sequence now opens with `SC003`, `SC004`, and `SC006`, and then resumes at `SC014-SC015`.
