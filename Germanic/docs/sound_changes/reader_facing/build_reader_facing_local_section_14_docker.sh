...
assembled_md="${script_dir}/reader_facing_local_section_14.md"
assembled_pdf="${script_dir}/reader_facing_local_section_14.pdf"
...
out = root / "reader_facing_local_section_14.md"
coverage_out = root / "reader_facing_manifest_coverage_01.md"
chapter_files = [
...
min_sc = min(int(sc[2:]) for sc in manifest_sc_numbers)
max_sc = max(int(sc[2:]) for sc in manifest_sc_numbers)
expected_gaps = [f"SC{i:03d}" for i in range(min_sc, max_sc + 1) if f"SC{i:03d}" not in manifest_sc_set]
sc001_013 = [f"SC{i:03d}" for i in range(1, 14)]
sc001_013_outside = all(sc not in manifest_sc_set for sc in sc001_013)
...
if sc001_013_outside:
    ...
    coverage_parts.append(
        "1. At least one of `SC001-SC013` appears in `report_manifest.tsv`, so those numbers are not wholly outside the current manifest-backed programme."
    )
...
docker run --rm --platform "${platform}" --entrypoint /bin/sh \
  -v "${repo_root}":/data -w /data "${image}" -c "
    ...
    pandoc Germanic/docs/sound_changes/reader_facing/reader_facing_local_section_14.md \
      --standalone \
      ...
      --citeproc \
      --pdf-engine=xelatex \
      -o Germanic/docs/sound_changes/reader_facing/reader_facing_local_section_14.pdf
  "

echo "Generated ${assembled_md}"
echo "Generated ${assembled_pdf}"
echo "Generated ${script_dir}/reader_facing_manifest_coverage_01.md"
