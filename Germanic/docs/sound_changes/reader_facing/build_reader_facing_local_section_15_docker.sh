...
assembled_md="${script_dir}/reader_facing_local_section_15.md"
assembled_pdf="${script_dir}/reader_facing_local_section_15.pdf"
...
out = root / "reader_facing_local_section_15.md"
coverage_out = root / "reader_facing_manifest_coverage_03.md"
chapter_files = [
...
file_rule_rows: dict[str, list[tuple[str, str, str]]] = {}
...
for name in chapter_files:
    ...
    for line in text.splitlines():
        ...
        rows.append((sc_number, rule_name, title))
...
if resumed_sc_numbers:
    ...
    expected_gaps = [f"SC{i:03d}" for i in range(resumed_min, resumed_max + 1) if f"SC{i:03d}" not in manifest_sc_set]
else:
    expected_gaps = []
early_programme_missing = [f"SC{i:03d}" for i in range(1, 14) if f"SC{i:03d}" not in manifest_sc_set]
...
if "SC002" not in manifest_sc_set:
    coverage_parts.append("2. `SC002` is absent because `report_manifest.tsv` does not include it.")
else:
    coverage_parts.append("2. `SC002` appears in `report_manifest.tsv`.")

remaining_early = [sc for sc in early_programme_missing if sc not in {"SC002", "SC003"}]
if remaining_early:
    coverage_parts.append(
        "3. The remaining early SC numbers outside the current manifest-backed programme are "
        + ", ".join(f"`{sc}`" for sc in remaining_early)
        + "."
    )
else:
    coverage_parts.append("3. No other early SC numbers remain outside the current manifest-backed programme.")

if "SC003" in manifest_sc_set and "SC014" in manifest_sc_set and "SC015" in manifest_sc_set:
    coverage_parts.append("4. The manifest-backed sequence now opens with `SC003` and then resumes at `SC014-SC015`.")
else:
    coverage_parts.append("4. The manifest-backed opening sequence no longer matches the expected `SC003` then `SC014-SC015` pattern.")
...
docker run --rm --platform "${platform}" --entrypoint /bin/sh \
  -v "${repo_root}":/data -w /data "${image}" -c "
    ...
    pandoc Germanic/docs/sound_changes/reader_facing/reader_facing_local_section_15.md \
      --standalone \
      ...
      --citeproc \
      --pdf-engine=xelatex \
      -o Germanic/docs/sound_changes/reader_facing/reader_facing_local_section_15.pdf
  "

echo "Generated ${assembled_md}"
echo "Generated ${assembled_pdf}"
echo "Generated ${script_dir}/reader_facing_manifest_coverage_03.md"
