...
assembled_md="${script_dir}/reader_facing_local_section_16.md"
assembled_pdf="${script_dir}/reader_facing_local_section_16.pdf"
...
out = root / "reader_facing_local_section_16.md"
coverage_out = root / "reader_facing_manifest_coverage_04.md"
chapter_files = [
...
early_opening_sc_numbers = [sc for sc in manifest_sc_numbers if 3 <= int(sc[2:]) < 14]
if early_opening_sc_numbers:
    early_min = min(int(sc[2:]) for sc in early_opening_sc_numbers)
    early_max = max(int(sc[2:]) for sc in early_opening_sc_numbers)
    early_gaps = [f"SC{i:03d}" for i in range(early_min, early_max + 1) if f"SC{i:03d}" not in manifest_sc_set]
else:
    early_gaps = []
...
expected_gaps = early_gaps + later_gaps
early_programme_missing = [
    f"SC{i:03d}"
    for i in range(1, 14)
    if f"SC{i:03d}" not in manifest_sc_set and f"SC{i:03d}" not in set(early_gaps)
]
...
if "SC005" not in manifest_sc_set:
    coverage_parts.append("4. `SC005` is absent because `report_manifest.tsv` does not include it.")
else:
    coverage_parts.append("4. `SC005` appears in `report_manifest.tsv`.")

coverage_parts.append("5. Coverage from `SC014` through `SC087` remains intact.")

if early_programme_missing:
    coverage_parts.append(
        "6. The remaining early SC numbers outside the current manifest-backed programme are "
        + ", ".join(f"`{sc}`" for sc in early_programme_missing)
        + "."
    )
else:
    coverage_parts.append("6. No other early SC numbers remain outside the current manifest-backed programme.")

if {"SC003", "SC004", "SC006", "SC014", "SC015"} <= manifest_sc_set:
    coverage_parts.append("7. The manifest-backed sequence now opens with `SC003`, `SC004`, and `SC006`, and then resumes at `SC014-SC015`.")
else:
    coverage_parts.append("7. The manifest-backed opening sequence no longer matches the expected `SC003`, `SC004`, `SC006`, then `SC014-SC015` pattern.")
...
docker run --rm --platform "${platform}" --entrypoint /bin/sh \
  -v "${repo_root}":/data -w /data "${image}" -c "
    ...
    pandoc Germanic/docs/sound_changes/reader_facing/reader_facing_local_section_16.md \
      --standalone \
      ...
      --citeproc \
      --pdf-engine=xelatex \
      -o Germanic/docs/sound_changes/reader_facing/reader_facing_local_section_16.pdf
  "

echo "Generated ${assembled_md}"
echo "Generated ${assembled_pdf}"
echo "Generated ${script_dir}/reader_facing_manifest_coverage_04.md"
