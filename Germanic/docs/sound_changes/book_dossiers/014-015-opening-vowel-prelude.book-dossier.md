...
assembled half. Its purpose is not to rival the better-documented
`SC016-SC020` pilot corridor to the right, but to keep the immediately
...
The literature supports an opening unstressed-vowel region, but not a classic
two-change chapter called "SC014-SC015." Ringe and Taylor give the clearest
comparative statement for SC014: unstressed `*ai` was usually monophthongized
...
cases [@Hogg1992, p. 117]. The `world` material then makes the rule more
...
So the literature distribution is already clear. SC014 is source-backed but
thinly bounded. SC015 is the source-backed center of the row.

## 4. Formal implementation

In CAPR terms the row is a very short two-step opening setup:

1. **SC014** removes the unstressed diphthongal quality of `*ai`.
2. **SC015** lowers or levels early unstressed front-vowel quality farther.

That formulation is useful precisely because it is more explicit than the
handbooks. The grammars describe the broader reduction of unstressed diphthongs
and front-vowel contrasts; CAPR turns those tendencies into two ordered rule
stages. The book should keep that distinction visible.
...
That means the next decision is not whether the row has any historical basis.
It does. The real question is how short and how asymmetric the finished prose
should be.

## 8. Relation to neighbouring changes

- **Immediate right neighbour:** pilot `SC016-SC020` early vocalic/final corridor
- **Broad/far rightward cross-reference:** promoted `SC035-SC037` report, specifically SC036 via `world`
...
short cautious opening bridge, or a more asymmetric SC014-note / SC015-led
treatment.
