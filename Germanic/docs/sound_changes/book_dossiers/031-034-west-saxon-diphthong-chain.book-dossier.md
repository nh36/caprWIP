...
`SC031-SC034` is now the clearest remaining larger scaffold candidate after the
promotion of `SC051`. It therefore tests a new question for the production
layer: can a broader four-change region become a coherent chapter, or does the
workflow eventually need to split around the strongest local reciprocal pair?
...
Campbell, Luick, Ringe and Taylor, Hogg, and Fulk all support the reality of a
shared diphthongal zone here, but they usually describe it as overlapping
developments rather than one neat four-step chain [@Campbell1959, §120;
@Campbell1959, §§170--176, 185, 222--227; @Luick1914, §§168--176, 182--183;
@RingeTaylor2014, §6.1.2; @RingeTaylor2014, §6.5.1; @Fulk2018, §4.13].

That source pattern is the key result. `SC031` and `SC034` look like the
strongest local historical core. `SC033` remains plausible but somewhat more
right-leaning toward the later brightening-breaking neighborhood. `SC032`
appears to be the least natural handbook singleton and the most likely candidate
for subordinate or residual treatment if the region is ever split.
...
That is a productive kind of uncertainty. It means the next human decision is
about chapter shape, not about whether the region exists at all.
