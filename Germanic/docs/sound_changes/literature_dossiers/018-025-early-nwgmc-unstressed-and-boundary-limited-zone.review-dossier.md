...
`SC018` remains boundary-limited on both sides. The earlier search stops at
bundled `PWGmcChanges`, and the later search reaches the current `SC087`
boundary without a real break. The source tradition is still real enough to
keep the rule visible: the Northwest Germanic / Old English development of
Germanic stressed `ō` toward the high long vowel region is standard handbook
material, and the same background also underlies the already promoted
`SC019-SC020` corridor [@Campbell1959, §138; @RingeTaylor2014; @Fulk2018].

That is enough to treat `SC018` as historically legible, but not enough to turn
either edge of the current card into positive chronology. If this rule ever
...
reason to create a non-contiguous chapter.

### SC024 `NWGmcLongELowering`

`SC024` is the strongest source-backed member of the row after `SC021` and
`SC023`. Its earlier side is runner-bounded at bundled `PWGmcChanges`, but its
later boundary is real and broad/far: `SC024 < SC056`, with `sheep` and
`year`. Moving it later yields `sċīep` / `ġīer` rather than expected
`sċēap` / `ġēar`.

The historical background is explicitly handbook-backed. Campbell treats West
Saxon `sċēap` and `ġēar` as outcomes of earlier long-vowel lowering/fronting in
the relevant region [@Campbell1959, §156], and Ringe & Taylor likewise discuss
the `*skēpa-` / `*jēra-` pathway behind these forms [@RingeTaylor2014].

This makes `SC024` a strong candidate for a later singleton note. But the
relevant positive link still points outward to the already promoted
`SC055-SC056` umlaut core, so the SC056 relation should remain a rightward
cross-reference rather than chapter architecture.

### SC025 `NWGmcLongENasalRounding`

`SC025` returns to the boundary-limited profile. Its earlier search stops at
bundled `PWGmcChanges`, and the later search reaches the `SC087` boundary with
no real break. The source tradition is nevertheless intelligible: Campbell
explicitly cites `moon` and `month` from earlier `*mēnō` / `*mēnōþ`, and
Ringe & Taylor treat the same long-vowel-before-nasal region in the broader
West Germanic development [@Campbell1959, §129; @RingeTaylor2014].

So `SC025` should remain visible as a historically legible but chronology-light
note. At present it does not justify promotion as a singleton report, much less
as part of a coherent multi-rule production chapter.

## What the source review implies
...
This remains possible only as a deliberately cautious editorial report about
the zone as a whole. The dossier does **not** support treating the row as one
internally connected chronology core.
