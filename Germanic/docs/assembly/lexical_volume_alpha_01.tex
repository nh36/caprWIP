...
\emph{Alpha 01 assembly scaffold. This document assembles the current
...
Each lexical entry retains the pilot structure: a generated derivation
summary, a boxed derivation trace split into Earlier Germanic changes
and Old English changes, and the current model-entry prose. The summary
...
Derivation: \emph{*nḗdrōn} \(\rightarrow\) \emph{nǣdre} (regular).
...
\emph{nǣdre, næddre} under the latter
...
*nadrōn} type (\citeproc{ref-Orel2003}{Orel 2003, 325}).

The selected input therefore is not a reshaped convenience form. It is
the comparative reconstruction that specifically underlies the Old
English noun.
...
\emph{næddre} to \emph{nædre}, and Fulk treats \emph{næddre} as the
later geminated form beside the older base
...
From \emph{*nḗdrōn}, the stressed long mid vowel develops to Old English
\emph{ǣ}, and the weak feminine ending remains as final \emph{-e},
giving \emph{nǣdre}. The doubled consonant of \emph{næddre} is secondary
and does not alter the inherited base form.
...
Derivation: \emph{*bōkō} \(\rightarrow\) \emph{bōc} (regular).
...
\emph{boc, bēce} among its reflexes (\citeproc{ref-Kroonen2013}{Kroonen
...
therefore \emph{*bōkō \textgreater{} bōc}.

\subsubsection{begin --- OE beġinnan}\label{begin-oe-beux121innan}

Derivation: \emph{*bigínnaną} \(\rightarrow\) \emph{beġinnan} (regular).
...
\emph{beginnan} (\citeproc{ref-BosworthToller1898}{Bosworth and Toller
1898, 84}; \citeproc{ref-ClarkHall1960}{Clark Hall 1960}). Those
...
Derivation: \emph{*bḗrō} \(\rightarrow\) \emph{bǣr} (regular).
...
English \emph{bar, bær} among the reflexes
...
Derivation: \emph{*búrdiz} \(\rightarrow\) \emph{byrd} (regular).
...
Derivation: \emph{*báiną} \(\rightarrow\) \emph{bān} (regular).
...
under \emph{*bainan} (\citeproc{ref-Kroonen2013}{Kroonen 2013};
...
\emph{bān}. The resulting form matches the attested Old English citation
noun.

\subsubsection{both --- OE bū}\label{both-oe-bux16b}

Derivation: \emph{*bō} \(\rightarrow\) \emph{bū} (regular).
...
inherited paradigm \emph{*bai, *bans, *bōz/*bōns, *bō}, with Old English
\emph{bēġen}, \emph{bā}, and neuter \emph{bū}
...
and Orel still gives \emph{OE bezen (\textless{} *bō-jenō)} beside
\emph{ON báðir}, \emph{OFris bēthe}, \emph{OS be-thia}, and \emph{OHG
bēde, beide} (\citeproc{ref-Orel2003}{Orel 2003, 65}). Fulk reports that
explanation cautiously and notes Seebold's preference for a
...
\emph{bā}, with compounds such as \emph{bā twā}, \emph{bū tū}, and
\emph{bām twām} (\citeproc{ref-SieversBrunner1965}{Sievers 1965, sect.
324} Anm. 2). Campbell and Fulk present the same basic pattern:
masculine \emph{bēġen}, feminine \emph{bā}, neuter \emph{bā, bū},
genitive \emph{bēġra, bēġ(e)a}, and dative \emph{bǣm}
...
\emph{ō \textgreater{} ū} in the West Germanic stage leading to Old
English (\citeproc{ref-Campbell1959}{Campbell 1959, sect. 122}). Brunner
states the same development more directly: \emph{Auslautendes ō
erscheint als û in bū \ldots{} cu \ldots{} hū, tū}
...
\bottomrule\noalign{}
\endlastfoot
\emph{*bō \textgreater{} bū} & PGmc neuter dual \textgreater{} OE neuter
dual & selected regular comparison & main line of the entry \\
...
Derivation: \emph{*báugijaną} \(\rightarrow\) \emph{bīeġan} (regular).
...
northwest Germanic and West Saxon development more fully as
\emph{PNWGmce *baugijana \textgreater{} *béagjan \textgreater{} WS OE
biegan} (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). The
...
\emph{bigan} with examples such as \emph{Ic bēge mīne cneówa} and
\emph{Se ord bīgde upp tō þām hiltum}
(\citeproc{ref-ClarkHall1960}{Clark Hall 1960};
...
therefore straightforward: \emph{*báugijaną \textgreater{} bīeġan}.

\subsubsection{breeches --- OE brēċ}\label{breeches-oe-brux113ux10b}

Derivation: \emph{*brōkiz} \(\rightarrow\) \emph{brēċ} (regular).
...
(\citeproc{ref-Kroonen2013}{Kroonen 2013}). Ringe and Taylor give the
plural development directly as \emph{PNWGmce *brokiz \textgreater{}
*breeci \textgreater{} OE bréc} (\citeproc{ref-RingeTaylor2014}{Ringe
and Taylor 2014}). The deeper verbal base belongs to the noun's
etymological background, while the selected input here is the plural
noun form \emph{*brōkiz}.
...
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). The development
is therefore regular: \emph{*brōkiz \textgreater{} brēċ}.

\subsubsection{calf --- OE ċealf}\label{calf-oe-ux10bealf}

Derivation: \emph{*kálbaz} \(\rightarrow\) \emph{ċealf} (regular).
...
\emph{*kalbaz, pl. *kalbizō}, while Orel cites \emph{*kalbaz} as the
citation form and Ringe and Taylor derive West Saxon \emph{Cealf} from
\emph{*kalbaz, *kalbiz-} (\citeproc{ref-Kroonen2013}{Kroonen 2013};
\citeproc{ref-Orel2003}{Orel 2003, 248};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, 220}). The
selected input here is the singular \emph{*kálbaz}, since the entry
concerns the citation-form noun.

\paragraph{Old English evidence}\label{old-english-evidence-10}

Clark Hall gives \emph{cealf I. (æ, e) nm. (nap. cealfru)}, and
Bosworth-Toller likewise records \emph{Caelf} / \emph{Cealf} beside
...
Derivation: \emph{*kúrną} \(\rightarrow\) \emph{corn} (regular).
...
Derivation: \emph{*dḗdiz} \(\rightarrow\) \emph{dǣd} (regular).
...
Derivation: \emph{*dúrą} \(\rightarrow\) \emph{dor} (regular).
...
and Taylor 2014}). The selected target here is therefore the attested
...
\textgreater{} dor}; the feminine \emph{duru} belongs to the separate
line identified by Kroonen and Ringe-Taylor
(\citeproc{ref-Kroonen2013}{Kroonen 2013};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).

\subsubsection{fare --- OE faran}\label{fare-oe-faran}

Derivation: \emph{*fáraną} \(\rightarrow\) \emph{faran} (regular).
...
participial \emph{faren- \textless{} *faræn- \textless{} *faran-} shows
why fronting elsewhere in the paradigm does not alter the infinitive
headword (\citeproc{ref-Fulk2018}{Fulk 2018}).

\subsubsection{fell --- OE fell}\label{fell-oe-fell}

Derivation: \emph{*féllą} \(\rightarrow\) \emph{fell} (regular).
...
\emph{*féllą \textgreater{} fell}.

\subsubsection{fern --- OE fearn}\label{fern-oe-fearn}

Derivation: \emph{*fárnaz} \(\rightarrow\) \emph{fearn} (regular).
...
Derivation: \emph{*félθuz} \(\rightarrow\) \emph{feld} (regular).
...
earlier \emph{*felþu- \textasciitilde{} *feldu-} may reflect either
inherited \emph{*þ \textasciitilde{} *d} alternation or the regular West
Germanic development \emph{*lþ \textgreater{} ld}
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, 170}). The
broader proto label \emph{*félθuz} can remain as comparative background,
while that narrower historical ambiguity does not affect the regular
classification.
...
\citeproc{ref-Campbell1959}{Campbell 1959, 169}). The selected target is
therefore the attested citation noun \emph{feld}, with the older
...
the regular \emph{*lþ \textgreater{} ld} development, both accounts
converge on the same Old English form
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, 170}).

\subsubsection{fly --- OE flēogan}\label{fly-oe-flux113ogan}

Derivation: \emph{*fléuganą} \(\rightarrow\) \emph{flēogan} (regular).
...
Outcome: \emph{flēogan}

\paragraph{Reconstruction and comparative
evidence}\label{reconstruction-and-comparative-evidence-18}

Ringe and Taylor derive the verb as \emph{*fleugana \textgreater{} OE
fléogan} and elsewhere contrast West Saxon \emph{fléogan} with Anglian
\emph{flégan}, alongside related forms \emph{fléoge / flége}
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). The selected
input \emph{*fléuganą} represents that inherited strong verb in the
notation used here.
...
English strong verb, and Bright gives the familiar paradigm \emph{flēag,
...
Ringe and Taylor also list related \emph{fléoge / flége} and Anglian
\emph{flégan}, which belong to the same family but do not replace the
infinitive \emph{flēogan} treated here
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).
...
Derivation: \emph{*léusaną} \(\rightarrow\) \emph{lēosan} (regular).
...
\emph{forlēosan / forloren} (\citeproc{ref-RingeTaylor2014}{Ringe and
Taylor 2014}).
...
Derivation: \emph{*gángaz} \(\rightarrow\) \emph{gang} (regular).
...
is the same lexeme in the accent notation used here.
...
is therefore regular: \emph{*gángaz \textgreater{} gang}.

\subsubsection{give --- OE ġiefan}\label{give-oe-ux121iefan}

Derivation: \emph{*gébaną} \(\rightarrow\) \emph{ġiefan} (regular).
...
Campbell gives \emph{gefan (W-S giefan)} among examples of initial
palatalization, and Clark Hall records the verb under plain
...
Derivation: \emph{*gúlθą} \(\rightarrow\) \emph{gold} (regular).
...
Ringe and Taylor cite the noun as \emph{*gulþa- / *gulda-}, and Kroonen
gives the same pair (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor
2014, 42}; \citeproc{ref-Kroonen2013}{Kroonen 2013}). The selected input
\emph{*gúlθą} preserves the older consonantal form while leaving open
whether the medial stop reflects inherited alternation or regular West
...
alternation \emph{*gulþa- / *gulda-} or as the ordinary West Germanic
change \emph{*lþ \textgreater{} ld}; both routes lead to the same Old
English consonantism (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor
2014, 42}).
...
Derivation: \emph{*grábaną} \(\rightarrow\) \emph{grafan} (regular).
...
Derivation: \emph{*gástiz} \(\rightarrow\) \emph{ġiest} (regular).
...
Derivation: \emph{*xḗrą} \(\rightarrow\) \emph{hǣr} (regular).
...
separate \emph{haddr / heordan / hād-} material does not displace the
ordinary simplex \emph{hǣr} treated here
(\citeproc{ref-Kroonen2013}{Kroonen 2013};
\citeproc{ref-ClarkHall1960}{Clark Hall 1960, 158}).

\subsubsection{harvest --- OE hierfest}\label{harvest-oe-hierfest}

Derivation: \emph{*xárbistuz} \(\rightarrow\) \emph{hierfest} (regular).
...
The selected target \emph{hierfest} represents the regular native West
Saxon outcome discussed by Bammesberger and Ringe-Taylor. The attested
Old English lexical tradition, however, is chiefly \emph{hærfest /
herfest}, commonly treated as non-West-Saxon or Anglian material in West
Saxon transmission (\citeproc{ref-Bammesberger1997}{Bammesberger 1997};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).

\subsubsection{hedge --- OE heġġ}\label{hedge-oe-heux121ux121}

Derivation: \emph{*xágjaz} \(\rightarrow\) \emph{heġġ} (regular).
...
lexical item itself is therefore well attested even though the selected
form is normalized.
...
\emph{heġġ} is the selected target here, while the ordinary
...
Derivation: \emph{*xélmaz} \(\rightarrow\) \emph{helm} (regular).
...
Derivation: \emph{*xélpaną} \(\rightarrow\) \emph{helpan} (regular).
...
Derivation: \emph{*xéndjō} \(\rightarrow\) \emph{hind} (regular).
...
Derivation: \emph{*xáldaną} \(\rightarrow\) \emph{healdan} (regular).
...
West Saxon \emph{healdan} is the selected target here. Anglian and
...
Derivation: \emph{*xúrną} \(\rightarrow\) \emph{horn} (regular).
...
\emph{*hurna- / *xurnan} type for this noun
(\citeproc{ref-Kroonen2013}{Kroonen 2013}; \citeproc{ref-Orel2003}{Orel
2003, 234}). The selected input \emph{*xúrną} is the nominative-style
form used in the derivation here.
...
only. It does not replace the selected input \emph{*xúrną} as the
...
Derivation: \emph{*láidijaną} \(\rightarrow\) \emph{lǣdan} (regular).
...
Derivation: \emph{*líznōjaną} \(\rightarrow\) \emph{liornian} (regular).
...
Derivation: \emph{*xlídą} \(\rightarrow\) \emph{hlid} (regular).
...
The selected input already represents the later Germanic \emph{hliđ-}
stage used for the derivation here. From \emph{*xlídą}, heavy-syllable
apocope yields \emph{hlid}, and the form belongs to the
...
\textgreater{} hlid} line that matches the attested Old English noun
(\citeproc{ref-Orel2003}{Orel 2003}; \citeproc{ref-Lloyd1966}{Lloyd
1966}).

\subsubsection{light --- OE līehtan}\label{light-oe-lux12behtan}

Derivation: \emph{*léuxtijaną} \(\rightarrow\) \emph{līehtan} (regular).
...
the related noun \emph{lēoht} and adjective \emph{leoht/liht}
(\citeproc{ref-ClarkHall1960}{Clark Hall 1960};
\citeproc{ref-BosworthToller1898}{Bosworth and Toller 1898}).
...
Derivation: \emph{*líndō} \(\rightarrow\) \emph{lind} (regular).
...
Derivation: \emph{*mélukz} \(\rightarrow\) \emph{meoloc} (regular).
...
Kroonen and Orel reconstruct the noun as \emph{*meluk- / *melukz}, and
the nominative-style input used here is \emph{*mélukz}
(\citeproc{ref-Kroonen2013}{Kroonen 2013}; \citeproc{ref-Orel2003}{Orel
2003, 306}).
...
describe West Saxon \emph{meolc \textless{} meoluc \textless{} *meluk},
Campbell likewise discusses \emph{meoluc} and \emph{meoloc}, and Anglian
shows \emph{milc} (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor
...
meoluc} preserves the fuller nominal shape
(\citeproc{ref-Campbell1959}{Campbell 1959};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).

\subsubsection{mother --- OE mōder}\label{mother-oe-mux14dder}

Derivation: \emph{*mōdēr} \(\rightarrow\) \emph{mōder} (regular).
...
\emph{*mōder- / *mōdēr} (\citeproc{ref-Kroonen2013}{Kroonen 2013};
...
Derivation: \emph{*nátją} \(\rightarrow\) \emph{nett} (regular).
...
Derivation: \emph{*márōn} \(\rightarrow\) \emph{mare} (regular).
...
related variant forms \emph{mera / mere}
...
oblique \emph{maran} and variant \emph{mere / mera}
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014};
\citeproc{ref-ClarkHall1960}{Clark Hall 1960, 213}).

\subsubsection{coat --- OE rocc}\label{coat-oe-rocc}

Derivation: \emph{*rúkkaz} \(\rightarrow\) \emph{rocc} (regular).
...
With \emph{*rúkkaz} as the selected input, Northwest Germanic u-lowering
and later loss of final \emph{-a} yield \emph{rocc} as a regular
outcome.
...
Derivation: \emph{*skḗpą} \(\rightarrow\) \emph{sċēap} (regular).
...
WS scéap}, while Orel preserves a Proto-Germanic noun of the
...
Derivation: \emph{*skíllingaz} \(\rightarrow\) \emph{sċilling}
(regular).
...
Outcome: \emph{sċilling}

\paragraph{Reconstruction and comparative
evidence}\label{reconstruction-and-comparative-evidence-44}

Kroonen treats the cognate set under \emph{*skellinga- \textasciitilde{}
*skillinga-} and connects it with \emph{*skeld-linga-}, while Orel
likewise gives the coin word with OE \emph{scilling} among the reflexes
(\citeproc{ref-Kroonen2013}{Kroonen 2013}; \citeproc{ref-Orel2003}{Orel
2003}). The selected input \emph{*skíllingaz} is the nominative-style
form used here to represent that inherited \emph{*-ing-} derivative.
...
Kroonen's \emph{*skellinga- \textasciitilde{} *skillinga-} and his
internal analysis \emph{*skeld-linga-} belong to the etymological
background of the cognate set. The selected input \emph{*skíllingaz} is
the specific form used for the derivation represented here
(\citeproc{ref-Kroonen2013}{Kroonen 2013}).

\subsubsection{show --- OE sċēawian}\label{show-oe-sux10bux113awian}

Derivation: \emph{*skáwōjaną} \(\rightarrow\) \emph{sċēawian} (regular).
...
records the Old English family as \emph{scēawian, scāwian}, which places
this entry in the ordinary show-verb set rather than in a special
...
Derivation: \emph{*slḗpaną} \(\rightarrow\) \emph{slǣpan} (regular).
...
form used here for that inherited sleep-verb.

\paragraph{Old English evidence}\label{old-english-evidence-46}

Clark Hall gives \emph{slæpan} with preterite \emph{slēp, slēap}, and
Bright likewise lists \emph{slæpan (slāpan), slēp slēpon slēpen}
(\citeproc{ref-ClarkHall1960}{Clark Hall 1960};
...
not the preterite forms and not the separate noun \emph{slǣp}.
...
spellings such as West Saxon \emph{slāpan/slæpan} and Anglian or Kentish
\emph{slēpan}, but those do not displace the infinitive chosen here
...
to noun or lookup background and \emph{slēp/slēap} are preterite forms
(\citeproc{ref-ClarkHall1960}{Clark Hall 1960};
\citeproc{ref-BrightCassidyRingler1971}{Bright 1971, 435}).

\subsubsection{smear --- OE smierwan}\label{smear-oe-smierwan}

Derivation: \emph{*smérwijaną} \(\rightarrow\) \emph{smierwan}
(regular).
...
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). The selected
input \emph{*smérwijaną} therefore represents the Kroonen-aligned PGmc
layer, while the later-stage dialect split belongs to a different
chronological level.
...
Derivation: \emph{*spánnaną} \(\rightarrow\) \emph{spannan} (regular).
...
2013}). The selected input \emph{*spánnaną} is the infinitive-style form
used here for that same verbal lexeme.
...
Derivation: \emph{*spárrô} \(\rightarrow\) \emph{spearra} (regular).
...
is the OE-facing nominal form used here for that same lexeme.
...
Derivation: \emph{*stéllijaną} \(\rightarrow\) \emph{stillan} (regular).
...
selected target is the verb \emph{stillan}, not the adjective.
...
Derivation: \emph{*súmaraz} \(\rightarrow\) \emph{sumer} (regular).
...
selected form is the regularized \emph{e}-form, while the common
...
headword spelling and \emph{sumeres/sumere} show that the
...
Derivation: \emph{*súndrōjaną} \(\rightarrow\) \emph{sundrian}
(regular).
...
Derivation: \emph{*swálwōn} \(\rightarrow\) \emph{swealwe} (regular).
...
\emph{*swḯną} \(\rightarrow\) \emph{swīn} (regular).

\paragraph{Derivation trace}\label{derivation-trace-54}

Proto input: \emph{*swḯną}
...
From selected input \emph{*swḯną}, loss of the final nasal vowel yields
\emph{swīn}. The outcome is therefore the regular monosyllabic noun with
preserved long root \emph{ī}.

\paragraph{Source note}\label{source-note-4}

The selected input writes stressed long \emph{ī} as \emph{*ḯ}, so
comparative \emph{*swī́ną} and derivational \emph{*swḯną} represent the
same lexical form.

\subsubsection{think --- OE þenċan}\label{think-oe-uxfeenux10ban}

Derivation: \emph{*θánkijaną} \(\rightarrow\) \emph{þenċan} (regular).
...
Bosworth-Toller preserves the verb under \emph{þencan/geþencan}, and the
citation form here is the ordinary infinitive \emph{þenċan}
(\citeproc{ref-BosworthToller1898}{Bosworth and Toller 1898}).
...
Derivation: \emph{*θúrnaz} \(\rightarrow\) \emph{þorn} (regular).
...
masculine pair \emph{*þurnuz \textasciitilde{} *þurnaz}
(\citeproc{ref-Kroonen2013}{Kroonen 2013}; \citeproc{ref-Orel2003}{Orel
2003}). The selected input \emph{*θúrnaz} belongs to that same
comparative family.
...
\emph{*tḯdiz} \(\rightarrow\) \emph{tīd} (regular).

\paragraph{Derivation trace}\label{derivation-trace-57}

Proto input: \emph{*tḯdiz}
...
From \emph{*tḯdiz}, final \emph{z} is lost and the high final vowel
...
Derivation: \emph{*táikną} \(\rightarrow\) \emph{tācn} (regular).
...
Derivation: \emph{*tūną} \(\rightarrow\) \emph{tūn} (regular).
...
Kroonen cites \emph{*tūna-} `fenced area', while Orel gives \emph{*tūnan
\textasciitilde{} *tūnaz} (\citeproc{ref-Kroonen2013}{Kroonen 2013};
\citeproc{ref-Orel2003}{Orel 2003, 452}). The selected input
\emph{*tūną} is the simple citation-form noun used in the derivation.
...
Derivation: \emph{*wádaną} \(\rightarrow\) \emph{wadan} (regular).
...
Derivation: \emph{*wérpaną} \(\rightarrow\) \emph{weorpan} (regular).
...
Derivation: \emph{*wáskaną} \(\rightarrow\) \emph{wascan} (regular).
...
Derivation: \emph{*wáxsaną} \(\rightarrow\) \emph{weaxan} (regular).
...
preserved here, since \emph{xs \textgreater{} s} belongs to forms where
another consonant follows, such as \emph{wæstm}, not to the infinitive
itself (\citeproc{ref-Campbell1959}{Campbell 1959};
\citeproc{ref-SieversBrunner1965}{Sievers 1965}).

\paragraph{Lexical note}\label{lexical-note-3}

The target here is the infinitive \emph{weaxan}. Noun \emph{weax} and
preterite \emph{wax/wēox} belong to different lexical or paradigm slots.

\subsubsection{way --- OE weġ}\label{way-oe-weux121}

Derivation: \emph{*wégaz} \(\rightarrow\) \emph{weġ} (regular).
...
Derivation: \emph{*wíndaną} \(\rightarrow\) \emph{windan} (regular).
...
selected target is the ordinary infinitive of the strong verb.

\paragraph{Development to Old
English}\label{development-to-old-english-67}

The selected input \emph{*wíndaną} yields the regular infinitive
...
Derivation: \emph{*wálθuz} \(\rightarrow\) \emph{weald} (regular).
...
in the same family. The selected target is normalized \emph{weald}, not
the variant form (\citeproc{ref-ClarkHall1960}{Clark Hall 1960};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).

\subsubsection{yarn --- OE ġearn}\label{yarn-oe-ux121earn}

Derivation: \emph{*gárną} \(\rightarrow\) \emph{ġearn} (regular).
...
early chain \emph{*garna \textgreater{} *geern \textgreater{} *gearn
\textgreater{} OE gearn} (\citeproc{ref-Kroonen2013}{Kroonen 2013};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, 220}). The
selected input \emph{*gárną} is the nominal citation form used here,
...
not replace the selected input \emph{*gárną}.
...
\emph{*kwéðuz} \(\rightarrow\) \emph{cwedu} (attested variant).
...
cwudu \textgreater{} cudu} and also note late West Saxon \emph{cweodu};
Clark Hall gives \emph{cwudu}, \emph{cweodu}, and \emph{cudu}
...
Derivation: \emph{*téxun} \(\rightarrow\) \emph{tēon} (attested
variant).
...
\emph{*tëhun}, while West Saxon \emph{tien, tȳn} belong to a different,
...
tēa} (\citeproc{ref-Campbell1959}{Campbell 1959, sect. 682}). Brunner
likewise lists West Saxon \emph{tien, tȳn} beside \emph{tēn, tēo, tēa}
in other dialects (\citeproc{ref-SieversBrunner1965}{Sievers 1965, sect.
325}).
...
\emph{tēon} is therefore a \emph{normalized spelling} of that
un-umlauted base.

\paragraph{Development to Old
English}\label{development-to-old-english-71}

From \textbf{\emph{*téxun}, lowering of medial unstressed \emph{u} gives
\emph{*téxon}}, breaking gives \textbf{\emph{*téoxon}, loss of
intervocalic \emph{h/x} yields \emph{*téoon}}, and contraction produces
**\_*tḗon\_\_, written \_\_tēon\_**. This is the regular bare-cardinal
path.

The umlauted forms \textbf{\emph{tien / tīen}} belong to a different
...
selected target \\
\emph{tien / tīen} & attested West Saxon umlauted simplex forms &
genuine OE variants, but not the bare-cardinal line modeled here \\
\emph{tēn / tēo / tēa} & attested un-umlauted simplex variants in other
dialects & support the same branch as the selected comparison form \\
\end{longtable}
}

\subsubsection{three --- OE þrīe}\label{three-oe-uxferux12be}

Derivation: \emph{*θréjez} \(\rightarrow\) \emph{þrīe} (attested
variant).
...
(\citeproc{ref-Kroonen2013}{Kroonen 2013}). The input
\textbf{\emph{*θréjez}} is therefore best understood as the inherited
...
line must be kept apart from feminine-neuter \textbf{\emph{þrēo}} and
from later reduced spellings of the masculine form.
...
dative \emph{þrim}, adding that late West Saxon has \emph{þry, þri} for
\emph{þrīe} (\citeproc{ref-Campbell1959}{Campbell 1959, sect. 683}).
Fulk presents the same masculine \emph{þrīe} beside the wider numeral
paradigm (\citeproc{ref-Fulk2018}{Fulk 2018, sect. 10.1}).
...
From \textbf{\emph{*θréjez}, loss of final \emph{-z} leaves a form of
the \emph{*θréje}} type. The following \textbf{\emph{j}} fronts the stem
vowel, then vocalizes between vowels, and contraction yields
\textbf{\emph{þrīe}}. The compact trace records the same sequence as
\textbf{\emph{*θrije \textgreater{} *θriie \textgreater{} þrīe}}.
...
target \\
\emph{þrī / þry} & later reduced masculine variant & genuine OE variant,
but not the conservative comparison form \\
...
Derivation: \emph{*wábsaz} \(\rightarrow\) \emph{wæfs} (attested
variant).
...
earlier \emph{waefs / wæfs} through restricted metatheses
...
English also shows \emph{wæps} and \emph{wæsp / wasp}, and dictionary
practice often favors \emph{wæps} or later spellings as headwords
(\citeproc{ref-ClarkHall1960}{Clark Hall 1960}).
...
The later forms \emph{wæps} and \emph{wæsp / wasp} belong to subsequent,
lexically restricted metatheses. They are genuine Old English forms, but
they are later within the variant history.
...
but secondary \\
\emph{wæsp / wasp} & later West Saxon metathesized variant & genuine OE
doublet, but not the selected form \\
\end{longtable}
}
...
\emph{*búttmaz} \(\rightarrow\) \emph{botm} (early analogy).
...
*buttaz}, summarized as \emph{*budman- \textasciitilde{} *buttman-}, and
gives Old English \emph{botm} as the reflex
...
Orel likewise preserves both sides of the comparison under \emph{*budmaz
*butmaz} (\citeproc{ref-Orel2003}{Orel 2003, 100}). The selected input
is thus a historical stem choice, not an arbitrary respelling.
...
\emph{*brándas} \(\rightarrow\) \emph{brandes} (early analogy).
...
\emph{*bréustą} \(\rightarrow\) \emph{brēost} (early analogy).
...
\emph{brēost}, with the expected \emph{eu \textgreater{} ēo} vowel
history (\citeproc{ref-Campbell1959}{Campbell 1959}). No special repair
is needed once the correct thematic formation is chosen.
...
\emph{*déliz} \(\rightarrow\) \emph{dile} (early analogy).
...
\emph{*fástijaną} \(\rightarrow\) \emph{festan} (early analogy).
...
\emph{*fláskōn} \(\rightarrow\) \emph{flasce} (early analogy).
...
ending, reaching \emph{flasce}. Once the weak feminine formation is
chosen, the noun follows a regular path to its Old English shape
(\citeproc{ref-Campbell1959}{Campbell 1959}).
...
\emph{*fúlgijaną} \(\rightarrow\) \emph{fylġan} (early analogy).
...
Ringe and Taylor make the split explicit as \emph{PNWGmc *fulgija-
\textasciitilde{} *fulgai- \textgreater{} OE fylgan \textasciitilde{}
folgian} and describe it as a dual formation that probably reflects an
older alternation between j-present and e-stative
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, 293--94}). This
is a stem-class choice, not a spelling choice. The selected input
\emph{*fúlgijaną} belongs to the class-I \emph{*fulgija- / *fulgjan-}
branch; the citation form \emph{*fulgēną} belongs to the parallel
class-II history behind \emph{folgian}.
...
Bright treats \emph{fylg(e)an} as a survival of the older conjugation
and contrasts it with forms that have conformed to the Second
Conjugation, \emph{folgian, folgode}
(\citeproc{ref-BrightCassidyRingler1971}{Bright 1971}). The relevant
comparison form in this entry is therefore the class-I verb \emph{fylgan
/ fylgean}, here normalized as \emph{fylġan}. The spelling with \emph{}
...
\emph{*-ē- / *-ai-} branch and is not the form modeled here.
...
English outcome of the class-I \emph{*fulgija- / *fulgjan-} formation,
whereas \emph{folgian} belongs to the parallel class-II branch.

\subsubsection{gall --- OE ġealla}\label{gall-oe-ux121ealla}

Derivation: citation reconstruction \emph{*gállą}; selected input
\emph{*gállô} \(\rightarrow\) \emph{ġealla} (early analogy).
...
and palatal \emph{} made explicit.
...
\emph{*knéxtaz} \(\rightarrow\) \emph{cniht} (early analogy).
...
this noun: Ringe and Taylor cite \emph{*kneht}, Orel gives
\emph{*knextaz}, and Kluge-Seebold likewise points to \emph{*knehta-}
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014};
\citeproc{ref-Orel2003}{Orel 2003, 256};
\citeproc{ref-KlugeSeebold2011}{Kluge 2011}). The selected input
\emph{*knéxtaz} follows that comparative evidence.
...
Campbell's discussion of related forms such as plural \emph{cneohtas}
helps show the same vowel environment from another point in the paradigm
(\citeproc{ref-Campbell1959}{Campbell 1959}).
...
\emph{cniht} (\citeproc{ref-Campbell1959}{Campbell 1959};
\citeproc{ref-SieversBrunner1965}{Sievers 1965}). With that corrected
input, the derivation is straightforward.
...
\emph{*xláðaną} \(\rightarrow\) \emph{hladan} (early analogy).
...
\emph{*laθōjaną}, but the Old English verb \emph{hladan} belongs to a
strong-verb line with voiced consonantism in the selected input
\emph{*xláðaną} (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014};
\citeproc{ref-Kroonen2013}{Kroonen 2013}). The two forms are related as
members of one word family, but they do not play the same role in the OE
derivation.
...
Old English dictionaries record the verb as \emph{hladan} and preserve
the expected strong-verb paradigm material around it
(\citeproc{ref-BosworthToller1898}{Bosworth and Toller 1898, 559};
\citeproc{ref-ClarkHall1960}{Clark Hall 1960}). The target is an
...
(\citeproc{ref-Campbell1959}{Campbell 1959};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). The resulting
Old English infinitive is \emph{hladan}.
...
\emph{*láppô} \(\rightarrow\) \emph{lappa} (early analogy).
...
\emph{*xláxjaną} \(\rightarrow\) \emph{hliehhan} (early analogy).
...
\emph{*láimą} \(\rightarrow\) \emph{lām} (early analogy).
...
\emph{*lúnganjō} \(\rightarrow\) \emph{lungen} (early analogy).
...
\emph{*nábulô} \(\rightarrow\) \emph{nafola} (early analogy).
...
\emph{*nabulō \textgreater{} *næbula \textgreater{} nafola}
(\citeproc{ref-Kroonen2013}{Kroonen 2013};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). The difference
is one of stage and notation rather than of lexeme identity: the
selected input \emph{*nábulô} is the pre-syncope form needed for the Old
English development.
...
The Old English record includes \emph{nafola}, \emph{nafela}, and Corpus
\emph{nabula} (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014};
\citeproc{ref-Campbell1959}{Campbell 1959}). The target of this entry is
the nominative singular \emph{nafola}, the form that matches the
selected derivational pathway most directly.
...
From \emph{*nábulô}, the trace gives \emph{*nábolô} by lowering of
unstressed \emph{u}, then \emph{*næbolô} by Anglo-Frisian brightening,
followed by A-restoration to \emph{*nabolô}
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). Intervocalic
\emph{b} then surfaces as \emph{f}, and final weak-tail shortening gives
\emph{nafola}.
...
\emph{*xnékkô} \(\rightarrow\) \emph{hnecca} (early analogy).
...
Old English dictionaries record the weak masculine noun \emph{hnecca}
(\citeproc{ref-ClarkHall1960}{Clark Hall 1960};
...
\emph{*nḗðlō} \(\rightarrow\) \emph{nǣdl} (early analogy).
...
Ringe and Taylor treat the word as a voiced/voiceless alternant, and
Ringe also cites the pair \emph{*nēþlō- \textasciitilde{} *nēdlō-}
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014};
\citeproc{ref-Ringe2006}{Ringe 2006}). The selected input \emph{*nḗðlō}
is the voiced Verner-grade form used for the Old English comparison,
while the citation form \emph{*nḗθlō} remains the broader lexeme label.
...
Old English has the attested citation form \emph{nǣdl}
(\citeproc{ref-ClarkHall1960}{Clark Hall 1960}). Campbell lists
\emph{nédl} among the expected unbroken forms after \emph{t} and
\emph{d}, and Hogg also includes \emph{nidi} / \emph{nǣdl} in the same
broader cluster history (\citeproc{ref-Campbell1959}{Campbell 1959};
\citeproc{ref-Hogg1992}{Hogg 1992}).
...
From \emph{*nḗðlō}, the trace gives \emph{*nḗdlō} by dental hardening,
then \emph{*nḗdlu}, \emph{*nǣdlu}, and finally \emph{*nǣdl}, hence
\emph{nǣdl} (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014};
\citeproc{ref-Campbell1959}{Campbell 1959}).
...
\emph{*núsō} \(\rightarrow\) \emph{nosu} (early analogy).
...
\textasciitilde{} *nusō-} and adds that the root \emph{*nus-} is likely
to have arisen as a secondary zero grade after a remodeling of the older
paradigm (\citeproc{ref-Kroonen2013}{Kroonen 2013}). Campbell is more
specific for Old English, citing \emph{nosu \textless{} *nusō}
(\citeproc{ref-Campbell1959}{Campbell 1959}).
...
Old English form discussed here. Orel's \emph{*nasō \ldots{} OE nasu}
...
trace: \emph{*núsō \textgreater{} *nósō \textgreater{} *nósu
\textgreater{} nosu}. The early special step lies in the choice of the
...
\emph{*sápą} \(\rightarrow\) \emph{sæp} (early analogy).
...
of a primary n-stem \emph{*safō, gen. *sappaz}
(\citeproc{ref-Kroonen2013}{Kroonen 2013}). Orel preserves the
comparative notation \emph{*sapōn \textasciitilde{} *sapan}
(\citeproc{ref-Orel2003}{Orel 2003}), while Kluge-Seebold instead gives
West Germanic \emph{*sapi-} and still cites Old English \emph{sæp n.}
(\citeproc{ref-KlugeSeebold2011}{Kluge 2011}).
...
Hall 1960}), and Kluge-Seebold likewise cites \emph{ae. sæp n.}
...
for the Old English form (\citeproc{ref-Orel2003}{Orel 2003}).
...
sepe} rather than to \emph{sæp}. The special step in this entry is
...
\emph{*sáiwiz} \(\rightarrow\) \emph{sǣ} (early analogy).
...
\textgreater{} *sei \textgreater{} OE sǣ}
...
\emph{sea} as continuing the same \emph{*saiui- \textgreater{} *sǣi}
history, with loss of \emph{u/w} before \emph{i}
...
\textgreater{} *sāwiz} and final \emph{*-z} loss \emph{*sāwiz
\textgreater{} *sāwi}, the non-initial \emph{*w} disappears before
...
\textgreater{} *sāwiz \textgreater{} *sāwi \textgreater{} *sāi
\textgreater{} *sǣi \textgreater{} sǣ}
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, sect. 6.7.1};
\citeproc{ref-Campbell1959}{Campbell 1959, sect. 406}).
...
\emph{*w \ldots{} *i} environment needed for the documented chronology &
sǣ & useful comparative label, but not the selected OE-facing input \\
...
\emph{*síbi} \(\rightarrow\) \emph{sife} (early analogy).
...
Kluge-Seebold gives \emph{wg. *sibi- n.~\ldots{} ae. sife}, and Campbell
...
(\citeproc{ref-Kroonen2013}{Kroonen 2013}). Orel's \emph{*sibaz \ldots{}
OE sife} preserves a broader handbook notation, but that a-stem shape
does not fit the Old English form treated here
(\citeproc{ref-Orel2003}{Orel 2003}).
...
From \emph{*síbi}, the documented trace gives \emph{*síβi \textgreater{}
*síβe \textgreater{} sife}. Medial \emph{b} is realized as a spirant and
later written \emph{f}, while the final unstressed \emph{i} lowers to
\emph{e}. The older s-stem background \emph{*sib-iz} explains the
morphology, but the selected input \emph{*síbi} is the immediate
...
\emph{*spárōjaną} \(\rightarrow\) \emph{sparian} (early analogy).
...
/ *sparēnan} (\citeproc{ref-Kroonen2013}{Kroonen 2013};
\citeproc{ref-Orel2003}{Orel 2003}). Ringe and Taylor, however,
reconstruct \emph{*sparai- \textasciitilde{} *sparja-} for the English
branch and derive the citation verb from a class-II line
...
\emph{*stábaz} \(\rightarrow\) \emph{stæf} (early analogy).
...
\textasciitilde{} *stabaz} (\citeproc{ref-Orel2003}{Orel 2003}), and
Kluge-Seebold explicitly marks \emph{g. *stabi-/a-}
...
is \emph{*stábaz \textgreater{} *stába \textgreater{} *stáb
\textgreater{} *stæb \textgreater{} stæf}. A direct continuation of
\emph{*stábiz}, by contrast, would produce i-mutated \emph{stefe} rather
...
*stabaz}; Kluge \emph{*stabi-/a-} & source-level stem-class uncertainty
& stæf & useful comparative background, but not a single OE-facing
input \\
...
\emph{*stébnō} \(\rightarrow\) \emph{stefn} (early analogy).
...
comparative label \textbf{\emph{*stámnaz}. Ringe and Taylor cite
\emph{*stebnō}} for the noun continued by Gothic \emph{stibna} and Old
English \emph{stebn \textgreater{} stefn \textgreater{} stemn}
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, 330}). Orel
likewise gives \textbf{\emph{*stebnō \textasciitilde{} *stemnō}},
whereas Kroonen prefers \textbf{\emph{*stimnō-}}, and Fulk describes the
...
These forms belong to the Old English noun \textbf{\emph{stefn}} `voice,
sound'. The selected input \textbf{\emph{*stébnō}} is therefore best
...
citation label \textbf{\emph{*stámnaz}}.
...
(\citeproc{ref-ClarkHall1960}{Clark Hall 1960}). Ringe and Taylor give
the OE chronology directly as \emph{stebn \textgreater{} stefn
\textgreater{} stemn} (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor
2014, 330}).

Bülbring and Luick treat \emph{stemn} as a later West Saxon development
from older \emph{stefn}, produced by \emph{fn \textgreater{} mn} only
after the earlier period of nasal influence on \emph{e}
...
From \textbf{\emph{*stébnō}, raising of final long \emph{ō} gives a
\emph{*stébnu}} stage. Regular fricativization of \emph{b} before
\emph{n} then yields \textbf{\emph{*stéβnu}}, and loss of the final high
vowel leaves **\_*stéβn\_\_, written \_\_stefn\_** in Old English. The
later form \textbf{\emph{stemn}} belongs to a separate West Saxon
assimilation after this stage (\citeproc{ref-RingeTaylor2014}{Ringe and
Taylor 2014, 330}; \citeproc{ref-Bulbring1902}{Bülbring 1902, sect.
445}).
...
\emph{stefn} by \emph{fn \textgreater{} mn} & real OE variant, but not
...
\emph{*swánas} \(\rightarrow\) \emph{swanes} (early analogy).
...
\textbf{\emph{*swanaz}} (\citeproc{ref-Orel2003}{Orel 2003, 367}). The
selected input \textbf{\emph{*swánas}} is not a competing lexeme
reconstruction. It is the \textbf{genitive singular} of the same
paradigm.

The question here is therefore one of paradigm cell rather than stem
history. The citation form remains \emph{*swanaz \textgreater{} swan};
the selected comparison form is the genitive singular \emph{*swánas
\textgreater{} swanes}.
...
however, also records the exact inflected form \emph{swanes}, glossing
\emph{swan, m., swan: gs. swanes} and citing the phrase \emph{swanes
feðre} (\citeproc{ref-BrightCassidyRingler1971}{Bright 1971}).
...
From \textbf{\emph{*swánas}, Anglo-Frisian brightening gives
\emph{*swánæs}}, and subsequent merger of unstressed \emph{æ} with
\emph{e} yields \textbf{\emph{swanes}}. The comparison is
straightforward once the genitive singular is chosen as the relevant
cell.
...
\emph{*θūs-èndi} \(\rightarrow\) \emph{þūsend} (early analogy).
...
Kroonen reconstructs the Germanic numeral as \textbf{\emph{*þūsundī-}}
and cites Old English \textbf{\emph{þūsend}} among its continuations
(\citeproc{ref-Kroonen2013}{Kroonen 2013, 554}). The selected input
\textbf{\emph{*θūs-èndi}} is not the same claim. It is an OE-oriented
...
show \textbf{\emph{þūsend}}, while related languages such as Old Saxon
and Old High German keep \emph{u} in the second syllable?
...
\textbf{\emph{*þȳsend}} type rather than attested \textbf{\emph{þūsend}
...
\textbf{\emph{e}}. Luick compares the word with \textbf{\emph{ærende}}
...
(\citeproc{ref-GermanicSlavicBaltic2025}{\emph{{Germanic}, {Slavic}, and
{Baltic} {`Thousand'}} 2025, sect. 2.1.4}).

The selected transponent \textbf{\emph{*θūs-èndi}} captures the OE-side
state from which the documented trace reaches \textbf{\emph{þūsend}}.
...
\emph{*tímbrą} \(\rightarrow\) \emph{timber} (early analogy).
...
Kroonen reconstructs the noun as \textbf{\emph{*timbra-}} and cites Old
English \textbf{\emph{timber}} among its continuations
(\citeproc{ref-Kroonen2013}{Kroonen 2013}). Ringe and Taylor instead
state the history from \textbf{\emph{PGmc *timra\textbf{ through West
Germanic }*timbr}} to Old English \textbf{\emph{timber}}
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).

The difference is therefore not over the Old English noun itself. It
concerns whether medial \textbf{\emph{b}} belongs in the comparative
citation form or appears in an early pre-Old-English stage of the
cluster.
...
\textbf{\emph{*tímbr}}, and epenthetic \textbf{\emph{e} in the final
cluster yields \emph{timber}}. Ringe and Taylor's \textbf{\emph{*timra
\textgreater{} *timbr \textgreater{} OE timber}} and the handbook
treatment of this epenthetic vowel point to the same Old English result
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014};
...
the selected input \\
Ringe-Taylor citation line & \emph{*timra \textgreater{} *timbr} &
...
\emph{*wákaną} \(\rightarrow\) \emph{wacan} (early analogy).
...
Kroonen gives the strong verb as \textbf{\emph{*wakan-}} with Old
English \textbf{\emph{wacan}} (\citeproc{ref-Kroonen2013}{Kroonen
2013}). Ringe and Taylor separately derive Old English
\textbf{\emph{wacian\textbf{ from weak }*wakai- \textasciitilde{}
*wakja-}} (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, sect.
3.3.2}).

The difference is therefore lexical and class-based, not graphic. Strong
\textbf{\emph{wacan} `wake up, arise' and weak \emph{wacian}} `be awake,
...
With strong \textbf{\emph{*wákaną}}, Anglo-Frisian brightening first
gives a form of the \textbf{\emph{*wækaną} type. A-restoration then
returns \emph{a}}, and the ordinary tail reductions yield
\textbf{\emph{wacan}. The weak verb \emph{wacian}} belongs to a
...
\textasciitilde{} *wakja-} & OE \emph{wacian} and related weak forms &
related lexeme, but not the target of this entry \\
...
\emph{*wátōr} \(\rightarrow\) \emph{wæter} (early analogy).
...
Kroonen reconstructs a heteroclitic noun
\textbf{\emph{*watar-\textasciitilde*watan-}} and states that the
Proto-Germanic material points to \textbf{\emph{*watōr, *watenaz}}
(\citeproc{ref-Kroonen2013}{Kroonen 2013, 616}). Ringe and Taylor
likewise start from singular \textbf{\emph{*wator}} before the Old
...
form that actually corresponds to Old English \textbf{\emph{wæter}}. The
...
\emph{wæteres, wætere, wæter(u), wætera, wæterum}
(\citeproc{ref-BrightCassidyRingler1971}{Bright 1971}). Ringe and Taylor
add the dialectal contrast between West Saxon \emph{weeter} and Mercian
\emph{weter} (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014,
sect. 6.5.2}).
...
From **\_*wátōr\textbf{, pre-final }*ō\_\_ becomes \textbf{a} before
final \_\_r\_\textbf{, yielding }\_*watar\_**
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, sect. 3.1.4}).
Anglo-Frisian brightening then gives **\emph{*wætær}, and merger of
unstressed \_æ/e\_\_ yields \_\_wæter\_**.
...
heteroclitic stem notation & \emph{*watar-\textasciitilde*watan-} &
source-faithful comparative reconstruction & explains why a singular
...
\emph{*xwálaz} \(\rightarrow\) \emph{hwæl} (early analogy).
...
\textbf{\emph{*xwalaz}} and notes some mixed \textbf{\emph{*xwaliz}}
evidence (\citeproc{ref-Orel2003}{Orel 2003}). Kroonen instead cites
\textbf{\emph{*hwali-}} (\citeproc{ref-Kroonen2013}{Kroonen 2013}).

Both notations agree on inherited initial \textbf{\emph{hw-/xw-}}, but
they differ in stem label. The a-stem-like input followed here is closer
to Orel's notation than to Kroonen's citation form.
...
326}). The comparison form is normalized here as \emph{hwæl} for the
...
From \textbf{\emph{*xwálaz\textbf{, final }-z} disappears and bare final
\emph{-a}} is lost. Anglo-Frisian fronting then yields \textbf{\emph{æ}}
in the closed monosyllable, and Old English orthography writes
\textbf{\emph{hwæl}}.
...
\emph{*xwḯnaną} \(\rightarrow\) \emph{hwīnan} (early analogy).

\paragraph{Derivation trace}\label{derivation-trace-105}

Proto input: \emph{*xwḯnaną}
...
\emph{*hwinan-}, and Orel likewise distinguishes strong \emph{*xwinanan}
from weak \emph{*wainōjanan} (\citeproc{ref-Kroonen2013}{Kroonen 2013};
\citeproc{ref-Orel2003}{Orel 2003}). Ringe and Taylor make the same
split at the Northwest Germanic level, linking Old Norse \emph{hvina}
and Old English \emph{hwinan} to the same strong verb
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).
...
(\citeproc{ref-Kroonen2013}{Kroonen 2013}; \citeproc{ref-Orel2003}{Orel
2003}). The selected input \emph{*xwḯnaną} therefore represents a
...
whistle' (\citeproc{ref-ClarkHall1960}{Clark Hall 1960}). Seebold keeps
the verb among the strong verbs and notes that only a present-tense
attestation is directly preserved, while Sievers-Brunner likewise lists
\emph{hwinan stv.} (\citeproc{ref-Seebold1970}{Seebold 1970};
\citeproc{ref-SieversBrunner1965}{Sievers 1965}).
...
Once the strong-verb input \emph{*xwḯnaną} is selected, the path to Old
...
selected strong verb & \emph{*xwḯnaną} & compact-trace output:
...
\emph{*wḯθagą} \(\rightarrow\) \emph{wīþiġ} (early analogy).

\paragraph{Derivation trace}\label{derivation-trace-106}

Proto input: \emph{*wḯθagą}
...
\emph{*wḯθagą} is thus a formation choice rather than a mere respelling
...
spelling with macrons and palatal \emph{}.
...
explanation is why the selected Proto-Germanic input is \emph{*wḯθagą}
rather than a comparative \emph{*wīþja-}-type headword.

\paragraph{Development to Old
English}\label{development-to-old-english-106}

From \emph{*wḯθagą}, Anglo-Frisian brightening gives a fronted vowel in
...
selected \emph{*-ag-} derivative & *wḯθagą & compact-trace output:
...
\emph{*wír-àldu} \(\rightarrow\) \emph{weorold} (early analogy).
...
pre-Old-English chain \emph{*weraldu \textgreater{} *weruld}
(\citeproc{ref-Orel2003}{Orel 2003};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, 341}).
...
give West Saxon \emph{weorold \textasciitilde{} worold}, Mercian
...
\emph{*weraldiz \textgreater{} *weraldu \textgreater{} *weruld} chain
while preserving the \emph{*wir-} notation of the selected comparative
label (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, 341}). The
...
\emph{*weraldiz \textgreater{} *weraldu \textgreater{} *weruld} &
Ringe-Taylor background chain to OE \emph{weorold \textasciitilde{}
worold} & explains the older comparative literature cited for the
word \\
...
\emph{*bánnas} \(\rightarrow\) \emph{bannes} (late analogy).
...
\emph{*bázjas} \(\rightarrow\) \emph{berġes} (late analogy).
...
*bazja-} (\citeproc{ref-Kroonen2013}{Kroonen 2013}). The selected input
\emph{*bázjas} is therefore not a rival lexeme headword, but a specific
genitive singular cell drawn from that paradigm.
...
gemination repair (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor
2014}).
...
and Taylor 2014}). This points to paradigm choice rather than to an
...
\emph{*kūi} \(\rightarrow\) \emph{cȳ} (late analogy).
...
\emph{*kō- \textasciitilde{} *ku-}, explicitly \emph{nom. *kōz, obl.
*kū-} (\citeproc{ref-Kroonen2013}{Kroonen 2013, 299}). The citation form
therefore belongs to the nominative singular, whereas the selected input
\emph{*kūi} belongs to the oblique stem.

Ringe and Taylor also posit a later PNWGmc nominative \emph{*kūaz
\textgreater{} *kūz} behind Old English \emph{cū}
...
\emph{cū(e), cȳ, cūs}, dat.sg. \emph{cȳ}, nom.-acc.pl. \emph{cȳ}, and
dat.pl. \emph{cūm} (\citeproc{ref-ClarkHall1960}{Clark Hall 1960}).
Ringe and Taylor likewise state that the root-noun \emph{cū} exhibits
dat.sg., nom.-acc.pl. \emph{cȳ \textless{} *cūi}, dat.pl. \emph{cūm
\textless{} *cūm}, and apparently gen.sg. \emph{cā \textless{} *cūiz}
...
\emph{*kūi \textgreater{} *kȳi \textgreater{} *kȳ \textgreater{} cȳ}.
...
Hall: \emph{cū(e), cȳ, cūs} & gen.sg. variable & too unstable to control
the entry \\
\end{longtable}
}
...
\emph{*fúnðanǭ} \(\rightarrow\) \emph{fundene} (late analogy).
...
\emph{*fúrxtīnaz} \(\rightarrow\) \emph{fyrhte} (late analogy).
...
\emph{*xámaras} \(\rightarrow\) \emph{hameres} (late analogy).
...
(\citeproc{ref-Kroonen2013}{Kroonen 2013}; \citeproc{ref-Orel2003}{Orel
2003, 197}; \citeproc{ref-ClarkHall1960}{Clark Hall 1960, 160}). The
selected input \emph{*xámaras} is the genitive singular of that same
...
(\citeproc{ref-SieversBrunner1965}{Sievers 1965}). The relevant
comparison form here is the attested genitive singular \emph{hameres}.
...
\emph{*xábēθi} \(\rightarrow\) \emph{hæfeþ} (late analogy).
...
(\citeproc{ref-Kroonen2013}{Kroonen 2013};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). Within that
...
stem in the infinitive from a non-geminating \emph{-ai- / -ē-} stem in
...
(\citeproc{ref-BosworthToller1898}{Bosworth and Toller 1898};
\citeproc{ref-ClarkHall1960}{Clark Hall 1960, 157}). Campbell notes
occasional unsyncopated forms that support the normalized target
\emph{hæfeþ} (\citeproc{ref-Campbell1959}{Campbell 1959}).
...
From \emph{*xábēθi}, early apocope of final \emph{-i}, subsequent vowel
changes, and the regular fricative outcome of \emph{b} in this
environment yield \emph{hæfeþ} (\citeproc{ref-RingeTaylor2014}{Ringe and
Taylor 2014}; \citeproc{ref-Campbell1959}{Campbell 1959}). The
derivation of the finite form itself is regular.
...
\emph{*xémonų} \(\rightarrow\) \emph{heofon} (late analogy).
...
\emph{*hemina-* \textasciitilde{} *hemna-*}
(\citeproc{ref-Kroonen2013}{Kroonen 2013}). The selected input
\emph{*xémonų} is an oblique singular form within that paradigm rather
than the lexeme-level citation form \emph{*xémenaz}.

That difference matters for the West Saxon target. The back-vocalic
oblique stem provides the environment for the diphthong seen in
\emph{heofon}, whereas a front-vocalic stem yields Anglian or Mercian
\emph{hefen} / \emph{heofen} (\citeproc{ref-Campbell1959}{Campbell
1959}; \citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).
...
word (\citeproc{ref-Campbell1959}{Campbell 1959}).
...
\emph{*líbēθi} \(\rightarrow\) \emph{lifeþ} (late analogy).
...
(\citeproc{ref-Kroonen2013}{Kroonen 2013}). Ringe and Taylor show that
the paradigm also contained a separate 3sg present stem, continued in
late Northumbrian \emph{lifed}, which they treat as an archaism
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).
...
\emph{*mánnas} \(\rightarrow\) \emph{mannes} (late analogy).
...
2013}; \citeproc{ref-Orel2003}{Orel 2003, 299};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). The selected
...
Campbell gives the paradigm \emph{mann, man / mannes / menn}
(\citeproc{ref-Campbell1959}{Campbell 1959}). Sievers-Brunner likewise
cites \emph{man mannes} and explains that word-final simplification
underlies forms such as \emph{man} beside inflected \emph{monnes}
(\citeproc{ref-SieversBrunner1965}{Sievers 1965}). Clark Hall keeps the
dictionary headword under \emph{mann}
(\citeproc{ref-ClarkHall1960}{Clark Hall 1960}).
...
\emph{*mízdai} \(\rightarrow\) \emph{meorde} (late analogy).
...
\emph{*náxti} \(\rightarrow\) \emph{niht} (late analogy).
...
Ringe and Taylor distinguish the high-vowel oblique and plural side of
the word from the nominative citation form: they cite \emph{gen.sg.
*nahtiz}, \emph{dat.sg. *nahti}, and \emph{nom.pl. *nahtiz}, and derive
West Saxon \emph{niht} from that side of the paradigm
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). The citation
reconstruction \emph{*náxtz} therefore belongs to the nominative-like
headword, while the selected input \emph{*náxti} represents the
...
\emph{dæg} by analogy with \emph{dat. sg. niht \textless{} *nahti}
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).
...
\emph{neahtas} (\citeproc{ref-Campbell1959}{Campbell 1959}).
...
From \emph{*náxti}, Anglo-Frisian brightening first fronts the root
vowel, and the following high vowel then triggers i-umlaut. In West
Saxon, the sequence before \emph{ht} yields \emph{niht}, whereas plural
forms with a following back vowel preserve the non-umlauted
\emph{neahtas} type (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor
2014}; \citeproc{ref-Campbell1959}{Campbell 1959};
\citeproc{ref-SieversBrunner1965}{Sievers 1965}).

The modeled path is therefore \emph{*náxti \textgreater{} *næxti
\textgreater{} *neaxti \textgreater{} *niexti \textgreater{} *nixti
\textgreater{} niht}.
...
\emph{*rástōz} \(\rightarrow\) \emph{ræste} (late analogy).
...
such as \emph{on ræste} and \emph{tó ræste}
...
\emph{*skúldramiz} \(\rightarrow\) \emph{sċuldrum} (late analogy).
...
Orel gives \textbf{\emph{*skuldr(j)ō}, a feminine ō-/jō-stem, and
explicitly notes that Old English \emph{sculdor} is masculine beside
OFrisian \emph{skulder}, Middle Low German \emph{schulder}, and Old High
German \emph{scultra, scultirra} (\citeproc{ref-Orel2003}{Orel 2003,
...
A dative/instrumental plural form \textbf{\emph{*skúldramiz}} aligns
...
(\citeproc{ref-BosworthToller1898}{Bosworth and Toller 1898};
\citeproc{ref-ClarkHall1960}{Clark Hall 1960}). The dative plural
...
Proto-Germanic \textbf{\emph{*skúldramiz} can be interpreted as a
...
development is \emph{*skúldramiz \textgreater{} *skúldrumiz
\textgreater{} *skúldrum \textgreater{} sċuldrum}.
...
\emph{*skáub} \(\rightarrow\) \emph{sċēaf} (late analogy).
...
\textasciitilde{} *skūban-} and cites Old English present forms
\emph{scēofan, scūfan} (\citeproc{ref-Kroonen2013}{Kroonen 2013}). Ringe
and Taylor also show that the English present system belongs to a wider
class-II split that is not identical with the preterite grade
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). The selected
input \emph{*skáub} is therefore not a spelling variant of the
infinitive but the singular preterite cell.

\paragraph{Old English evidence}\label{old-english-evidence-124}

The ordinary dictionary verb is \emph{scūfan/scēofan}, but the preterite
itself is well attested. Bright gives the principal parts \emph{scufan,
sceaf, scufon, scofen} and also quotes \emph{he sceaf þa mid þam
scylde}; Sweet gives the same paradigm
(\citeproc{ref-BrightCassidyRingler1971}{Bright 1971};
\citeproc{ref-Sweet1953}{Sweet 1953}). The normalized form here is
\emph{sċēaf}, regularizing the attested spellings \emph{sceaf} and
prefixed \emph{āsceaf}.
...
gives \emph{*skáub \textgreater{} *skáeub \textgreater{} *skēab
\textgreater{} *skēaβ \textgreater{} sċēaf}.
...
\emph{*spánnai} \(\rightarrow\) \emph{spanne} (late analogy).
...
\emph{*mákô} \(\rightarrow\) \emph{maca} (late analogy).
...
cites \emph{macian} beside \emph{lapian} and similar verbs with restored
\emph{a}, and Ringe and Taylor place the Germanic verb in the same
class, comparing West Germanic continuants such as Old Frisian
\emph{makia}, Old Saxon \emph{makon}, and Old High German \emph{mahhon}
(\citeproc{ref-Campbell1959}{Campbell 1959};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).

The selected input \emph{*mákô} is not the citation form of the lexeme
but a finite paradigm cell. Ringe and Taylor's account of the class-II
weak paradigm is the reason for choosing it: the imperative singular
belongs to the small set of finite cells that preserve the trimoric
\emph{*ō} directly, whereas the ordinary infinitive continues the
remodelled \emph{*-ōja-} formation behind \emph{macian}
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).

\paragraph{Old English evidence}\label{old-english-evidence-127}

The dictionary headword is \emph{macian}
(\citeproc{ref-ClarkHall1960}{Clark Hall 1960}). The selected form in
this entry is therefore not the lemma but the imperative singular
...
In this class-II environment A-restoration returns the stem vowel to
\emph{a}, and unstressed long vowel shortening then gives \emph{*maka},
whence \emph{maca} (\citeproc{ref-Campbell1959}{Campbell 1959};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).
...
Derivation: citation reconstruction \emph{*makōną}; selected input
\emph{*mákōθi} \(\rightarrow\) \emph{macaþ} (late analogy).
...
Kroonen derives the Old English verb from \textbf{\emph{*makōjan-} on
the make-family base \emph{*maka-} (\citeproc{ref-Kroonen2013}{Kroonen
2013}). Ringe and Taylor likewise place the verb among the class-II weak
verbs and note that the present 2sg, 3sg, and imperative singular
preserve suffixal \emph{*ō}} rather than the remodelled infinitival
formation (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).

The selected input \textbf{\emph{*mákōθi}} is therefore a finite 3sg
...
traditions (\citeproc{ref-Campbell1959}{Campbell 1959, sect. 355.4};
...
After early loss of final \emph{-i}, \textbf{\emph{*mákōθi\textbf{
yields }*mákōθ}. Anglo-Frisian brightening gives \emph{*mækōθ}, but
...
Campbell's \textbf{\emph{lufas, -aþ (\textless{} -ōsi, -ōþi)} and Ringe
and Taylor's discussion of stable \emph{a} in the finite non-\emph{j}
cells point to \emph{*makōθ \textgreater{} *makaθ \textgreater{} macaþ}}
(\citeproc{ref-Campbell1959}{Campbell 1959, sect. 355.4};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).
...
\emph{*búrô} \(\rightarrow\) \emph{bora} (late analogy).
...
Kroonen reconstructs the bore-family verb as \textbf{\emph{*burojan-}
and cites Old English \emph{borian} among its continuants
(\citeproc{ref-Kroonen2013}{Kroonen 2013}). In Ringe and Taylor's
account of class-II weak verbs, the imperative singular belongs to the
finite cells that preserve inherited suffixal \emph{*ō}}, unlike the
remodelled infinitive (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor
2014}).

The selected input \textbf{\emph{*búrô}} is therefore an imperative cell
...
(\citeproc{ref-ClarkHall1960}{Clark Hall 1960}). The comparison form
...
non-\emph{j} finite cell represented by \textbf{\emph{*búrô}}.

\paragraph{Development to Old
English}\label{development-to-old-english-129}

Northwest Germanic lowering first gives **\emph{*bórô\textbf{ from
}*búrô}, and late shortening of the unstressed long vowel then yields
\_*bóra\_\_, whence \_\_bora\_**.

Ringe and Taylor's class-II imperative singular \textbf{\emph{-a
\textless{} *-ō}} points to exactly this type of outcome
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). The selected
form therefore isolates the regular finite-cell development more cleanly
than the remodelled infinitive does.
...
Derivation: citation reconstruction \emph{*burōną}; selected input
\emph{*búrōθi} \(\rightarrow\) \emph{boraþ} (late analogy).
...
Kroonen reconstructs the bore-family verb as \textbf{\emph{*burojan-}
and cites Old English \emph{borian} among its reflexes
...
Campbell's class-II pattern \textbf{\emph{lufas, -aþ (\textless{} -ōsi,
-ōþi)} and Ringe and Taylor's account of stable \emph{a}} in the
class-II 2sg and 3sg make this finite cell the relevant comparison form
for the ending (\citeproc{ref-Campbell1959}{Campbell 1959, sect. 355.4};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).
...
Early loss of final \emph{-i} first gives **\emph{*búrōθ\textbf{ from
}*búrōθi}. Northwest Germanic lowering then produces \emph{*bórōθ}, and
late shortening of unstressed \_ō\textbf{ yields }*bóraθ\_\_, whence
\_\_boraþ\_**.

Campbell's class-II ending evidence and Ringe and Taylor's discussion of
stable \textbf{\emph{a}} in the finite non-\emph{j} cells support
exactly this sequence (\citeproc{ref-Campbell1959}{Campbell 1959, sect.
355.4}; \citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).
...
\emph{*líznô} \(\rightarrow\) \emph{liorna} (late analogy).
...
leornian} from a learn-family base of the \textbf{\emph{*lizn-} type,
and Kroonen likewise keeps the weak verb as \emph{*liznōn-}
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014};
\citeproc{ref-Kroonen2013}{Kroonen 2013, 380}). Fulk cites the same Old
English family from \emph{*liznō-}} (\citeproc{ref-Fulk2018}{Fulk
2018}).

The selected input \textbf{\emph{*líznô}} is a finite imperative cell of
...
The selected form develops regularly as \textbf{\emph{*líznô
\textgreater{} *lírnô\textbf{ by rhotacism, then }*líornô\textbf{ by
breaking before \textbf{rn}, and finally }*líorna}} by late shortening
of the unstressed long vowel.

Campbell's Northumbrian \textbf{\emph{io} evidence and Ringe and
Taylor's explicit statement that no form of \emph{liornian} stood in an
...
Derivation: citation reconstruction \emph{*liznōjaną}; selected input
\emph{*líznōθi} \(\rightarrow\) \emph{liornaþ} (late analogy).
...
For the ending, Campbell's \textbf{\emph{lufas, -aþ (\textless{} -ōsi,
-ōþi)} and Ringe and Taylor's discussion of stable \emph{a}} in the
...
tradition (\citeproc{ref-SieversBrunner1965}{Sievers 1965}). Campbell
likewise notes Northumbrian forms with \emph{io} beside \emph{leornian}
(\citeproc{ref-Campbell1959}{Campbell 1959, sect. 123} n.~2).
...
The selected form develops as \textbf{\emph{*líznōθi \textgreater{}
*lírnōθi\textbf{ by rhotacism, then }*lírnōθ} after early apocope of
final \emph{-i\textbf{, then }*líornōθ\textbf{ by breaking before
\textbf{rn}, and finally }*líornaθ \textgreater{} liornaþ}} by late
shortening of the unstressed long vowel.

Campbell's Northumbrian \textbf{\emph{io} evidence and Ringe and
Taylor's statement that no form of \emph{liornian} stood in an
i-umlauting environment support the stem, while Campbell's class-II
ending evidence supports the final \emph{-aþ}}
(\citeproc{ref-Campbell1959}{Campbell 1959, sect. 123} n.~2;
\citeproc{ref-Campbell1959}{Campbell 1959, sect. 355.4};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).
...
\emph{*líkkô} \(\rightarrow\) \emph{licca} (late analogy).
...
verbs, with PWGmc \emph{*li/ekkōn} continuing as Old English
...
The form treated here is not that remodeled infinitive but a finite cell
in bare trimoric \emph{*-ō}. Campbell's weak class-II discussion and
Ringe and Taylor's account of the paradigm both distinguish those finite
singular cells from the ordinary \emph{*-ōja-} citation formation
(\citeproc{ref-Campbell1959}{Campbell 1959};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).

\paragraph{Old English evidence}\label{old-english-evidence-133}

Bosworth-Toller lemmatizes the verb as \emph{liccian}, and Campbell and
Brunner likewise cite \emph{liccian} with preserved geminate \emph{cc}
(\citeproc{ref-BosworthToller1898}{Bosworth and Toller 1898};
\citeproc{ref-Campbell1959}{Campbell 1959};
\citeproc{ref-SieversBrunner1965}{Sievers 1965}). The Old English
evidence therefore establishes the verbal headword and its consonantal
frame securely.
...
brief. Trimoric \emph{*-ô} shortens late to \emph{-a} in this finite
class-II cell, so \emph{*líkkô} yields \emph{*líkka} and then
\emph{licca} (\citeproc{ref-Campbell1959}{Campbell 1959};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). The same stem
consonantism that appears in \emph{liccian} is preserved here, giving
\emph{cc} throughout the finite form.
...
Derivation: citation reconstruction \emph{*likkōną}; selected input
\emph{*líkkōθi} \(\rightarrow\) \emph{liccaþ} (late analogy).
...
the class-II present endings as \emph{lufas, -aþ (\textless{} -ōsi,
-ōþi)}, and Ringe and Taylor likewise treat the class-II 2sg and 3sg as
forms with stable \emph{-a-} (\citeproc{ref-Campbell1959}{Campbell
1959}; \citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).

\paragraph{Old English evidence}\label{old-english-evidence-134}

Bosworth-Toller lemmatizes the verb as \emph{liccian}, and the same
consonantal frame appears in Campbell and Brunner's grammatical
citations of \emph{liccian} (\citeproc{ref-BosworthToller1898}{Bosworth
and Toller 1898}; \citeproc{ref-Campbell1959}{Campbell 1959};
\citeproc{ref-SieversBrunner1965}{Sievers 1965}). The Old English
headword is therefore clear even though the entry here is not about the
citation form.
...
\emph{*líkkōθi} first loses final \emph{-i}, giving \emph{*líkkōθ}. Late
shortening of unstressed \emph{*ō} then yields \emph{*líkkaθ}, written
\emph{liccaþ} (\citeproc{ref-Campbell1959}{Campbell 1959};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). Because this
ending never contains \emph{-j-}, the form does not pass through an
i-umlauted \emph{-eþ} stage; the regular class-II outcome is \emph{-aþ}.
...
\emph{*skáwô} \(\rightarrow\) \emph{sċēawa} (late analogy).
...
(\citeproc{ref-Orel2003}{Orel 2003}). The selected input in this entry
is not that infinitive but the imperative singular \emph{*skáwô}, a
finite class-II cell that preserves bare \emph{*-ō} rather than the
remodeled \emph{*-ōja-} formation (\citeproc{ref-RingeTaylor2014}{Ringe
and Taylor 2014}).
...
(\citeproc{ref-BrightCassidyRingler1971}{Bright 1971}). The form treated
here is therefore an attested finite paradigm form, not a reconstructed
convenience form.
...
From \emph{*skáwô}, Old English \emph{*aw} before vocalic material
yields \emph{*skḗawô}, initial \emph{sk} palatalizes, and late
shortening of trimoric \emph{*-ô} gives \emph{*ʃḗawa}, written
\emph{sċēawa} (\citeproc{ref-Campbell1959}{Campbell 1959}). The result
...
Derivation: citation reconstruction \emph{*skawōną}; selected input
\emph{*skáwōθi} \(\rightarrow\) \emph{sċēawaþ} (late analogy).
...
\emph{skawōn}, and Old High German \emph{scouwōn}
(\citeproc{ref-Orel2003}{Orel 2003}). The selected input in this entry
is the present third singular \emph{*skáwōθi}, a finite class-II cell
rather than the remodeled infinitive
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).

Campbell states the class-II present endings as \emph{lufas, -aþ
(\textless{} -ōsi, -ōþi)}, and Ringe and Taylor likewise treat the
class-II 2sg and 3sg as forms with stable \emph{-a-}
(\citeproc{ref-Campbell1959}{Campbell 1959};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). The relevant
comparison is therefore the 3sg cell itself, not an i-umlauted
alternative.
...
1971}). The evidence thus establishes the \emph{scēaw- / -awað}
...
\emph{*skáwōθi} first loses final \emph{-i}, giving \emph{*skáwōθ}. Old
English \emph{*aw} before vocalic material yields \emph{*skḗawōθ},
initial \emph{sk} palatalizes, and late shortening of unstressed
\emph{*ō} gives \emph{*ʃḗawaθ}, written \emph{sċēawaþ}
(\citeproc{ref-Campbell1959}{Campbell 1959}). Campbell's chronology
matters here: late-shortened \emph{ō} yields \emph{a} too late for
Anglo-Frisian fronting, and the class-II 3sg ending therefore gives
\emph{-aþ} rather than \emph{-eþ} (\citeproc{ref-Campbell1959}{Campbell
1959}; \citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). Because
the ending never contains \emph{-j-}, no i-umlaut applies.
...
\emph{*knúbbô} \(\rightarrow\) \emph{cnobba} (reconstructed Old English
comparator).
...
the voiced branch behind later English \emph{knob}.
...
\emph{cnopp / cnoppa} type (\citeproc{ref-BosworthToller1898}{Bosworth
and Toller 1898}; \citeproc{ref-ClarkHall1960}{Clark Hall 1960}). Those
forms are genuine Old English evidence, but they belong to the voiceless
...
\bottomrule\noalign{}
\endlastfoot
\emph{*knúbbô -\textgreater{} cnobba} & reconstructed OE form;
trace-supported & selected target \\
\emph{cnopp / cnoppa} & attested OE branch & important control form, but
belongs to the voiceless branch \\
...
\emph{*ráukaz} \(\rightarrow\) \emph{rēac} (reconstructed Old English
comparator).
...
Derivation: \emph{*stráwjaną} \(\rightarrow\) \emph{strīeġan}
...
Kroonen cites the inherited weak verb as \textbf{\emph{*straujan-}} and
gives Old English \textbf{\emph{streowian}} as its dictionary
continuation (\citeproc{ref-Kroonen2013}{Kroonen 2013, 483}). Ringe and
Taylor make the split within Old English explicit: the inherited class-I
verb is continued by Anglian \textbf{\emph{strēgan}, while West Saxon
...
\textbf{\emph{*striegan}} directly from Proto-Germanic \emph{*straujana}
...
The target \emph{strīeġan} is therefore a reconstructed Old English
form, not an attested manuscript lemma. It is the reconstructed West
Saxon reflex of the inherited class-I verb, chosen to keep the inherited
branch distinct from the better-attested remodelled West Saxon lemma.

\paragraph{Development to Old
English}\label{development-to-old-english-139}

From \textbf{\emph{*stráwjaną}}, the inherited West Saxon development
passes through \emph{*straujaną}, fronting and leveling to a
\textbf{\emph{*strēajan-}} stage, i-umlaut to \textbf{\emph{*strīejan}},
and retention or strengthening of the glide after the front diphthong,
written here as \textbf{\emph{ġ}. The resulting form is
\emph{strīeġan}}.

This differs from Anglian \textbf{\emph{strēgan}}, where smoothing
removes the diphthongal sequence, and from West Saxon
\textbf{\emph{strewian / streowian / strēawian}}, where the verb has
already been remodelled into class II (\citeproc{ref-Fulk2018}{Fulk
...
trace-supported & selected target \\
\emph{strewian / streowian / strēawian} & attested remodelled West Saxon
class-II forms & genuine OE evidence, but not the inherited branch
modeled here \\
\end{longtable}
}
...
\textasciitilde{} *fun-} and explains the front-mutated West Germanic
forms from an oblique form of the \emph{*fu(w)eri} type
(\citeproc{ref-Kroonen2013}{Kroonen 2013}). The selected input
...
\bottomrule\noalign{}
\endlastfoot
lexeme-level heteroclitic headword & \emph{*fōr \textasciitilde{} *fun-}
& comparative background only & fire family & explains the wider lexeme,
but not the selected oblique input \\
...
Old English \emph{bucca} (\citeproc{ref-Kroonen2013}{Kroonen 2013};
...
\emph{rūst m.} (\citeproc{ref-ClarkHall1960}{Clark Hall 1960}), and
...
\bottomrule\noalign{}
\endlastfoot
\emph{*rústō -\textgreater{} rost} & computed regular output from one
competing citation reconstruction & shows that this citation
reconstruction does not reach attested \emph{rust} \\
\emph{*rustaz -\textgreater{} rost} & expected regular output from the
source-supported citation reconstruction & shows that correcting the
stem class does not solve the vowel problem \\
\emph{*rústu -\textgreater{} rust} & regular high-vowel comparator &
useful negative control, but not a defensible citation-form solution \\
\emph{rust} & attested Old English noun, normalized from \emph{rūst /
rúst / rust} & selected target; the citation-form development remains
unexplained \\
\end{longtable}
}
...
*wólfaz \textgreater{} *wólfa \textgreater{} wolf}.
...
\bottomrule\noalign{}
\endlastfoot
\emph{*wúlfaz -\textgreater{} wolf} & computed regular output from the
citation form & shows the regular development expected from the
inherited a-stem \\
\emph{OHG wolf} & comparative regular cognate & confirms that the
\emph{o}-vocalism is the ordinary outcome \\
\emph{*wúlfi / *wúlfis -\textgreater{} wylf / wylfe} & expected
high-vowel control forms & shows why oblique high-vowel cells do not
solve the noun's vowel history \\
...
English still has a lexical exception of the \emph{wull / wulle} type.
...
\bottomrule\noalign{}
\endlastfoot
\emph{*wúllō -\textgreater{} woll} & regular trace output & shows what
the deterministic sound laws produce \\
\emph{wull / wulle} & attested OE exception & selected target and
...
Adamczyk, Elżbieta. 2001. \emph{Reflexes of {Sievers'} Law in {Old
English}}.

\bibitem[\citeproctext]{ref-Bammesberger1997}
Bammesberger, Alfred. 1997. \emph{{Old English} \emph{h{æ}rfest} and Its
{Germanic} Cognates}.
...
\emph{{Germanic}, {Slavic}, and {Baltic} {`Thousand'}}. 2025.
...
Kroonen, Guus. 2011. {`The {Proto-Germanic} n-Stems: A Study in
Diachronic Morphophonology'}. In \emph{The {Proto-Germanic} n-Stems: A
Study in Diachronic Morphophonology}, vol. 18. Leiden Studies in
Indo-European. Rodopi.
...
Lloyd, Albert L. 1966. \emph{The {a-}Umlaut of *{i} in {Germanic}}.
