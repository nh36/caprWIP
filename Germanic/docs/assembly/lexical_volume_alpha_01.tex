...
\emph{*nadrōn} type (\citeproc{ref-Orel2003}{Orel 2003, 325}).
...
(\citeproc{ref-BosworthToller1898}{Bosworth and Toller 1898, 699}), so
\emph{sculdra} belongs to the Old English record beside the stronger
masculine paradigm headed by \emph{sculdor}. Brunner and Luick also
record later spellings such as \emph{sceoldor} and the i-mutated dative
plural \emph{scyldrum}, which reflect secondary phonological and
...
dat./inst.pl. input & *skúldramiz & regular output: \emph{sċuldrum} &
sculdrum & matches both the output and the dative plural comparison
form \\
...
\emph{-awað} finite-cell pattern directly.
...
but not the finite form compared here \\
imperative singular companion & \emph{*skáwô} & regular output
\emph{sċēawa} & scēawa & useful family control, but not the target of
this entry \\
present third singular & \emph{*skáwōθi} & regular output \emph{sċēawaþ}
& normalized sċēawaþ; source-side pattern \emph{-sceawað} & exact match
for the finite form compared here \\
\end{longtable}
}
...
\citeproc{ref-Orel2003}{Orel 2003, 338}). The form followed here,
...
\bottomrule\noalign{}
\endlastfoot
regular inherited noun path & *búkkaz & regular output: \emph{bocc} &
regular output, but not the target \\
