...
The older explanation of {\emph{bēġen}} derives it from
{\emph{*bō-jen-}}, and Orel still gives OE \emph{bezen} (\textless{}
\emph{*bō-jenō)} beside \emph{ON} {\emph{báðir}}, \emph{OFris}
{\emph{bēthe}}, \emph{OS} {\emph{be-thia}}, and \emph{OHG} {\emph{bēde}}
...
\emph{bū}{]}\{.iv lang=oe display=*bō \textgreater{} bū sort=bo
role=selected\_input\} treated here.
...
\textgreater{} \emph{bū}{]}\{.iv lang=oe display=*bō \textgreater{} bū
sort=bo role=selected\_input\}.
...
\bottomrule\noalign{}
\endlastfoot
{[}\emph{*bō} \textgreater{} \emph{bū}{]}\{.iv lang=oe display=*bō
\textgreater{} bū sort=bo role=selected\_input\} & PGmc neuter dual
...
Toller 1898, 164}; \citeproc{ref-ClarkHall1960}{Clark Hall 1960, 95}).
The form discussed here is therefore an attested Old English noun with
single \emph{l}.
...
the OE form \\
selected i-stem formation & *déliz & regular output: \emph{dile} & dile
...
Kroonen places the verb within the wider \emph{*fastu-} adjective family
and its derived \emph{*fasten-} verbal line, the comparative background
behind Old English `to fast' (\citeproc{ref-Kroonen2013}{Kroonen 2013,
171}). Ringe and Taylor, however, distinguish the Old English verb more
closely: they treat OE `to fast' as originally a class-I weak verb that
later acquired the stative meaning through lexical confusion
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, 110}).

The form followed here, \emph{*fástijaną}, therefore represents the
...
under the adjective \emph{fæst}, whereas \emph{festan} is the regular
inherited class-I comparison form.
...
selected class-I weak verb & *fástijaną & regular output: \emph{festan}
& festan & exact match between formation and attested OE verb \\
...
derivation \\
selected weak feminine formation & *fláskōn & regular output:
...
likewise has a separate \emph{fylgean} entry
...
selected class-I formation & *fúlgijaną & regular output: \emph{fylġan}
& fylġan / fylgan & exact match between input, output, and class \\
\end{longtable}
}
...
506}). The form followed here, \emph{*knéxtaz}, follows that comparative
evidence.
...
handbook-supported reconstruction & *knéxtaz & regular output:
...
background & cneohtas & supports the vowel environment, but not the Old
English form here cell \\
\end{longtable}
}
...
2014, 344}). The form followed here, \emph{*fúnðanǭ}, belongs to the
...
It therefore reaches \emph{fundene} in the form where the trace and the
attested evidence match directly.

\paragraph{Old English evidence}\label{old-english-evidence-113}

Bosworth-Toller records \emph{fundene} under \emph{findan}, citing the
form in \emph{Beón} \emph{þá} herigeata swa fundene
(\citeproc{ref-BosworthToller1898}{Bosworth and Toller 1898, 219}).
...
more familiar nominative \emph{funden} is the ordinary dictionary form,
...
establishes the lexeme, but not the form compared here \\
nominative participial line & *fúnðanaz & ordinary dictionary
\emph{funden} type & funden & important paradigm background, but not the
form compared here \\
oblique participle compared here & *fúnðanǭ & regular output:
...
2003, 120}). The form followed here, \emph{*fúrxtīnaz}, is not a
different lexeme but an oblique singular cell within that in-stem
paradigm.
...
also records nominative forms such as \emph{fyrhtu} and \emph{fyrhto}
(\citeproc{ref-BosworthToller1898}{Bosworth and Toller 1898, 160}).
Clark Hall separately preserves adjective and verb material under
\emph{fyrht} / \emph{fyrhtan}, which helps keep the noun distinct from
the adjectival family (\citeproc{ref-ClarkHall1960}{Clark Hall 1960,
141}).
...
evidence, but the Old English form here of this entry is the oblique
cell.
...
family context & useful lexeme label, but not the cell compared here \\
...
but later remodeled \\
selected oblique singular & *fúrxtīnaz & regular output: \emph{fyrhte} &
fyrhte & exact match between input, output, and attested cell \\
\end{longtable}
}
...
\citeproc{ref-ClarkHall1960}{Clark Hall 1960, 160}). The form followed
here, \emph{*xámaras}, is the genitive singular of that same noun rather
than a different lexeme.
...
but not the Old English form here \\
genitive singular & *xámaras & regular output: \emph{hameres} & hameres
...
The form followed here, \emph{*xábēθi}, is therefore the 3sg present
...
leveling \\
3sg present & *xábēθi & regular output: \emph{hæfeþ} & hæfeþ & exact
match between input, output, and finite form compared here \\
...
\emph{borian} & borian & family background, but not the cell compared
here \\
imperative singular & \emph{*búrô} & regular output \emph{bora} & bora &
exact match \\
...
\emph{borian} & borian & family background, but not the cell compared
here \\
3sg present & \emph{*búrōθi} & regular output \emph{boraþ} & boraþ &
exact match \\
