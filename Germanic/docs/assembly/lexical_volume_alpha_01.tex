...
(\emph{pwgmc}) stage, not Proto-Germanic; §D explains its labial.

The word is an inherited \textbf{mn-stem}
(\citeproc{ref-Kroonen2013}{Kroonen 2013, 220}): PIE \emph{h₂ék-mōn,
gen. }h₂k-mn-ós, remodelled in Germanic to nom. \Recon{hemō} `heaven',
gen. \Recon{hemnaz} `heaven'. In the zero-grade \textbf{oblique cluster}
cells the sequence \emph{-mn- is adjacent, and the change }mn
\textgreater{} βn/bn/fn* is regular there (Fulk §6.14
...
(\citeproc{ref-Fulk2018}{Fulk 2018})) \textbf{generalized the labial}
into the vowel-bearing stem. The \emph{b} of \emph{xébun is therefore
analogical }within this nominative input*, but its historical source is
regular and recoverable --- back to the inherited genitive (see §E).
...
{\emph{himins}} `heaven') --- is \textbf{not fully recoverable}. It is
the irreducible residue of the reconstruction; the \emph{b} is not.
...
The obliques go \textbf{deeper} than the selected nominative and derive
the labial themselves. Live outputs under the corrected literal
...
Distinctions kept explicit: \textbf{attested} OE forms (\emph{heofon},
\emph{heofones}, \emph{heofnes}, \emph{heofnum}) descend from the
re-vowelled stem; the cluster outputs
\emph{hefnes}/\emph{hifn}/\emph{hefnum} are \textbf{reconstructed
regular outputs}, and for Old English they are \textbf{counterfactual}
--- the actual attested obliques carry \emph{eo} from back mutation on
the retained/\hspace{0pt}restored medial vowel, then late syncope
...
The oblique lines are historically \textbf{essential} --- they show that
the labial has a regular origin, all the way back to the inherited
genitive \emph{h₂k-mn-ós --- but they are inferior as the single CAPR
input→target datum. In Old English every oblique cluster form encounters
morphological \textbf{re-vowelling} (restoration of a medial vowel into
the labialized \emph{-βn-} stem) before it reaches an attested Old
English shape. That re-vowelling is a non-regular event; it interrupts
any single deterministic path from a cluster oblique to an attested OE
form. The nominative input }xébun begins \textbf{after} that
restructuring and then proceeds by regular sound change to a secure
attested target --- the longest clean modellable span.

\paragraph{G. Attestation warning}\label{g.-attestation-warning}

Bare Old English \emph{hefn} is \textbf{not} securely attested and must
not be reintroduced as an attested comparator. The earliest witnesses
(Cædmon's Hymn) are {\emph{hefun}} `heaven', {\emph{hefen}} `heaven',
and (Moore Bede) {\emph{heben}} `heaven' --- all with a \textbf{medial
vowel}. The reconstructed cluster output \emph{hefn} (§E) is a
reconstruction, never a manuscript form.
...
\item
  ...
  input from the citation reconstruction \textbf{before} the
  specifically Old English changes apply).
\item
...
\textbf{regular and modelled}, with multiplicity 1. Live trace under the
...
the historical \textbf{adjacent} \emph{mn} \textgreater{} \emph{βn} (the
rule \emph{PNWGmcMnDissimilation} / SC022). The \emph{e} of \emph{stefn}
is regular from \emph{á} via brightening (\emph{á} \textgreater{}
\emph{æ}) plus i-umlaut triggered by the i-stem ending (\emph{æ}
\textgreater{} \emph{e}), so both the vowel and the consonant follow by
regular sound change once the i-stem input is selected.

Note the contrast with \emph{heaven}. Here SC022
(\emph{PNWGmcMnDissimilation}) \textbf{fires directly inside} the
selected derivation, deriving the labial of \emph{stefn} from the
adjacent \emph{-mn-} cluster of \emph{*stámniz}. In \emph{heaven}, by
contrast, the \textbf{same historical change \emph{mn} \textgreater{}
\emph{βn}/\emph{fn} operated only in the deeper cluster-bearing oblique
prehistory}; the resulting labial was then \textbf{generalized} into the
vowel-bearing \emph{*hebun-} stem, so the selected CAPR path
\emph{*xébun} \textgreater{} \emph{heofon} begins \emph{after} that
analogy and does \textbf{not} itself contain SC022 (see
...
a labial from an intervocalic \emph{m} --- has been \textbf{retired} in
favour of this literal adjacent \emph{mn} \textgreater{} \emph{βn}.
Selecting the i-stem input \emph{*stámniz} over the a-stem citation
\emph{*stámnaz} is a \textbf{pre-OE input selection}; once selected, the
Old English development is regular. Hence the classification
\emph{early\_analogy} (analogy separates the input from the citation
