...
\emph{*bō}, with Old English {\emph{bēġen}} `both', {\emph{bā}} `both',
and neuter {\emph{bū}} `both' (\citeproc{ref-Kroonen2013}{Kroonen 2013,
47}). For the present entry, the relevant inherited form is the
unextended neuter dual {\emph{*bō}} `both'.
...
{kalbaz} `calf' with plural {kalbizō} `calves', while Orel cites
{kalbaz} `calf' as the citation form and Ringe and Taylor derive West
Saxon \emph{Cealf} from \emph{*kalbaz}, \emph{*kalbiz-}
...
The derivational input here is the singular {kálbaz} `calf', since the
entry concerns the citation-form noun.
...
either inherited \emph{þ} \textasciitilde{} \emph{*d} alternation or the
...
\emph{*hurna-} / {xurnan} `horn' type for this noun
(\citeproc{ref-Kroonen2013}{Kroonen 2013}; \citeproc{ref-Orel2003}{Orel
...
dialect evidence shows that \emph{liornian} `learn' is a genuine Old
English form (\citeproc{ref-Campbell1959}{Campbell 1959, sect. 123}
n.~2).
...
and nominative {melukz} `milk', and the nominative-style input used here
is {mélukz} `milk' (\citeproc{ref-Kroonen2013}{Kroonen 2013};
...
\emph{*mōder-} with citation {mōdēr} `mother'
(\citeproc{ref-Kroonen2013}{Kroonen 2013}; \citeproc{ref-Orel2003}{Orel
2003}).
...
masculine pair {þurnuz} `thorn' and {þurnaz} `thorn'
(\citeproc{ref-Kroonen2013}{Kroonen 2013}; \citeproc{ref-Orel2003}{Orel
...
`town' and {tūnaz} `town' (\citeproc{ref-Kroonen2013}{Kroonen 2013};
\citeproc{ref-Orel2003}{Orel 2003, 452}). The derivational input {tūną}
`town' is the simple citation-form noun used in the derivation.
...
Kroonen separates noun {weljan} `will, wish' from verb {weljan} `to
want', while Orel and Kluge represent the noun as {weljōn} `will'
(\citeproc{ref-Kroonen2013}{Kroonen 2013}; \citeproc{ref-Orel2003}{Orel
...
Kroonen reconstructs the word as a stem complex {budmō} `bottom' with
genitive {buttaz} `bottom', summarized as \emph{*budman-}
\textasciitilde{} \emph{*buttman-}, and gives Old English \emph{botm} as
the reflex (\citeproc{ref-Kroonen2013}{Kroonen 2013, 120}). The
comparative label {búdmaz} `bottom' names the lexeme-level stem complex,
while the derivational input {búttmaz} `bottom' represents the
pre-Old-English form with oblique \emph{*butt-} generalized into the
nominative formation.

Orel likewise preserves both sides of the comparison under {budmaz}
`bottom' and {butmaz} `bottom' (\citeproc{ref-Orel2003}{Orel 2003,
100}). The derivational input is thus a historical stem choice, not an
...
u-stem \emph{*kraftu-}, while Orel prints {kraftiz} `craft' and
...
English form discussed here. Orel's {nasō} `nose' alongside OE
\emph{nasu} `nose' preserves the competing full-grade notation and shows
that the two lines should not be collapsed without comment
...
preserves the comparative notation {sapōn} `sap' and {sapan} `sap'
...
\subsubsection{\texorpdfstring{voice/sound --- OE \emph{stefn (legacy
row label:
stem)}}{voice/sound --- OE stefn (legacy row label: stem)}}\label{voicesound-oe-stefn-legacy-row-label-stem}

Derivation: form followed here \emph{*stébnō}; Old English form
...
The source tradition behind {\emph{stefn}} `voice, sound' is not the
same lexical item as the comparative label {stámnaz} `stem', which
belongs to a different semantic family. Ringe and Taylor cite {stebnō}
`voice, sound' for the noun continued by Gothic {\emph{stibna}} `voice,
sound' and Old English \emph{stebn} \textgreater{} \emph{stefn}
\textgreater{} \emph{stemn} (\citeproc{ref-RingeTaylor2014}{Ringe and
Taylor 2014, 330}). Orel likewise cites {stebnō} `voice, sound' and
{stemnō} `voice, sound', while Kroonen gives \emph{*stimnō-}; Fulk
...
The selected OE-facing transponent is therefore {{stébnō} `voice,
sound'}. The comparative label {stámnaz} `stem' remains background
metadata from the legacy row heading and should not control the lexical
gloss of the OE evidence line.

\paragraph{Old English evidence}\label{old-english-evidence-98}

Clark Hall records {\emph{stefn}} `voice, sound' and cross-refers
{\emph{stemn}} `voice, sound' to the same noun
(\citeproc{ref-ClarkHall1960}{Clark Hall 1960, 276}). Ringe and Taylor
give the chronology directly as \emph{stebn} \textgreater{} \emph{stefn}
\textgreater{} \emph{stemn} (\citeproc{ref-RingeTaylor2014}{Ringe and
Taylor 2014, 330}).

Bülbring and Luick treat {\emph{stemn}} `voice, sound' as a later West
Saxon development from older {\emph{stefn}} `voice, sound', produced by
\emph{fn} \textgreater{} \emph{mn} after the earlier period of nasal
...
From {stébnō} `voice, sound', raising of final long \emph{ō} gives
{stébnu} `voice, sound'. Regular fricativization of \emph{b} before
\emph{n} then yields {stéβnu} `voice, sound', and loss of the final high
vowel leaves {stéβn} `voice, sound', written \emph{stefn} `voice, sound'
in Old English. The later form \emph{stemn} `voice, sound' belongs to a
...
\bottomrule\noalign{}
\endlastfoot
{\emph{*stámnaz}} & comparative citation label for the stem/trunk family
& does not itself control the \emph{stefn} derivation discussed here &
legacy comparative background only \\
{\emph{*stébnō}} & voice/sound noun transponent & regular output:
{\emph{stefn}} `voice, sound' & Old English-facing input \\
{\emph{stemn}} `voice, sound' & later attested West Saxon doublet &
secondary form from \emph{stefn} by \emph{fn} \textgreater{} \emph{mn} &
real OE variant, not the conservative comparison anchor \\
\end{longtable}
}
...
material points to {watōr} `water' and {watenaz} `water'
(\citeproc{ref-Kroonen2013}{Kroonen 2013, 616}). Ringe and Taylor
likewise start from singular {wator} `water' before the Old English
branch (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, sect.
3.1.4}).
...
paradigm \emph{wæteres}, \emph{wætere}, \emph{wæter(u)}, \emph{wætera},
...
\emph{weeter} `water' and Mercian \emph{weter} `water'
...
The comparative sources are not uniform. Orel gives {{xwalaz} `whale'}
and notes some mixed {{xwaliz} `whale'} evidence
...
{\emph{hwæl}} in a closed monosyllable.

\paragraph{Development to Old
English}\label{development-to-old-english-104}

From {xwálaz} `whale', final \emph{-z} disappears and bare final
...
loss of the final high vowel leaves \emph{cȳ} `cow'. The development is
...
OFrisian {\emph{skulder}} `shoulder', Middle Low German \emph{schulder},
and Old High German {\emph{scultra}} `shoulder', {\emph{scultirra}}
`shoulder' (\citeproc{ref-Orel2003}{Orel 2003, 345}). Kroonen
reconstructs {\emph{*skuldra-}}, a masculine a-stem, and derives the Old
High German feminine forms from {\emph{*skuldrjōn-}}
...
`make' from PWGmc {makon} `make' through {mekojan} `make'
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, 191}).

The derivational input {{mákōθi} `makes'} is therefore a finite 3sg cell
...
After early loss of final \emph{-i}, \emph{*mákōθi} yields {mákōθ}
`makes'. Anglo-Frisian brightening gives {mækōθ} `makes', but Campbell
lists \emph{macian} among class-II verbs with restored \emph{a}, so the
stem returns to \emph{mak-} before the ending is reduced
(\citeproc{ref-Campbell1959}{Campbell 1959, sect. 159}).

The ending then follows ordinary class-II 3sg development: \emph{*makōθ}
\textgreater{} \emph{*makaθ} \textgreater{} \emph{macaþ}. Campbell's
lufas, \emph{-aþ} (\textless{} \emph{-ōsi}, \emph{-ōþi)} and Ringe and
Taylor's discussion of stable class-II \emph{a} in finite non-\emph{j}
cells support this sequence (\citeproc{ref-Campbell1959}{Campbell 1959,
...
The derivational input {búrô} `bore (iptv.2sg)' is therefore an
...
non-\emph{j} finite cell represented by {búrô} `bore (iptv.2sg)'.

\paragraph{Development to Old
English}\label{development-to-old-english-129}

Northwest Germanic lowering first gives {bórô} `bore (iptv.2sg)' from
{búrô} `bore (iptv.2sg)', and late shortening of the unstressed long
vowel then yields {bóra} `bore (iptv.2sg)', whence \emph{bora} `bore'.
...
isolates the finite 3sg cell {búrōθi} `bores' rather than the
infinitive.
...
Clark Hall lemmatizes the verb as \emph{borian}; the imperative singular
\emph{bora} and present-third-singular \emph{boraþ} are the relevant
comparison forms (\citeproc{ref-ClarkHall1960}{Clark Hall 1960, 48}).
...
From {búrōθi} `bores', loss of final \emph{-i} first gives {búrōθ}
`bores'. Northwest Germanic lowering then produces {bórōθ} `bores', and
late shortening of unstressed \emph{ō} yields {bóraθ} `bores'.
...
The derivational input {{líznô} `learn (iptv.2sg)'} is a finite
...
\textgreater{} \emph{*lírnô} by rhotacism, then {líornô} `learn
(iptv.2sg)' by breaking before \emph{rn}, and finally \emph{*líorna}
\textgreater{} \emph{liorna} by late shortening of the unstressed long
vowel.
...
{{líznōθi} `learns'} is the finite 3sg cell of that family, not the
...
and {\emph{liornes}}, beside the West-Saxon-oriented {\emph{leornian}}
tradition (\citeproc{ref-SieversBrunner1965}{Brunner 1965, sect. 417}
...
\emph{*lírnōθi} by rhotacism, then {lírnōθ} `learns' after early apocope
of final \emph{-i}, then {líornōθ} `learns' by breaking before
\emph{rn}, and finally \emph{*líornaθ} \textgreater{} \emph{liornaþ} by
...
the fuller weak-verb reconstruction {{likkōjanan} `lick'} with the same
Old English continuation (\citeproc{ref-Orel2003}{Orel 2003, 285}).
...
singular {líkkōθi} `licks', not the remodeled infinitive. Campbell
...
{{líkkōθi} `licks'} first loses final \emph{-i}, giving {líkkōθ}
`licks'. Campbell's class-II present endings lufas, \emph{-aþ}
...
and {kauwjan} `chew', yielding Anglian \emph{strēzan} beside West Saxon
...
{striegan} `strew' directly from Proto-Germanic {straujana} `strew'
...
The target {{strīeġan} `strew'} is therefore a reconstructed Old English
form, not an attested manuscript lemma. It represents the inferred West
Saxon reflex of the inherited class-I branch.

\paragraph{Development to Old
English}\label{development-to-old-english-139}

From {stráwjaną} `strew', the inherited West Saxon line passes through
{straujaną} `strew', then a fronted \emph{*strēajan-} stage, then
{strīejan} `strew' by i-umlaut, with retained/strengthened glide after
the front diphthong to yield reconstructed \emph{*strīeġan}.
...
reconstruction uniformly. Orel cites {rustaz} `rust' (sb.m./f.) with Old
...
Sievers-Brunner notes that oblique \emph{wulfe} continues \emph{wulfi}
or older \emph{wulfai} (\citeproc{ref-SieversBrunner1965}{Brunner 1965,
sect. 160}).
