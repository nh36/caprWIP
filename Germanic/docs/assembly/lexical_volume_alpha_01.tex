...
in \emph{*dḗdiz} marks stress on the same reconstructed long vowel.
...
front vocalism then gives \emph{sċilling}. The \emph{i} of derivational
\emph{-ing-} remains, so the regular outcome is \emph{sċilling}, not
\emph{*sċilleng} (\citeproc{ref-Campbell1959}{Campbell 1959};
...
yields \emph{ēaw}, and \emph{*ō} survives between \emph{*w} and
\emph{*j} in the Class II suffix. The development therefore runs
regularly to {\emph{sċēawian}}, without the direct \emph{*aw+j} problem
...
here (\citeproc{ref-SieversBrunner1965}{Brunner 1965};
...
has \emph{þry}, \emph{þri} for {\emph{þrīe}}
...
From {\emph{*wábsaz}}, the regular Old English path passes through loss
...
{\emph{*fúlgijaną}} belongs to the class-I \emph{*fulgija-} /
...
{\emph{*gállą}}, but the Old English noun itself belongs with a weak
noun \emph{*gallōn-}, cited here as {\emph{*gállô}}
...
identity: the derivational input {\emph{*nábulô}} is the pre-syncope
...
A competing comparative label {\emph{*xnákkaz}} belongs to the wider
...
329}). The form followed here, {\emph{*nḗðlō}}, is the voiced
Verner-grade form used for the Old English comparison, while the
citation form {\emph{*nḗθlō}} remains the broader lexeme label.
...
The derivational input {\emph{*sápą}} therefore does not replace those
...
the English history: {\emph{*sái}} names the lexeme, but
{\emph{*sáiwiz}} preserves the medial \emph{*w} and the final high vowel
...
background \emph{*sib-iz} explains the morphology, but the derivational
input {\emph{*síbi}} is the immediate pre-Old-English form.
...
{\emph{*stábiz}} \textasciitilde{} {\emph{*stábaz}}
(\citeproc{ref-Orel2003}{Orel 2003, 368}). A direct i-stem input in
\emph{*-iz} would predict i-mutation in Old English, whereas the
...
comparative label {\emph{*stámnaz}}. Ringe and Taylor cite
\emph{*stebnō} for the noun continued by Gothic {\emph{stibna}} and Old
English \emph{stebn} \textgreater{} \emph{stefn} \textgreater{}
\emph{stemn} (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014,
330}). Orel likewise gives \emph{*stebnō} \textasciitilde{}
\emph{*stemnō}, whereas Kroonen prefers \emph{*stimnō-}, and Fulk
describes the etymology of \emph{stefn, stemn} as insecure
...
sound'. The derivational input {\emph{*stébnō}} is therefore best
treated as the OE-facing transponent supported by that source tradition.
It does not settle the deeper comparative reconstruction implied by the
citation label {\emph{*stámnaz}}.
...
{\emph{*swánaz}} (\citeproc{ref-Orel2003}{Orel 2003, 367}). The form
followed here, \emph{*swánas}, is not a competing lexeme reconstruction.
It is the genitive singular {\emph{*swánas}} of the same paradigm.

The question here is therefore one of paradigm cell rather than stem
history. The citation form remains {\emph{*swánaz}} \textgreater{}
{\emph{swan}}; the comparison form is the genitive singular
...
{\emph{*θūs-èndi}} is not the same claim. It is an OE-oriented
...
With strong {\emph{*wákaną}}, Anglo-Frisian brightening first gives a
form of the \emph{*wækaną} type. A-restoration then returns \emph{a},
and the ordinary tail reductions yield {\emph{wacan}}. The weak verb
{\emph{wacian}} belongs to a different prehistory and is not the
expected outcome of this input.
...
lexeme with \emph{a} in an open syllable, beside singular {\emph{hwæl}}
in the closed monosyllable.
...
Orel likewise distinguishes strong {\emph{*xwinanan}} from weak
{\emph{*wainōjanan}} (\citeproc{ref-Orel2003}{Orel 2003, 201}). Ringe
...
derivational input {\emph{*xwī́naną}} therefore represents a competing
...
Once the strong-verb input {\emph{*xwī́naną}} is selected, the path to
...
376}). The derivational input {\emph{*wī́θagą}} is thus a formation
...
From {\emph{*wī́θagą}}, Anglo-Frisian brightening gives a fronted vowel
...
Taylor discuss the lowered form \emph{*weraldiz} and its pre-Old-English
chain \emph{*weraldu} \textgreater{} \emph{*weruld}
...
The derivational input {\emph{*wír-àldu}} therefore combines the older
...
316}). The comparative label {\emph{*júgunθiz}} already stands at a
later Germanic stage with \emph{g}, and the derivational input
{\emph{*júgunθ}} is later again: it represents the form after final
\emph{-i} has been lost.
...
From the derivational input {\emph{*júgunθ}}, the later development is
regular: palatal fronting yields \emph{*jéugunθ}; nasal-spirant
lengthening and loss give \emph{*jéogūθ} (\citeproc{ref-Fulk2018}{Fulk
2018, 109}); unstressed long-vowel shortening then produces
\emph{*jéoguθ}, which surfaces as {\emph{ġeoguþ}}. Campbell preserves
\emph{u} after accented \emph{u} in forms such as {\emph{duguþ}} and
...
and Taylor give northern WGmc {\emph{*hebun}} \textgreater{} West Saxon
and Northumbrian {\emph{heofon}}, Mercian {\emph{heofen}}
...
The derivational input {\emph{*líbēθi}} therefore represents a finite
...
{\emph{*mánnas}} belongs to a different level: it is the
...
input {\emph{*mízdai}} is a dative-singular cell rather than the
citation form. The Old English evidence for the \emph{meord} side is
oblique.
...
445}). The derivational input {\emph{*rástōz}} therefore does not
...
but the comparison here uses the singular preterite {\emph{*skáub}}, not
the infinitive.
...
{\emph{*spánnai}} therefore supplies the conservative comparison form:
it preserves the medial geminate and yields {\emph{spanne}}, while
citation {\emph{*spannō}} gives the nominative background form.
...
{\emph{*θéstilaz}} therefore remains in view as the lexeme-level
headword, while the derivational input {\emph{*θístilas}} is a specific
...
The derivational input {\emph{*mákô}} is not the citation form of the
...
The derivational input {\emph{*mákōθi}} is therefore a finite 3sg cell
...
The derivational input {\emph{*líznô}} is a finite imperative cell of
...
{\emph{*líznōθi}} is the finite 3sg cell of that family, not the
...
and the 3sg {\emph{liornes}}, beside the West-Saxon-looking
...
finite stem \emph{liorn-}; the exact \emph{-aþ} ending follows the
regular class-II 3sg pattern.
...
50}). Orel gives the fuller weak-verb reconstruction
{\emph{*likkōjanan}} with the same Old English continuation
...
The target \emph{*cnobba} is a \textbf{reconstructed Old English form},
not a directly attested one. I use it for the voiced branch because
attested \emph{cnoppa} continues the voiceless branch and therefore
represents a different prehistory.
...
Luick groups \emph{*strauwjan} with the same set as \emph{*hauwja-} and
...
English form}, not an attested manuscript lemma. It is the inferred West
Saxon reflex of the inherited class-I verb; the attested West Saxon
lemma belongs to the remodelled class-II formation.
...
{\emph{*fūri}} therefore does not function as an arbitrary substitute
...
The derivational input {\emph{*táppô}} is therefore the inherited noun
...
From {\emph{*táppô}}, the regular inherited noun path gives
...
with Orel's \emph{*rustaz} than with the competing \emph{*rústō}
