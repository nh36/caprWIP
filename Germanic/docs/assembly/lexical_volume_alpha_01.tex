...
cross-references {\emph{næddre}} to {\emph{nædre}}, and Fulk treats
{\emph{næddre}} as the later geminated form beside the older base
...
\emph{be-}, giving {\emph{beġinnan}} `begin'.
...
{\emph{bar}} `bier'. The normalized target {\emph{bǣr}} represents the
same long vowel (\citeproc{ref-ClarkHall1960}{Clark Hall 1960};
\citeproc{ref-BosworthToller1898}{Bosworth and Toller 1898, 73};
...
125}). The prefixed form \emph{gebyrd} `birth' is also well established
in the tradition: Kroonen lists \emph{(ge-)byrd}, Bosworth-Toller has a
...
\emph{*bō-jenō)} beside ON {\emph{báðir}} `both', OFris {\emph{bēthe}}
`both', OS {\emph{be-thia}} `both', and OHG {\emph{bēde}} `both'
...
{\emph{beide}} `both'; it does not displace the inherited neuter
{\emph{*bō}} `both' \textgreater{} {\emph{bū}} `both' treated here.
...
Saxon \emph{biegan} `bow'; Campbell lists \emph{biegan} `bow' among the
...
(\citeproc{ref-Kroonen2013}{Kroonen 2013}). Ringe and Taylor give the
plural development directly as PNWGmc {brōkiz} `breeches' \textgreater{}
{breeci} `breeches' \textgreater{} OE \emph{bréc} `breeches'
...
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}). The development
is therefore regular: {brōkiz} `breeches' \textgreater{} \emph{brēċ}
`breeches'.
...
Saxon lowering of stressed long \emph{ē} yield \emph{dǣd} `deed';
...
and Taylor 2014}). The derivational input {fléuganą} `fly' represents
that inherited strong verb in the notation used here.
...
From {léusaną} `lose', Old English diphthong leveling gives {lēosaną}
`lose', and later nasal apocope and weak-tail reduction yield
\emph{lēosan} `lose' (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor
2014}). The prefixed forms follow the same verbal base with added
\emph{for-}.
...
produces \emph{ie}, so the regular outcome is \emph{ġiest} `guest'
(\citeproc{ref-Campbell1959}{Campbell 1959};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014}).
...
{liornian} `learn' `learn' is the Northumbrian member of the Old English
...
\emph{hlid} `lid', and Lloyd includes OE \emph{hlid} `lid' beside ON
\emph{hliþó} `lid' and OHG \emph{(h)lit} `lid' among forms that retain
...
background only. The form represented here is the later {xlídą} `lid'
\textgreater{} \emph{hlid} `lid' line that matches the attested Old
English noun (\citeproc{ref-Orel2003}{Orel 2003};
\citeproc{ref-Lloyd1966}{Lloyd 1966}).
...
such as \emph{liehtan} `illuminate', \emph{lihtan} `illuminate', and
\emph{līhtan} `illuminate', distinct from the related noun \emph{lēoht}
`light' and adjective \emph{leoht} / \emph{liht} `light'
(\citeproc{ref-ClarkHall1960}{Clark Hall 1960};
\citeproc{ref-BosworthToller1898}{Bosworth and Toller 1898}).
...
passes through a West Saxon \emph{liehtan} `illuminate' stage, and is
represented here by normalized \emph{līehtan} `illuminate'. The word
treated in this entry is therefore the verb `to light, illuminate', not
the related noun from {leuxtą} `light' (\citeproc{ref-Fulk2018}{Fulk
...
\textless{} {mélukz} `milk', Campbell likewise discusses \emph{meoluc}
`milk' and \emph{meoloc} `milk', and Anglian shows \emph{milc} `milk'
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014};
\citeproc{ref-Campbell1959}{Campbell 1959}).
...
\emph{sċilling} `shilling', not {sċilleng} `shilling'
(\citeproc{ref-Campbell1959}{Campbell 1959};
...
From {{skáwōjaną} `show'}, the inherited \emph{aw} sequence before a
...
without the direct {aw+j} `show' problem seen in other verb types
(\citeproc{ref-Campbell1959}{Campbell 1959};
\citeproc{ref-Orel2003}{Orel 2003}).
...
(\emph{slāpan}), \emph{slēp} `slept', \emph{slēpon} `slept',
\emph{slēpen} `slept' (\citeproc{ref-ClarkHall1960}{Clark Hall 1960};
...
under \emph{ge-stillan} `still, stop' and related forms
...
Orel distinguishes three related formations: adverbial {sunþraz}
`asunder' \textgreater{} \emph{sundor} `asunder', Class I verbal
{sunþrjanan} `sunder' \textgreater{} \emph{syndrian} `sunder', and Class
II verbal {sunþrōjanan} `sunder' \textgreater{} \emph{sundrian} `sunder'
(\citeproc{ref-Orel2003}{Orel 2003}).
...
early chain {garna} `yarn' \textgreater{} {geern} `yarn' \textgreater{}
{gearn} `yarn' \textgreater{} OE \emph{gearn} `yarn'
(\citeproc{ref-Kroonen2013}{Kroonen 2013};
\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, 220}). The
...
uninflected form without umlaut is reflected in \emph{tēon} `ten'
...
West Saxon \emph{tien} `ten', \emph{tȳn} `ten' belong to a different,
...
late West Saxon \emph{wasp} from earlier \emph{waefs} / \emph{wæfs}
through restricted metatheses (\citeproc{ref-Bulbring1902}{Bülbring
1902, sect. 484} Anm. 3; \citeproc{ref-SieversBrunner1965}{Brunner 1965,
sects 193, 204}).
...
{kráftaz} `craft' yields \emph{cræft} `craft' and is therefore the form
...
\emph{a} in exactly this environment, including {\emph{flasce}} `flask'
after inflected {\emph{flascan}} `flask'
...
Ringe and Taylor distinguish PNWGmc {fulgija-} `follow'
\textasciitilde{} {fulgai-} `follow' \textgreater{} OE {\emph{fylgan}}
`follow' \textasciitilde{} {\emph{folgian}} `follow' and describe it as
a dual formation that probably reflects an older alternation between
j-present and e-stative (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor
2014, 293--94}). This is a stem-class choice, not a spelling choice. The
derivational input {{fúlgijaną} `follow'} belongs to the class-I
\emph{*fulgija-} / \emph{*fulgjan-} branch; the citation form {{fulgēną}
`follow'} belongs to the parallel class-II history behind
{\emph{folgian}} `follow'.
...
\emph{*gallōn-} `gall', cited here as {{gállô} `gall'}
...
`laugh', and \emph{hlihhan} `laugh' to \emph{hliehhan}
...
give the derivational pathway {nabulō} `navel' \textgreater{} {næbula}
`navel' \textgreater{} \emph{nafola} `navel'
...
Ringe and Taylor give the pre-OE line {nabulō} `navel' \textgreater{}
{næbula} `navel' \textgreater{} OE \emph{nafola} `navel'
...
{nēþlō} `needle' \textasciitilde{} {nēdlō} `needle' \textgreater{} OE
\emph{nédl} `needle' (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor
2014, 329}). The form followed here, {{nḗðlō} `needle'}, is the voiced
...
\textasciitilde{} {nēdlō} `needle' \textgreater{} OE \emph{nédl}
...
\textless{} {nusō} `nose' (\citeproc{ref-Campbell1959}{Campbell 1959,
44}).
...
\emph{nosa}, and cross-refers \emph{nasu} to \emph{nosu} `nose'
...
current trace: {núsō} `nose' \textgreater{} {nósō} `nose' \textgreater{}
{nósu} `nose' \textgreater{} \emph{nosu} `nose'. The early special step
lies in the choice of the zero-grade input, not in any late Old English
repair.
...
preserves the word family as \emph{*saf}/ppan- `sap', with Old English
\emph{sæp} `sap' (\citeproc{ref-Kroonen2013}{Kroonen 2013, 420}). Orel
...
the fuller form {{saiwiz} `sea'} and derive it through {sawi} `sea'
\textgreater{} {sei} `sea' \textgreater{} OE \emph{sǣ} `sea'
...
treats \emph{sea} as continuing the same {saiui} `sea' \textgreater{}
{sǣi} `sea' history, with loss of \emph{u}/\emph{w} before \emph{i}
...
After Proto-West-Germanic monophthongization {sáiwiz} `sea'
\textgreater{} {sāwiz} `sea' and final \emph{*-z} loss {sāwiz} `sea'
\textgreater{} {sāwi} `sea', the non-initial \emph{*w} disappears before
unstressed \emph{*i}, and the following high vowel fronts the root vowel
before final apocope. The documented chain is {sáiwiz} `sea'
\textgreater{} {sāwiz} `sea' \textgreater{} {sāwi} `sea' \textgreater{}
{sāi} `sea' \textgreater{} {sǣi} `sea' \textgreater{} \emph{sǣ} `sea'
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, sect. 6.7.1};
\citeproc{ref-Campbell1959}{Campbell 1959, sect. 406}).
...
From {{síbi} `sieve'}, the regular derivation gives {síβi} `sieve'
\textgreater{} {síβe} `sieve' \textgreater{} {\emph{sife}} `sieve'.
Medial \emph{b} is realized as a spirant and later written \emph{f},
while the final unstressed \emph{i} lowers to \emph{e}. The older s-stem
...
is {stábaz} `staff' \textgreater{} {stába} `staff' \textgreater{} {stáb}
`staff' \textgreater{} \emph{stæb} `staff' \textgreater{} \emph{stæf}
`staff'. A direct continuation of {stábiz} `staff', by contrast, would
produce i-mutated \emph{stefe} rather than the attested singular.
...
sound'. The derivational input \textbf{{{stébnō} `stem' `stem'}} is
therefore best treated as the OE-facing transponent supported by that
source tradition. It does not settle the deeper comparative
reconstruction implied by the citation label \textbf{{{stámnaz}
`stem'}}.
...
{\emph{stefn}} `stem', not the later West Saxon doublet {\emph{stemn}}
`stem'.

\paragraph{Development to Old
English}\label{development-to-old-english-98}

From \textbf{{stébnō} `stem' `stem'}, raising of final long \emph{ō}
gives a \textbf{{stébnu} `stem' `stem'} stage. Regular fricativization
of \emph{b} before \emph{n} then yields \textbf{{stéβnu} `stem' `stem'},
and loss of the final high vowel leaves \textbf{{stéβn} `stem' `stem'},
written \textbf{stefn{* in Old English. The later form \_**}\_
`stem'stemn} belongs to a separate West Saxon assimilation after this
stage (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, 330};
...
{\emph{stefn}} `stem' & Old English-facing input \\
{\emph{stemn}} `stem' & later attested West Saxon doublet & secondary
form from \emph{stefn} by \emph{fn} \textgreater{} \emph{mn} & real OE
variant, but not the selected comparator \\
\end{longtable}
}
...
vowel was lost or neutralized before the ordinary OE umlaut outcome
could develop.
...
prove a unique \emph{ærende} `message'-type analogy
...
`wake'wacan}}. The weak verb {\emph{wacian}} `wake' belongs to a
...
merger of unstressed \emph{æ}/\emph{e} yields {wæter} `water'.
...
Bosworth-Toller preserves the plural {\emph{hwalas}} `whales'
...
The derivational input {{xwī́naną} `whine'} therefore represents a
...
heavy ja-stem \emph{-e} / zero outcome & {\emph{wīþiġ}} `withy' & does
not account cleanly for the OE suffix \\
...
word as {\emph{*ju(w)unþi-}}. The comparative label {{júgunθiz} `youth'}
already stands at a later Germanic stage with \emph{g}, and the
derivational input {{júgunθ} `youth'} is later again: it represents the
...
genitive against nominative {\emph{ban}}, the ordinary nominative form
of the same noun, rather than against a directly cited genitive
headword.
...
{\emph{bēag}} `bowed'. That is the paradigm cell in which the inherited
...
nom.-acc.pl. \emph{cȳ} `cow', and dat.pl. \emph{cūm} `cow'
...
\emph{cā}, \emph{cȳ} `cow', \emph{cū(e)}, and \emph{cūs} `cow' all
appearing in the local source record.

\paragraph{Development to Old
English}\label{development-to-old-english-112}

{\emph{kūi}} `cow' is the dative singular of the oblique \emph{*kū-}
...
development is \emph{*kūi} \textgreater{} \emph{*kȳi} \textgreater{}
...
path to attested \emph{cȳ} `cow', while the broader Old English paradigm
shows how far the oblique \emph{*kū-} grade spread beyond that one cell.
...
{\emph{fundene}} in the form where the trace and the attested evidence
match directly.

\paragraph{Old English evidence}\label{old-english-evidence-113}

Bosworth-Toller records {\emph{fundene}} `found' under {\emph{findan}}
...
{\emph{funden}} `found' and \emph{tō-fundennes} `finding'
...
oblique {\emph{fundene}} `found'. It is an Old English form in its own
...
genitive singular {\emph{hameres}}.

\paragraph{Development to Old
English}\label{development-to-old-english-115}

From {{xámaras} `hammer'}, Anglo-Frisian brightening and the subsequent
merger of unstressed \emph{æ} with \emph{e} yield {\emph{hameres}}
`hammer'. The derivation of that oblique form is straightforward once
the genitive singular cell is selected.
...
From {{xábēθi} `have'}, the finite form yields {\emph{hæfeþ}} `has'
...
{\emph{hefzen}} show the later West-Saxon back-mutation and suffix
reshaping behind {\emph{heofon}} (\citeproc{ref-Campbell1959}{Campbell
1959, sect. 210.1}; \citeproc{ref-Campbell1959}{1959, sect. 381}).
...
derive West Saxon {\emph{niht}} `night' from that side
...
`night'} represents the dative-singular cell.
...
such as {\emph{neaht}}, {\emph{neht}}, and {\emph{nieht}}
...
`night', giving genitive \emph{nihte, nihtes}, dative \emph{niht,
nihte}, nominative plural \emph{niht} `night', and the contrasting
plural-side forms represented by {\emph{neahtas}}
...
`night' `night' via {nehti} `night' and {neahti} `night'
...
{\emph{ræste}} `rest', including prepositional examples such as on
\emph{ræste} and \emph{tó} \emph{ræste}
...
\emph{schulder}, and Old High German {\emph{scultra}} `shoulder',
...
\textbf{PWGmc {{skuldru} `shoulder' `shoulder'}} for the Old English
branch (\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, 142}).
...
Proto-Germanic \textbf{{skúldramiz} `shoulder' `shoulder'} can be
interpreted as a dative/instrumental plural form. In this environment
the post-tonic \emph{a} before \emph{m} is raised to \emph{u}, giving a
form of the \textbf{{skúldrumiz} `shoulder' `shoulder'} type. Unstressed
\emph{u} is regularly preserved before \emph{m}, especially in the
dative plural ending \emph{-um}: Campbell states this explicitly, and
Hogg formulates the same condition for the dative plural inflexion
...
{\emph{sċuldrum}} `shoulder' & {\emph{sculdrum}} `shoulder' & matches
...
and \emph{sċēaf} `shove' belong to different cells of the same strong
...
background, but not the comparison used for \emph{sċēaf} \\
1/3 sg. preterite & {\emph{*skáub}} & documented regular output:
{\emph{sċēaf}} `shove' & {\emph{sċēaf}} `shove' & direct match for the
singular preterite \\
...
`thistle' / {\emph{ðistel}}. Clark Hall gives {\emph{ðistel}} as the
noun headword (\citeproc{ref-ClarkHall1960}{Clark Hall 1960, 326}). The
Old English form here here is the genitive singular {\emph{þistles}}
`thistle', which preserves the same stem in an oblique form where the
cluster is medial.
...
{\emph{macaþ}} `makes', set beside the dictionary headword and the
related imperative singular {\emph{maca}}.
...
{\emph{boraþ}} `bore' & {\emph{boraþ}} `bore' & useful control, but not
the target \\
\end{longtable}
}
...
singular \emph{bora} `bore' and present-third-singular \emph{boraþ}
`bores' are the relevant comparison forms
...
The derivational input \textbf{{{líznô} `learn' `learn (iptv.2sg)'}} is
a finite imperative cell of that family, not the citation form of the
verb.
...
`learn (iptv.2sg)'} by breaking before \textbf{rn{*, and finally \_**}\_
`learn \emph{(iptv.2sg)'*líorna}} by late shortening of the unstressed
long vowel.
...
\textbf{{{líznōθi} `learns' `learn (3sg)'}} is the finite 3sg cell of
...
West-Saxon-looking \textbf{{\emph{leornian}} `learn'} tradition
(\citeproc{ref-SieversBrunner1965}{Brunner 1965, sect. 417} Anm. 10).
...
\emph{*lírnōθi} by rhotacism, then \textbf{{lírnōθ} `learns' `learn
(3sg)'} after early apocope of final \textbf{-i{*, then \_**}\_ `learn
...
{\emph{liccian}} `lick' and the related present {\emph{liccaþ}}, not a
...
The attested noun is {\emph{rēc}} `reek', not {{rēac} `reek' `reek'}.
Clark Hall records {\emph{rēc}} `reek' as the noun and also preserves
related forms such as {\emph{rēcels}} `incense'; Kroonen likewise gives
OE {\emph{rēc}} `reek' under the noun family
...
The Old English form here {{rēac} `reek' `reek'} is therefore a
...
`reek' `reek'}. The attested noun {\emph{rēc}} `reek' belongs to the
same lexical family, but reflects a later smoothed surface form rather
than the regular noun target represented here.
...
{\emph{streowian}} `strew' is a remodelled class-II verb
(\citeproc{ref-RingeTaylor2014}{Ringe and Taylor 2014, sect. 6.1}
n.~27).

Luick groups {strauwjan} `strew' `strew' with the same set as
\emph{*hauwja-} and {kauwjan} `strew' `strew', yielding Anglian
\emph{strēzan} `strew' beside West Saxon forms of the \emph{hīez},
ciezan type (\citeproc{ref-Luick1914}{Luick 1914-\/-1940, sect. 98}).
Fulk likewise allows an early West Saxon \textbf{{striegan} `strew'
`strew'} directly from Proto-Germanic {straujana} `strew' `strew'
...
The target \textbf{{{strīeġan} `strew' `strew'}} is therefore a
...
passes through {straujaną} `strew' `strew', fronting and leveling to a
\emph{*strēajan-} stage, i-umlaut to \textbf{{strīejan} `strew'
`strew'}, and retention or strengthening of the glide after the front
diphthong, written here as \textbf{\emph{ġ{*.\_ The resulting form is
\_**}} \emph{`strew'*strīeġan}}.
...
{{strīeġan} `strew' `strew'} & reconstructed West Saxon inherited
...
The noun is the ordinary Germanic a-stem {fúglaz} `fowl' `fowl',
continued by forms such as Old Norse \emph{fugl} and Old High German
\emph{fogal} (\citeproc{ref-Kroonen2013}{Kroonen 2013, 197};
...
From {fúglaz} `fowl' `fowl', the regular cascade yields \emph{fogol}
...
\emph{*fúglaz} to attested \emph{fugol} `fowl'.
...
English \emph{rust} and Old Saxon and Old High German \emph{rost} `rust'
...
`rust', not \emph{rust} (\citeproc{ref-Campbell1959}{Campbell 1959,
sect. 115}). The same lowering would also affect comparative
