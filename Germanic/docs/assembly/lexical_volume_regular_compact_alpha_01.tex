...
\emph{Alpha 01 compact lexical volume. This volume assembles the current
lexeme-report corpus in manifest order. Regular entries use the compact
book-prose layer; non-regular entries retain their current entry prose.}
...
This alpha assembles the current lexical corpus with the compact
derivation-trace report and project bibliography. The lexical data layer
follows the aligned Germanic dataset as represented by the model-entry
metadata and the current compact trace source; comparative dictionaries,
Old English dictionaries, and historical grammars remain cited in the
entry prose.
...
Regular entries use the compact book-prose overlay in this assembly
pass, while the remaining derivation classes retain their current entry
prose. All entries keep a generated derivation summary and a boxed
derivation trace split into Earlier Germanic changes and Old English
changes.
...
\emph{nǣdre}, \emph{næddre} under the latter (Kroonen 2013, 426). Orel
likewise points from the masculine entry to a feminine \emph{*nēdrōn}
\textasciitilde{} \emph{*nadrōn} type (Orel 2003, 325). The Old English
word is securely represented by \emph{nǣdre}, with \emph{næddre} as a
secondary variant. From \emph{*nḗdrōn}, the stressed long mid vowel
...
standard example of A-restoration (Orel 2003, 33; Campbell 1959, 61;
Ringe and Taylor 2014, 270). Bosworth-Toller and Clark Hall record
\emph{bacan} as the Old English infinitive (Bosworth and Toller 1898,
72; Clark Hall 1960, 19). The selected input \emph{*bákaną} represents
the same weak verb in the accent notation used here. From
\emph{*bákaną}, Anglo-Frisian brightening first gives \emph{*bækaną}.
A-restoration before single \emph{k} returns the stem vowel to \emph{a},
and later weak-tail reductions yield \emph{bacan}.
...
\emph{bēce} (Kroonen 2013, 81). The selected input \emph{*bōkō} is the
nominative-singular shape of that family. The selected form here is
nominative \emph{bōc}; \emph{bēċe} belongs to the same paradigm but is
not the citation form used for this derivation. From \emph{*bōkō}, final
long \emph{ō} raises in Northwest Germanic, and high-vowel apocope
yields \emph{bōc}.
...
vowels in Old English (Ringe and Taylor 2014, 218), and Campbell lists
\emph{ginnan} among familiar examples of palatal \emph{g} in this verb
family (Campbell 1959, 174). Bosworth-Toller lemmatizes the verb as
\emph{be-ginnan} (Bosworth and Toller 1898, 84). Clark Hall likewise
gives \emph{beginnan} (Clark Hall 1960, 34). From \emph{*bigínnaną},
heavy-syllable nasal apocope yields \emph{*bigínnan}. Ringe and Taylor
explicitly cite \emph{bi- \textgreater{} be-} as an Old English
unstressed-prefix development (Ringe and Taylor 2014, 350).
...
\emph{bær} among the reflexes (Kroonen 2013, 717; Clark Hall 1960, 32;
Bosworth and Toller 1898, 73). The selected input \emph{*bḗrō} is the
same noun in the accent notation used here. Clark Hall and
Bosworth-Toller give \emph{bær}, and Kroonen also records \emph{bar}.
The normalized form \emph{bǣr} marks the long vowel explicitly. From
\emph{*bḗrō}, final long \emph{ō} raises, long \emph{ē} lowers to
\emph{ǣ}, and apocope yields \emph{bǣr}.
...
English \emph{(ge-)byrd} among the reflexes (Kroonen 2013, 122). The
selected input \emph{*búrdiz} is the nominative-style form that stands
behind that stem label. Clark Hall and Bosworth-Toller both attest
simplex \emph{byrd} as an Old English noun meaning `birth' (Clark Hall
1960, 70; Bosworth and Toller 1898, 125). From \emph{*búrdiz}, loss of
final \emph{z} gives \emph{*búrdi}.
...
broader derivational setting (Hogg 1992, 357).
...
under \emph{*bainan} (Kroonen 2013, 86; Orel 2003, 71). Both are
comparative headword conventions for the same neuter noun whose Old
English reflex is \emph{bān}. Clark Hall records \emph{bān} as the
ordinary Old English noun (Clark Hall 1960, 44). West Germanic
monophthongization turns stressed \emph{*ai} into \emph{ā}, giving
\emph{*bāną}; heavy-syllable nasal apocope then yields \emph{bān}.
...
\emph{*bō} beside the extended dual forms (Kroonen 2013, 47). Brunner,
Campbell, and Fulk preserve the Old English dual paradigm with masculine
\emph{bēġen}, feminine \emph{bā}, and neuter \emph{bū} (Sievers 1965,
324; Campbell 1959, 295; Fulk 2018, 223). The selected form is neuter
...
\emph{bū} belongs to that set (Campbell 1959, 122; Sievers 1965, 69).
...
(Orel 2003, 45; Fulk 2018, 223). Those analyses concern \emph{bēġen} and
the related extended forms. The derivation here is the neuter \emph{*bō
\textgreater{} bū}.
...
Taylor give the fuller development to West Saxon \emph{biegan} (Kroonen
2013, 75; Ringe and Taylor 2014, 94). The verb belongs to the weak
causative member of the bend-family. Clark Hall and Bosworth-Toller
record the Old English weak verb (Clark Hall 1960, 34; Bosworth and
Toller 1898, 102). The spelling \emph{bīeġan} used here normalizes that
attested form. From pre-Old-English \emph{*bēagjan}, palatalization of
\emph{gj} and i-mutation yield the West Saxon infinitive.
...
and plural `breeches' among its reflexes (Kroonen 2013, 78). Ringe and
Taylor give the plural development directly as PNWGmc \emph{*brokiz}
\textgreater{} \emph{*breeci} \textgreater{} OE \emph{bréc} (Ringe and
Taylor 2014, 223). Bright notes \emph{brōc} with plural \emph{brēc}, and
Clark Hall gives \emph{bréc} fp. breeches while also listing \emph{broc}
as a feminine noun probably represented chiefly in the plural (Bright
1971, 38; Clark Hall 1960, 64). After loss of final \emph{-z}, the stem
ends in \emph{-ki}, so the velar palatalizes and \emph{ō} undergoes
i-umlaut to \emph{ē}; final high-vowel apocope then yields \emph{brēċ}
(Ringe and Taylor 2014, 223).
...
from \emph{*kalbaz}, \emph{*kalbiz-} (Kroonen 2013, 318; Orel 2003, 248;
Ringe and Taylor 2014, 220). Clark Hall gives \emph{cealf}, and
Bosworth-Toller likewise records \emph{Caelf / Cealf} among the ordinary
Old English spellings (Clark Hall 1960, 73; Bosworth and Toller 1898,
131). After loss of final \emph{-z} and bare \emph{-a}, Anglo-Frisian
brightening gives \emph{*kælb}, and breaking before \emph{l} plus
consonant yields \emph{*kealb}.
...
reflexes (Kroonen 2013, 352; Orel 2003, 264). The singular form
...
Bosworth-Toller treats \emph{corn} as an ordinary noun headword (Clark
Hall 1960, 79; Bright 1971, 347; Bosworth and Toller 1898, 144). With
...
northwest Germanic \emph{*dadiz} (Orel 2003, 71; Ringe and Taylor 2014,
27). The stress-marked form \emph{*dḗdiz} represents that same inherited
noun in a notation that keeps the stressed long vowel explicit. Campbell
states that Primitive Germanic long \emph{ē} appears as West Saxon
\emph{ǣ} but in other Old English dialects mostly as \emph{ē} (Campbell
1959, sec. 128). Brunner gives the contrast explicitly as West Saxon
\emph{dǣd} beside non-West-Saxon \emph{dēd} (Sievers 1965, sec. 98).
From inherited \emph{*dēdiz}, loss of final \emph{-z} and the West Saxon
lowering of stressed long \emph{ē} yield \emph{dǣd} (Campbell 1959, sec.
128); Anglian \emph{dēd} preserves the non-West-Saxon outcome (Sievers
1965, sec. 98).
...
German \emph{tura} as reflexes of \emph{*durō-} instead (Kroonen 2013,
150). Clark Hall records \emph{dor} as a neuter noun and separately
records feminine \emph{duru} with its own inflection (Clark Hall 1960,
...
\emph{faran} among the reflexes (Kroonen 2013, 149; Orel 2003, 132;
Campbell 1959, 61). Clark Hall and Bosworth-Toller distinguish strong
\emph{faran} from weak \emph{færan}, while forms such as \emph{fære} and
\emph{færð} belong to the present tense (Clark Hall 1960, 113; Bosworth
and Toller 1898, 250). From \emph{*fáraną}, brightening first gives
\emph{*færaną}; A-restoration before single \emph{r} returns
\emph{*faraną}, and later reductions yield \emph{faran}.
...
\emph{Fell} (Kroonen 2013, 135). The selected input \emph{*féllą} is the
derivable singular form of that same inherited noun. Clark Hall records
\emph{fell} as the noun `fell, skin, hide' (Clark Hall 1960, 115).
Bright's glossary likewise gives \emph{fell, n.}, with inflected forms
such as accusative singular \emph{fel} and dative plural \emph{fellum}
(Bright 1971, 277). With \emph{*féllą}, no special earlier reshaping is
needed: heavy-syllable nasal apocope yields \emph{*féll}, surfacing as
\emph{fell}.
...
\emph{*farnan} with Old English \emph{fearn} (Kroonen 2013, 169; Orel
...
nominative-style \emph{*fárnaz}. Clark Hall gives \emph{fearn} as an Old
English noun, and Bosworth-Toller records \emph{fearn} with inflected
forms such as \emph{fearnes}, \emph{fearna}, and \emph{fearne} (Clark
Hall 1960, 114; Bosworth and Toller 1898, 219). From \emph{*fárnaz},
loss of final \emph{-z} and final \emph{-a} gives \emph{*fárn};
Anglo-Frisian brightening then yields \emph{*færn} (Fulk 2018, sec.
4.12), and breaking before \emph{r} plus consonant gives \emph{fearn}
(Campbell 1959, sec. 144).
...
(Ringe and Taylor 2014, 170). Clark Hall records \emph{feld} with
oblique forms such as \emph{felda} and \emph{felde}, and Campbell notes
early place-name spellings in \emph{-felth} beside the later standard
form (Clark Hall 1960, 114; Campbell 1959, 169). In the modeled pathway,
medial \emph{*lþ} becomes \emph{ld}, final \emph{-z} is lost, and
high-vowel apocope then yields \emph{feld}.
...
\emph{flége} (Ringe and Taylor 2014, 189, 324). The selected input
\emph{*fléuganą} represents that inherited strong verb in the notation
used here. Clark Hall likewise records \emph{flēogan} (Clark Hall 1960,
106). Bright gives the familiar paradigm \emph{flēag}, flugon, flogen
with present \emph{fleogeð} (Bright 1971, 363). From \emph{*fléuganą},
Old English diphthong leveling gives \emph{*flēoganą}; heavy-syllable
nasal apocope and weak-tail reduction then yield \emph{flēogan} (Ringe
and Taylor 2014, 189).
...
but do not replace the infinitive \emph{flēogan} treated here (Ringe and
Taylor 2014, 324).
...
(Kroonen 2013, 374; Orel 2003, 282). The inherited verbal base is
...
(Ringe and Taylor 2014, 357; Clark Hall 1960, 113; Bosworth and Toller
1898, 248). From \emph{*léusaną}, Old English diphthong leveling gives
\emph{*lēosaną}, and later nasal apocope and weak-tail reduction yield
\emph{lēosan} (Ringe and Taylor 2014, 357).
...
English family \emph{forlēosan} / forloren (Ringe and Taylor 2014, 357).
...
gong}, Old Saxon \emph{gang}, and Old High German \emph{gang} (Orel
2003, 164). The selected input \emph{*gángaz} is the same lexeme in the
accent notation used here. Bosworth-Toller records \emph{gang} as the
noun `going, journey, way' (Bosworth and Toller 1898, 159), and Bright's
glossary gives \emph{gong (gang), m., path, course} (Bright 1971, 392).
From \emph{*gángaz}, loss of final \emph{-z} gives \emph{*gánga}, and
later loss of final bare \emph{-a} yields \emph{gang}.
...
English \emph{giefan} among its reflexes (Kroonen 2013, 212). Ringe and
Taylor contrast West Saxon \emph{giefan} with Mercian \emph{for-geofan}
and Northumbrian \emph{geafa}, showing that the inherited verb takes
different later dialectal shapes (Ringe and Taylor 2014, 230). Campbell
gives \emph{gefan (W-S giefan)} among examples of initial
palatalization, and Clark Hall records the verb under plain
\emph{giefan} with forms such as \emph{geaf} and \emph{giefen} (Campbell
1959, sec. 428; Clark Hall 1960, 134). From \emph{*gébaną}, initial
\emph{g} palatalizes before \emph{e}; West Saxon palatal
diphthongization then yields \emph{ie}, and later tail reduction gives
\emph{giefan} (Campbell 1959, sec. 185; Ringe and Taylor 2014, 230).
...
West Saxon vocalism (Ringe and Taylor 2014, 230).
...
Ringe and Taylor cite the noun as \emph{*gulþa-} / \emph{*gulda-} (Ringe
and Taylor 2014, 42). Kroonen gives the same pair (Kroonen 2013, 194).
The selected input \emph{*gúlθą} preserves the older consonantal form
while leaving open whether the medial stop reflects inherited
alternation or regular West Germanic development. Bosworth-Toller and
Clark Hall both record \emph{gold} as the ordinary Old English neuter
noun (Bosworth and Toller 1898, 121; Clark Hall 1960, 152). From
\emph{*gúlθą}, the regular consonant development gives \emph{*gúldą};
Northwest Germanic / Old English lowering then yields \emph{*góldą}, and
apocope gives \emph{gold} (Campbell 1959, sec. 414; Ringe and Taylor
2014, 42). Ringe and Taylor note that the medial stop can be understood
either as alternation \emph{*gulþa-} / \emph{*gulda-} or as the ordinary
West Germanic change \emph{*lþ} \textgreater{} ld; both routes lead to
the same Old English consonantism (Ringe and Taylor 2014, 42).
...
a-restoration before a single consonant and a following back vowel
(Campbell 1959, 61). Clark Hall records \emph{grafan} as the verb `to
dig, grave' and separately records noun \emph{græf} `grave, trench'
(Clark Hall 1960, 153). From \emph{*grábaną}, Anglo-Frisian brightening
first gives a fronted stem vowel.
...
evidence preserves forms of the \emph{gest} type (Campbell 1959, 91;
Ringe and Taylor 2014, 296). Bosworth-Toller and Clark Hall record
spellings such as \emph{gist}, \emph{gest}, \emph{giest}, and
\emph{gyst} (Bosworth and Toller 1898, 387; Clark Hall 1960, 149). The
derivation is regular: brightening and i-mutation reshape the stem, and
West Saxon palatal diphthongization gives \emph{ie}.

Dialect note. Anglian \emph{gest} is genuine Old English evidence
alongside West Saxon \emph{ġiest} (Ringe and Taylor 2014, 296; Bosworth
and Toller 1898, 387).
...
(Kroonen 2013, 260). The selected input \emph{*xḗrą} represents that
same long-ē stem in the present derivation. Clark Hall and
Bosworth-Toller record \emph{hær} / \emph{hǣr} as the ordinary Old
English noun `hair' (Clark Hall 1960, 158; Bosworth and Toller 1898,
510). From \emph{*xḗrą}, Northwest Germanic lowering gives a long front
...
displace the ordinary simplex \emph{hǣr} treated here (Kroonen 2013,
260; Clark Hall 1960, 158).
...
of the \emph{hierfest / hyrfest} type (Bammesberger 1997, 224; Ringe and
Taylor 2014, 141). Clark Hall records \emph{hærfest}, with
\emph{herfest} as a variant in the lexical tradition (Clark Hall 1960,
158; 1960, 171). From \emph{*xárbistuz}, Anglo-Frisian brightening,
breaking, and i-mutation produce a \emph{hierbist-} stage, and later
lowering of unstressed medial \emph{i} to \emph{e} gives
\emph{hierfest}.
...
material in West Saxon transmission (Bammesberger 1997, 230; Ringe and
Taylor 2014, 267).
...
The selected input models a palatal \emph{*-gj-} noun. West Germanic
j-gemination yields the geminate stop in this environment (Campbell
1959, sec. 407). Bosworth-Toller records the noun under the standard
spelling \emph{hecg} (Bosworth and Toller 1898, 495), while Clark Hall
preserves the related lexicographic material under \emph{hege} and
adjacent hedge-family forms (Clark Hall 1960, 152). From \emph{*xágjaz},
later Old English palatalization and loss of final \emph{j} produce the
palatal-geminate outcome \emph{heġġ}.
...
(Bosworth and Toller 1898, 495; Clark Hall 1960, 152).
...
distinguishes a different lexeme \emph{*helman-} `rudder' (Kroonen 2013,
259). The selected input \emph{*xélmaz} is the nominative-style form
used for the helmet noun itself. Clark Hall and Bosworth-Toller record
\emph{helm} as the ordinary Old English noun for `helmet', while
\emph{helma} belongs to a separate rudder lexeme (Clark Hall 1960, 168;
Bosworth and Toller 1898, 542). From \emph{*xélmaz}, loss of final
\emph{z} and later loss of the short final vowel yield \emph{helm}.
...
\emph{helma}, which is a different noun meaning `helm, rudder' (Kroonen
2013, 259; Clark Hall 1960, 168).
...
this input (Bright 1971, 57). Clark Hall and Bosworth-Toller record
\emph{helpan} as the verbal headword `to help' (Clark Hall 1960, 168;
Bosworth and Toller 1898, 542). From \emph{*xélpaną}, no special repair
is needed beyond the ordinary reduction of the infinitive ending.

Form note. Noun \emph{help} belongs to a separate lexical line and
should not replace verbal \emph{helpan} as the target here (Clark Hall
1960, 168; Bosworth and Toller 1898, 542).
...
Kroonen cites the animal name as \emph{*hindō-} f.~`hind' (Kroonen 2013,
266). The selected input \emph{*xéndjō} represents that same noun in the
present derivation. Clark Hall and Bosworth-Toller record \emph{hind} as
the noun `hind, female deer' (Clark Hall 1960, 173; Bosworth and Toller
1898, 554). From \emph{*xéndjō}, i-mutation produces the front-vocalic
Old English stem, and later apocope plus loss of final \emph{j} yield
\emph{hind}.
...
(Clark Hall 1960, 173; Bosworth and Toller 1898, 554).
...
Mercian \emph{haldan} (Campbell 1959, sec. 144; Ringe and Taylor 2014,
199). Bright gives the ordinary strong-verb citation form and principal
parts \emph{healdan, heold, heoldon, healden} (Bright 1971, 62). From
...
rather than corrections to that choice (Campbell 1959, sec. 144; Ringe
and Taylor 2014, 199).
...
\emph{*hurna-} / \emph{*xurnan} type for this noun (Kroonen 2013, 259;
Orel 2003, 234). The selected input \emph{*xúrną} is the
nominative-style form used in the derivation here. Clark Hall,
Bosworth-Toller, and Bright all record \emph{horn} as the ordinary Old
English noun (Clark Hall 1960, 179; Bosworth and Toller 1898, 108;
Bright 1971, 400). From \emph{*xúrną}, Northwest Germanic u-lowering
gives \emph{*xórną}, and later loss of the final nasal leaves
\emph{horn}.
...
the derivational form used here (Kroonen 2013, 259; Orel 2003, 234).
...
\emph{*laidjan-} type for `lead' (Ringe and Taylor 2014, 249; Kroonen
2013, 363). Clark Hall and Bosworth-Toller both record \emph{lædan} /
\emph{lǣdan} as the ordinary Old English verb `to lead, guide, conduct'
(Clark Hall 1960, 194; Bosworth and Toller 1898, 612). The selected form
is the ordinary infinitive citation form. From \emph{*láidijaną},
monophthongization of \emph{*ai} first gives a \emph{*lād-} stage, and
later syncope, i-mutation, weak-tail reduction, and loss of \emph{j}
after a heavy stem yield \emph{lǣdan} (Ringe and Taylor 2014, 249).
...
\emph{*liznō-} type, and Kroonen keeps the same comparative base (Ringe
and Taylor 2014, 318; Kroonen 2013, 328). Campbell records Northumbrian
\emph{liornian} and treats the \emph{eo} of \emph{leornian} as secondary
(Campbell 1959, 123; 1959, 296). The Northumbrian spelling preserves
\emph{io} where original \emph{eo} and \emph{io} remain distinct. The
selected target \emph{liornian} therefore represents the Northumbrian
regular form.

Dialect note. Clark Hall and Bright give \emph{leornian} as the usual
headword (Clark Hall 1960, 193; Bright 1971, 58). From
...
and OHG \emph{(h)lit} among forms that retain \emph{i} (Orel 2003, 216;
Lloyd 1966, 738). Clark Hall and Bosworth-Toller record \emph{hlid} as
the noun `lid, cover, door, gate' (Clark Hall 1960, 175; Bosworth and
Toller 1898, 563). The selected input already represents the later
Germanic \emph{hliđ-} stage used for the derivation here.
...
English noun (Orel 2003, 216; Lloyd 1966, 738).
...
Saxon \emph{liehtan} from the same weak-verb formation (Fulk 2018, 81;
Ringe and Taylor 2014, 264). Clark Hall preserves the verb family under
spellings such as \emph{liehtan}, \emph{lihtan}, and \emph{līhtan},
distinct from the related noun \emph{lēoht} and adjective
\emph{leoht/liht} (Clark Hall 1960, 204). From \emph{*léuxtijaną}, the
regular verbal line preserves \emph{*xt}, passes through a West Saxon
\emph{liehtan} stage, and is represented here by normalized
\emph{līehtan}.
...
shows \emph{lyhtan} (Ringe and Taylor 2014, 264; Campbell 1959, sec.
39).
...
record Old English \emph{lind} `linden, lime-tree' (Kroonen 2013, 337;
Clark Hall 1960, 198; Bosworth and Toller 1898, 630). The selected input
\emph{*líndō} is the citation-form noun used here. Northwest Germanic
final \emph{ō} raises to \emph{*líndu}, and high-vowel apocope yields
\emph{lind}. Clark Hall also gives adjectival \emph{linden} `made of
linden-wood'; that is a related adjective, not the noun treated here
(Clark Hall 1960, 198).
...
\emph{*mélukz} (Kroonen 2013, 404; Orel 2003, 306). Old English
preserves a mixed dossier for this noun. The unsyncopated line from
\emph{*mélukz} loses final \emph{*z}, lowers unstressed \emph{u} to
\emph{o}, and with back mutation yields \emph{meoloc}.
...
Anglian \emph{milc} (Campbell 1959, sec. 574.5). Syncopated \emph{meolc}
and Anglian \emph{milc} belong to the competing leveled tradition
associated with oblique forms, whereas \emph{meoloc / meoluc} preserves
the fuller nominal shape (Ringe and Taylor 2014, 268).
...
\emph{*mōdēr} (Kroonen 2013, 368; Orel 2003, 274). Clark Hall, Campbell,
and Ringe and Taylor preserve the Old English headword tradition
\emph{mōdor / modor} with oblique \emph{mēder} in the paradigm (Clark
Hall 1960, 219; Campbell 1959, 598; Ringe and Taylor 2014, 312). The
...
geminate outcome after a short vowel (Orel 2003, 282; Fulk 2018, sec.
6.15). Bosworth-Toller records \emph{nett} as the noun (Bosworth and
Toller 1898, 29), and Campbell notes that final geminates are often
graphically simplified in Old English spelling (Campbell 1959, sec. 66).
...
evidence is \emph{nett} (Campbell 1959, sec. 66; Orel 2003, 282).
...
maran}, and variant \emph{mere} (Ringe and Taylor 2014, 192). Orel
preserves the same comparative lemma though with a different Old English
headword tradition (Orel 2003, 262). Clark Hall records \emph{mare}
`nightmare, monster' and also preserves related variant forms \emph{mera
/ mere} (Clark Hall 1960, 213). The selected simplex input \emph{*márōn}
regularly gives \emph{mare} after brightening, A-restoration before the
n-stem ending, and later reduction of the final vowel.
...
(Ringe and Taylor 2014, 192; Clark Hall 1960, 213).
...
not as the separate stone word (Orel 2003, 347; Kroonen 2013, 290).
Clark Hall records \emph{rocc} as an over-garment or tunic and preserves
compounds such as \emph{bisceoprocc} and \emph{breóstrocc} (Clark Hall
1960, 259). With \emph{*rúkkaz} as the selected input, Northwest
Germanic u-lowering and later loss of final \emph{-a} yield \emph{rocc}
as a regular outcome.

Source note. This entry concerns the garment noun only. The stone word
seen in \emph{stānrocc} belongs to a different lexical history (Clark
Hall 1960, 259).
...
\emph{*skēp-} type for the same lexeme (Ringe and Taylor 2014, 142; Orel
2003, 340). Clark Hall records \emph{scēap} with spelling variation, and
Campbell likewise lists West Saxon \emph{scéap} among the
palatal-diphthongized forms (Clark Hall 1960, 266; Campbell 1959, sec.
185). From \emph{*skḗpą}, Northwest Germanic lowering gives
\emph{*skǣpą}; after apocope and palatalization the West Saxon branch
diphthongizes to \emph{sċēap}.
...
\emph{scip}. The form represented here is the West Saxon headword (Ringe
and Taylor 2014, 142; Campbell 1959, sec. 186).
...
Kroonen explains the coin word as continuing \emph{*skeld-linga-}
(Kroonen 2013, 482), while Orel likewise gives OE \emph{scilling} among
the reflexes (Orel 2003, 377). The selected input \emph{*skíllingaz} is
the nominative-style form used here to represent that inherited
\emph{*-ing-} derivative. Clark Hall records \emph{scilling}, and
Campbell cites \emph{scilling} among nouns whose derivational
\emph{-ing} keeps \emph{i} in unstressed syllables (Clark Hall 1960,
269; Campbell 1959, sec. 574.6). From \emph{*skíllingaz}, loss of final
\emph{-az} yields \emph{*skílling}.

Form note. Kroonen's \emph{*skeld-linga-} analysis belongs to the
etymological background of the cognate set (Kroonen 2013, 482). The
selected input \emph{*skíllingaz} is the specific form used for the
derivation represented here.
...
and Brunner records Old English \emph{scēawian}, \emph{scāwian} (Orel
2003, 339; Kroonen 2013, 482; Sievers 1965, 148). From
...
\emph{w} and \emph{j}. The result is \emph{sċēawian} (Campbell 1959,
138; Orel 2003, 339).

Form note. The spelling \emph{sċēawian} normalizes initial in dictionary
\emph{scēawian} (Campbell 1959, 19; Hogg 1992, 236).
...
cites the same family under root \emph{*slēb-} (Kroonen 2013, 493; Fulk
...
\emph{slēpen} (Clark Hall 1960, 281; Bright 1971, 435). From
...
\emph{slēp}/\emph{slēap} are preterite forms (Clark Hall 1960, 281;
Bright 1971, 435).
...
Kroonen gives the comparative headword as \emph{*smerwjan-} (Kroonen
2013, 458). Ringe and Taylor instead cite a later-stage
\emph{*smirwijana}, from which they derive West Saxon \emph{smierwan},
Mercian \emph{smirwan}, and Northumbrian \emph{smiriga} (Ringe and
Taylor 2014, 198, 263). The selected form is the West Saxon citation
form \emph{smierwan}. From \emph{*smérwijaną}, breaking before \emph{r +
consonant} yields \emph{eo}, and later i-umlaut produces \emph{ie}.
...
\emph{smirian} or \emph{smyrian} in the same lexical field (Ringe and
Taylor 2014, 263; Clark Hall 1960, 283).
...
\emph{spannan} among the reflexes (Kroonen 2013, 505). The selected
input \emph{*spánnaną} is the infinitive-style form used here for that
same verbal lexeme. Clark Hall keeps noun \emph{spann} and verb
\emph{spannan} separate, and Brunner likewise records \emph{sponnan,
spannan stv.} {[}Clark Hall (1960), 286; Sievers (1965), §392; §396{]}.
...
with the noun treated elsewhere (Clark Hall 1960, 286).
...
\emph{sparro} (Kroonen 2013, 506; Orel 2003, 362). The selected input
\emph{*spárrô} is the OE-facing nominal form used here for that same
lexeme. The noun represented here is \emph{spearra}. From
\emph{*spárrô}, Anglo-Frisian brightening gives \emph{*spærrô}, and OE
breaking before geminate \emph{rr} yields \emph{*spearrô}, later
\emph{spearra}.
...
English glosses overlap (Kroonen 2013, 506; Orel 2003, 362).
...
The selected input \emph{*stéllijaną} represents the verbal j-formation
used for the OE row. Kluge and Seebold keep the wider West Germanic
family of adjective \emph{still} and verb \emph{stillen} in view (Kluge
2011, 885). Clark Hall gives \emph{stillan} as the verb and separately
\emph{stille} as the adjective (Clark Hall 1960, 292). As a heavy-stem
...
to the related adjectival branch of the family (Clark Hall 1960, 292;
Kluge 2011, 886).
...
\emph{*sumeraz} (Kroonen 2013, 492; Ringe and Taylor 2014, 46; Orel
2003, 425). The selected input \emph{*súmaraz} follows the \emph{*a}
vocalism that underlies the regular development represented here. Clark
Hall gives \emph{sumor m., gs. sumeres, ds. sumera, sumere} (Clark Hall
1960, 281). Bright likewise lists \emph{sumor (sumer)} with genitive
\emph{sumeres} (Bright 1971, 440). From \emph{*súmaraz}, loss of final
\emph{-az} is followed by fronting and merger in the unstressed second
syllable, yielding \emph{sumer}.
...
real in Old English (Clark Hall 1960, 281; Bright 1971, 440).
...
sundrian (Orel 2003, 426). Clark Hall keeps \emph{sundrian} and
\emph{syndrian} separate from adverbial \emph{sundor} and preserves the
prefixed verbal family \emph{ā-sundrian} (Clark Hall 1960, 296). From
...
\emph{sundrian} (Orel 2003, 426; Clark Hall 1960, 296).
...
\emph{swealwe} and Mercian \emph{swalwe} develop (Kroonen 2013, 535;
Ringe and Taylor 2014, 200). The selected etymological comparison
belongs to the swallow-bird family, not to the verb \emph{swelgan}.
Clark Hall records \emph{swealwe (a, o)} as the noun headword (Clark
Hall 1960, 299). From \emph{*swálwōn}, brightening yields
\emph{*swælw-}, and breaking before \emph{lw} gives \emph{*swealw-}.
...
\emph{swaluwe} (Campbell 1959, sec. 365) or \emph{swalewan} in the same
broader spelling tradition.
...
Kroonen cites the noun as \emph{*swina-} `pig' (Kroonen 2013, 542). The
selected comparative form here is the bare neuter citation cell
\emph{*swī́ną}, which matches the singular noun aimed at in Old English
rather than an oblique stem form. Clark Hall records \emph{swin (y)} as
the ordinary noun headword (Clark Hall 1960, 301). From selected input
\emph{*swī́ną}, loss of the final nasal vowel yields \emph{swīn}.
...
Kroonen gives the verb as \emph{*þankjan-} `to think' (Kroonen 2013,
533). The noun \emph{*þankaz} belongs only to the wider derivational
background. Bosworth-Toller preserves the verb under
\emph{þencan}/\emph{geþencan}, and the citation form here is the
ordinary infinitive \emph{þenċan} (Bosworth and Toller 1898, 442). From
...
(Campbell 1959, sec. 438; Hogg 1992, sec. 3.4.2.4).
...
masculine pair \emph{*þurnuz} \textasciitilde{} \emph{*þurnaz} (Kroonen
2013, 33; Orel 2003, 430). The selected input \emph{*θúrnaz} belongs to
that same comparative family. Bright lists \emph{þorn}, m., and Clark
Hall likewise treats \emph{þorn} as the ordinary noun headword (Bright
1971, 450; Clark Hall 1960, 327). The inherited stem shows regular
lowering of \emph{u} to \emph{o} before \emph{r}, and final loss yields
\emph{þorn}.
...
the same feminine noun (Kroonen 2013, 556; Orel 2003, 446). The related
verb \emph{tīdan} is separate. Bright records \emph{tīd} with singular
\emph{tīde} and plural \emph{tīda}, and Clark Hall treats \emph{tīd} as
the ordinary noun `time, period, season' (Bright 1971, 444; Clark Hall
1960, 309). From \emph{*tī́diz}, final \emph{z} is lost and the high
final vowel drops, leaving \emph{tīd}. The note matters only because
English \emph{tide} can pull in the separate weak verb \emph{tīdan}; the
noun targeted here is \emph{tīd}.
...
`sign, token' (Kroonen 2013, 506; Orel 2003, 438). The selected input
...
(Campbell 1959, sec. 574), and also records the later broken form
\emph{tācen} (Campbell 1959, sec. 365). Sievers-Brunner likewise
preserves both unbroken and broken shapes (Sievers 1965, sec. 155).
Monophthongization of \emph{ai} yields \emph{ā}, and loss of the final
nasal vowel leaves \emph{tācn}.
...
not displace the older unbroken form (Sievers 1965, sec. 155).
...
\emph{*tūnan} \textasciitilde{} \emph{*tūnaz} (Kroonen 2013, 566; Orel
...
ordinary headword `enclosure, yard, village, town' (Clark Hall 1960,
356). The inherited long \emph{ū} is preserved, and loss of the final
nasal vowel yields \emph{tūn} regularly (Sievers 1965, sec. 71).
...
Campbell describes A-restoration before a following back vowel, and
Luick explicitly includes \emph{wadan} among the standard open-syllable
examples (Campbell 1959, sec. 744; Luick 1914-\/-1940, 239). Clark Hall
gives \emph{wadan} as the verb `to go, move, stride, advance', and
Bright lists the same infinitive in the strong-verb paradigm (Clark Hall
1960, 353; Bright 1971, 456). From \emph{*wádaną}, Anglo-Frisian
brightening first gives \emph{*wædaną}. This infinitive belongs to the
A-restoration class. The citation form is therefore \emph{wadan}, not a
fronted \emph{wæden}-type output.
...
Ringe and Taylor give preterite \emph{*warp} (Ringe and Taylor 2014,
195) and infinitive \emph{*werpana} (Ringe and Taylor 2014, 197). The
selected input here is the verbal form \emph{*wérpaną}. Clark Hall
records \emph{weorpan} as the strong verb headword and separately lists
\emph{wearp} as both noun and preterite. Breaking before \emph{r + C}
yields \emph{weor-}, and the infinitive develops regularly to
\emph{weorpan} (Campbell 1959, sec. 146; Hogg 1992, sec. 3.4.2.2).
English \emph{warp} also points to related \emph{wearp} material. The
selected form here is the infinitive \emph{weorpan}.
...
family (Kroonen 2013, 575; Orel 2003, 489; Ringe and Taylor 2014, 142).
Clark Hall heads the verb as \emph{wascan} (Clark Hall 1960, 343), while
Sievers-Brunner also notes the variant \emph{wæscan} (Sievers 1965, sec.
392). From \emph{*wáskaną}, brightening gives \emph{*wæskaną}.
...
within the same verbal family (Kroonen 2013, 606; Orel 2003, 478; Ringe
and Taylor 2014, 191). Clark Hall gives \emph{weaxan} as the verb
headword and separately records bare \emph{wax} as a preterite form;
Bright likewise treats \emph{weaxan} as the infinitive (Clark Hall 1960,
358; Bright 1971, 458). From \emph{*wáxsaną}, brightening and breaking
yield \emph{weax-}, and the infinitive develops regularly to
\emph{weaxan}. The selected form is the infinitive \emph{weaxan}. Noun
\emph{weax} and preterite wax/\emph{wēox} belong to different lexical or
paradigm slots.
...
derivational form here is nominative-singular \emph{*wégaz} (Kroonen
2013, 577). Ringe and Taylor use the same word as the standard contrast
between singular palatal \emph{weġ} and inflected \emph{wegas/wegum}
(Ringe and Taylor 2014, 341). The Old English singular is the ordinary
noun \emph{weg}, here normalized as \emph{weġ} to show the palatal
final. From \emph{*wégaz}, final \emph{*z} is lost and the weak tail
...
wēpna-} and cites Old English \emph{wæpn} among the reflexes (Kroonen
2013, 617; Campbell 1959, 150; 1959, 226). Campbell preserves unbroken
...
elsewhere (Bright 1971, 29; Clark Hall 1960, 355). The selected form
...
(Campbell 1959, 150; 1959, 226; Bright 1971, 29; Clark Hall 1960, 355).
...
\emph{*weljan-} 1 `to want', while Orel represents the noun as
\emph{*weljōn} (Kroonen 2013, 618; Orel 2003, 492). The selected
derivational form \emph{*wéljô} is the noun-side input used for this
row. Clark Hall lemmatizes noun \emph{willa m.} separately from verb
\emph{willan} (Clark Hall 1960, 368). From \emph{*wéljô}, j-gemination
yields a heavy stem, i-umlaut gives \emph{will-}, and later shortening
plus j-loss produce \emph{willa}. The selected form is the noun
\emph{willa} `will, wish'. Related verb \emph{willan} belongs to a
separate lexeme and should not be substituted for the noun row (Kroonen
2013, 618; Clark Hall 1960, 368).
...
the present row belongs to the verb (Kroonen 2013, 587). Clark Hall and
Bosworth-Toller record \emph{windan} as the verb headword (Clark Hall
1960, 335; Bosworth and Toller 1898, 101). The selected input
\emph{*wíndaną} yields the regular infinitive \emph{windan} by ordinary
heavy-syllable apocope and weak-tail reduction. English \emph{wind} also
names the noun. This row targets the class-III verb, not the noun
(Kroonen 2013, 587; Clark Hall 1960, 335).
...
\emph{weald} beside other West Germanic \emph{wald} forms (Kroonen 2013,
611). The selected input \emph{*wálθuz} is the nominative singular used
for the derivation. Clark Hall makes \emph{weald} the main noun headword
and cross-refers \emph{wald} and \emph{wold} to it (Clark Hall 1960,
343). \emph{*lþ} voices to \emph{ld}, Anglo-Frisian brightening yields
\emph{wæld-}, and breaking before the cluster gives \emph{weald-};
apocope then yields \emph{weald}.
...
variant form (Clark Hall 1960, 343).
...
\emph{*gearn} \textgreater{} OE \emph{gearn} (Kroonen 2013, 209; Ringe
and Taylor 2014, 220). Clark Hall records \emph{gearn (e) n.} `yarn,
spun wool' (Clark Hall 1960, 145). From \emph{*gárną}, brightening and
breaking before \emph{rn} yield \emph{gearn}; palatalization of initial
\emph{g} before the resulting front-vocalic sequence gives normalized
\emph{ġearn}.
...
(Kroonen 2013, 355). Orel likewise lists \emph{cwidu} under the cognate
set (Orel 2003, 266). The selected input \emph{*kwéðuz} therefore
represents the older e-grade, voiced-dental form behind the chosen
variant \emph{cwedu}.
...
\emph{cudu} (Ringe and Taylor 2014, 338; Clark Hall 1960, 84). Attested
...
\emph{hund-tēon-tig} (Fulk 2018, sec. 10.2). Brunner gives the same
contrast more broadly: \emph{tēon} develops from \emph{*tëhun}, while
West Saxon tien, \emph{tȳn} belong to a different, umlauted branch of
the numeral history (Sievers 1965, secs. 129.2, 129 Anm. 6, 234).
...
\emph{tēa} (Campbell 1959, sec. 682). Brunner likewise lists West Saxon
tien, \emph{tȳn} beside \emph{tēn}, \emph{tēo}, \emph{tēa} in other
dialects (Sievers 1965, sec. 325).
...
\emph{tēoða} and \emph{-tēontig} (Sievers 1965, sec. 129.2; Fulk 2018,
sec. 10.2). The comparison form \emph{tēon} is therefore a normalized
spelling of that un-umlauted base.
...
a front-vocalic trigger (Fulk 2018, sec. 10.2; Sievers 1965, sec. 129
Anm. 6).
...
rather than under one Old English-ready paradigm cell (Kroonen 2013,
586). The input \emph{*θréjez} is therefore best understood as the
...
\emph{þri} for \emph{þrīe} (Campbell 1959, sec. 683). Fulk presents the
same masculine \emph{þrīe} beside the wider numeral paradigm (Fulk 2018,
sec. 10.1).
...
variants \emph{wæsp} and \emph{wæps} (Fulk 2018, sec. 6.5). Bülbring and
Brunner then make the chronology more explicit by deriving later
\emph{wæps} and late West Saxon \emph{wasp} from earlier \emph{waefs} /
\emph{wæfs} through restricted metatheses (Bülbring 1902, sec. 484 Anm.
3; Sievers 1965, secs. 193, 204).
...
Brunner (Bülbring 1902, sec. 484 Anm. 3; Sievers 1965, sec. 193). Later
Old English also shows \emph{wæps} and \emph{wæsp} / \emph{wasp}, and
dictionary practice often favors \emph{wæps} or later spellings as
headwords (Clark Hall 1960, 341).
...
(Kroonen 2013, 120). The comparative label \emph{*búdmaz} names the
lexeme-level stem complex, while the selected input \emph{*búttmaz}
...
\emph{*budmaz} \emph{*butmaz} (Orel 2003, 100). The selected input is
...
(Clark Hall 1960, 63). Bosworth-Toller cross-references \emph{bodan} to
\emph{botm}, showing the wider reflex family without weakening the
attested lemma (Bosworth and Toller 1898, 112).
...
Old English \emph{brand} and its continental cognates (Orel 2003, 53).
The selected input \emph{*brándas} is not a different lexeme but the
genitive singular of that same a-stem noun.
...
Old English dictionaries lemmatize the noun as \emph{brand} (Clark Hall
1960, 49; Bosworth and Toller 1898, 116). Bosworth-Toller also records
inflectional forms such as \emph{brandas}, \emph{branda}, and
\emph{brandum} under the same entry (Bosworth and Toller 1898, 116).
...
and Old Saxon \emph{briost} (Kroonen 2013, 114; Orel 2003, 95; Ringe and
Taylor 2014, 43).
...
Clark Hall records the noun as \emph{brēost} / \emph{breóst} (Clark Hall
1960, 65). The form is an established Old English lexeme, not a
reconstructed target assembled from comparative evidence alone.
...
vowel history (Campbell 1959, sec. 115). No special repair is needed
...
\textasciitilde{} \emph{*kraftuz} (Kroonen 2013, 340; Orel 2003, 259).
The comparative label \emph{*kráftiz} remains in view as a lexeme-level
shorthand, while \emph{*kráftaz} is the pre-Old-English form used for
...
both give \emph{cræft} as the headword (Clark Hall 1960, 19; Bosworth
and Toller 1898, 145).
...
(Fulk 2018, 170). The selected input \emph{*déliz} therefore represents
the i-stem side of the paradigm, whereas the citation label
\emph{*déljaz} is a broader comparative headword.
...
background for the OE side (Fulk 2018, 170).
...
the variant \emph{dili} (Bosworth and Toller 1898, 164; Clark Hall 1960,
95). The form discussed here is therefore an attested Old English noun
with single \emph{l}.
...
behind Old English `to fast' (Kroonen 2013, 171). Ringe and Taylor,
...
stative meaning through lexical confusion (Ringe and Taylor 2014, 110).
...
related \emph{fæstan} / \emph{fǣstan} spellings and meanings (Bosworth
and Toller 1898, 213). The form selected here is \emph{festan}, which
...
to a weak feminine formation \emph{*fláskōn} / \emph{*flaskǭ} (Orel
2003, 104). That distinction is crucial for the suffixal history of the
noun.
...
Saxon \emph{flaxe} is also noted as a secondary variant (Bosworth and
Toller 1898, 235; Clark Hall 1960, 121).
...
\emph{flascan} (Campbell 1959, sec. 158). Once the weak feminine
formation is chosen, the noun follows a regular path to its Old English
shape.
...
English \emph{fylg(e)an} continue a formation \emph{*fulgjan-} (Kroonen
2013, 159). The comparative headword and the class-I formation are
...
e-stative (Ringe and Taylor 2014, 293--94). This is a stem-class choice,
not a spelling choice. The selected input \emph{*fúlgijaną} belongs to
the class-I \emph{*fulgija-} / \emph{*fulgjan-} branch; the citation
form \emph{*fulgēną} belongs to the parallel class-II history behind
\emph{folgian}.
...
(Clark Hall 1960, 125). Bosworth-Toller likewise has a separate
\emph{fylgean} entry (Bosworth and Toller 1898, 275).

Bright notes traces of the older conjugation in \emph{fylg(e)an} (Bright
1971, 77) and lists \emph{folgian (fylgean)} in the glossary (Bright
1971, 364). The relevant comparison form in this entry is therefore the
class-I verb \emph{fylgan} / \emph{fylgean}, here normalized as
\emph{fylġan}. The spelling with ġ represents the palatalized velar
before a front-vocalic environment.
...
no \emph{*j} intervened (Ringe and Taylor 2014, 96). The same
front-vocalic environment then triggers i-umlaut, so \emph{u} becomes
\emph{y} (Ringe and Taylor 2014, sec. 6.6.2).
...
\emph{*gallōn-}, cited here as \emph{*gállô} (Kroonen 2013, 165). The
...
(Bosworth and Toller 1898, 297; Clark Hall 1960, 145; Bright 1971, 372).
The form used here, \emph{ġealla}, is a normalized spelling with macrons
omitted and palatal ġ made explicit.

Campbell also notes dialectal variation, contrasting West Saxon or
Kentish \emph{gealla} with Anglian \emph{galla} (Campbell 1959, sec.
486). The target of this entry is the West Saxon type \emph{ġealla}.
...
before \emph{ll}, yielding \emph{ġealla} (Campbell 1959, sec. 486). Once
the weak masculine input is chosen, the noun follows a regular path to
its attested Old English form.
...
\emph{*knextaz} (Ringe and Taylor 2014, 142; Orel 2003, 256).
Kluge-Seebold likewise points to \emph{*knehta-} (Kluge 2011, 506). The
selected input \emph{*knéxtaz} follows that comparative evidence.
...
Old English dictionaries record the noun as \emph{cniht} (Clark Hall
1960, 63; Bosworth and Toller 1898, 71). Campbell cites plural
\emph{cneohtas} among the broken forms, showing the same vowel
environment from another point in the paradigm (Campbell 1959, sec.
146).
...
\emph{cniht} beside plural \emph{cneohtas} (Campbell 1959, sec. 305).
Sievers-Brunner gives the same contrast as \emph{cniht \ldots{}
cneohtas} (Sievers 1965, sec. 122). With that corrected input, the
...
Ringe and Taylor cite the strong verb \emph{hladan} directly (Ringe and
Taylor 2014, 248). The citation label \emph{*laθōjaną} is used here only
as a broader family heading, not as the direct source of the OE
derivation.
...
expected strong-verb paradigm material around it (Bosworth and Toller
1898, 559). Clark Hall likewise records \emph{hladan} (Clark Hall 1960,
159). The target is an attested infinitive rather than a reconstructed
paradigm cell.
...
in the present system (Campbell 1959, sec. 744). Ringe and Taylor
likewise derive \emph{hladan} from the voiced strong-verb line (Ringe
and Taylor 2014, 248). The resulting Old English infinitive is
\emph{hladan}.
...
variant \emph{læppa} (Orel 2003, 236; Clark Hall 1960, 180). The
selected input \emph{*láppô} follows that evidence.
...
Campbell cites \emph{lappa} as a case of restored \emph{a} (Campbell
1959, sec. 158). Sievers-Brunner records \emph{lappa} beside variant
\emph{læppa} (Sievers 1965, sec. 10). The dictionary tradition also
preserves \emph{læppa} (Clark Hall 1960, 180; Bosworth and Toller 1898,
613).
...
\emph{a} (Campbell 1959, sec. 158). Sievers-Brunner records \emph{lappa}
beside variant \emph{læppa} at the same Old English stage (Sievers 1965,
sec. 10). With the weak masculine input chosen, the selected
...
the citation label \emph{*lákaną}, while the selected input
...
Bosworth-Toller records \emph{hlihhan} as the verb `to laugh' (Bosworth
and Toller 1898, 551). Clark Hall cross-references \emph{hlæhan},
\emph{hlehhan}, and \emph{hlihhan} to \emph{hliehhan} (Clark Hall 1960,
160--61). Bright's glossary likewise gives \emph{hlihhan (hliehhan,
hlyhhan)} (Bright 1971, 315). The target of this entry is the West Saxon
\emph{hliehhan}.
...
consonant (Campbell 1959, sec. 407). Ringe and Taylor derive Old English
\emph{hliehhan} from the j-present branch via breaking before the
palatalized geminate (Ringe and Taylor 2014, 240). They separately
compare the related noun \emph{hleahtor} as the outcome of
\emph{*hlahtraz} (Ringe and Taylor 2014, 328).
...
\emph{lām} as a neuter reflex of that family (Orel 2003, 272; Kroonen
...
for `loam, clay, mud' (Bosworth and Toller 1898, 604; Clark Hall 1960,
...
\emph{lungen} and close West Germanic cognates (Kroonen 2013, 384). The
selected input \emph{*lúnganjō} models that derived feminine formation
rather than the base noun. The notation differs slightly from Kroonen's
...
inflected forms such as \emph{lungenne} and \emph{lungena} (Bosworth and
Toller 1898, 634). Clark Hall also preserves a small family of compounds
such as \emph{lungenādl}, \emph{lungensealf}, and \emph{lungenwyrt}
(Clark Hall 1960, 191).
...
\emph{nafela} among its reflexes (Kroonen 2013, 420), while Ringe and
Taylor give the derivational pathway \emph{*nabulō} \textgreater{}
\emph{*næbula} \textgreater{} \emph{nafola} (Ringe and Taylor 2014,
270). The difference is one of stage and notation rather than of lexeme
identity: the selected input \emph{*nábulô} is the pre-syncope form
...
\textgreater{} \emph{nafela} (Ringe and Taylor 2014, 336). Campbell
likewise records \emph{nafela} beside Corpus \emph{nabula} (Campbell
1959, sec. 159). The target of this entry is the nominative singular
...
\emph{*næbula} \textgreater{} OE \emph{nafola} (Ringe and Taylor 2014,
270). The trace represents that same development with stress-marked
notation and explicit intermediate weakening. Intervocalic \emph{b} then
surfaces as \emph{f}, and final weak-tail shortening gives
\emph{nafola}.
...
\emph{hnecca} among the e-grade descendants (Kroonen 2011, 167).
Kluge-Seebold likewise identifies \emph{ae. hnecca} as an ablaut partner
of the a-grade \emph{Nacken} family (Kluge 2011, 347).

A competing comparative label \emph{*xnákkaz} remains useful for the
wider family, and Orel also gives an a-grade headword line (Orel 2003,
218). The selected input \emph{*xnékkô}, however, is the form that
matches the Old English branch.

\paragraph{Old English evidence}\label{old-english-evidence-20}

Clark Hall records the weak masculine noun \emph{hnecca} (Clark Hall
1960, 162). Bosworth-Toller likewise records \emph{hnecca} (Bosworth and
Toller 1898, 567). The target is therefore an attested citation form,
not an oblique cell or a reconstructed lemma.
...
\emph{nédl} (Ringe and Taylor 2014, 329). The selected input
\emph{*nḗðlō} is the voiced Verner-grade form used for the Old English
comparison, while the citation form \emph{*nḗθlō} remains the broader
lexeme label.
...
Clark Hall records the attested citation form \emph{nǣdl} (Clark Hall
1960, 210). Campbell lists \emph{nédl} among the expected unbroken forms
after \emph{t} and \emph{d} (Campbell 1959, sec. 367). Hogg also
includes \emph{nidi} / \emph{nǣdl} in the same broader cluster history
(Hogg 1992, 95).
...
\ldots{} \textgreater{} OE \emph{nédl} (Ringe and Taylor 2014, 329).
Campbell likewise lists \emph{nédl} among the expected unbroken forms
after \emph{t} and \emph{d} (Campbell 1959, sec. 367). The trace
expresses the same pathway with the voiced alternant selected for the
...
the older paradigm (Kroonen 2013, 423). Campbell is more specific for
Old English, citing \emph{nosu} \textless{} \emph{*nusō} (Campbell 1959,
44).
...
the two lines should not be collapsed without comment (Orel 2003, 320).
...
among the few surviving early Old English feminine u-stems (Ringe and
Taylor 2014, 385). Clark Hall likewise gives \emph{nosu f.}, with
genitive-dative singular \emph{nosa}, and cross-refers \emph{nasu} to
\emph{nosu} (Clark Hall 1960, 810).
...
\emph{sæp} m. (Kroonen 2013, 420). Orel preserves the comparative
notation \emph{*sapōn} \textasciitilde{} \emph{*sapan} (Orel 2003, 319).
...
Clark Hall records \emph{sæp} (e) n. (Clark Hall 1960, 247). The target
is therefore an attested neuter Old English noun. Orel's plain
\emph{sap} notation belongs to comparative normalization, not to the
spelling adopted here for the Old English form (Orel 2003, 319).
...
whose English reflex is cited as OE \emph{sæ} (Kroonen 2013, 423). Ringe
and Taylor write the fuller form \emph{*saiwiz} and derive it through
\emph{*sawi} \textgreater{} \emph{*sei} \textgreater{} OE \emph{sǣ}
(Ringe and Taylor 2014, sec. 6.7.1). The comparative headword is
therefore shorter than the form required for the English history:
\emph{*sái} names the lexeme, but \emph{*sáiwiz} preserves the medial
\emph{*w} and the final high vowel that control the later development.
...
\emph{sæ}; the normalized form here is \emph{sǣ} (Kroonen 2013, 423).
Campbell likewise treats \emph{sea} as continuing the same
\emph{*saiui-} \textgreater{} \emph{*sǣi} history, with loss of
\emph{u}/\emph{w} before \emph{i} (Campbell 1959, sec. 406).
...
\emph{*sāi} \textgreater{} \emph{*sǣi} \textgreater{} \emph{sǣ} (Ringe
and Taylor 2014, sec. 6.7.1; Campbell 1959, sec. 406).
...
groups \emph{sife} with short neuter i-stems such as \emph{spere} (Kluge
2011, 847; Campbell 1959, sec. 609). The older morphological background
is the s-stem \emph{*sib-iz}, but the selected input is the normalized
i-stem form \emph{*síbi}.
...
form treated here (Orel 2003, 328).

\paragraph{Old English evidence}\label{old-english-evidence-25}

Clark Hall gives \emph{sibi (GL) \ldots{} = sife} and also \emph{sife
n.~`sieve'} (Clark Hall 1960, 263). Campbell likewise cites Corpus
Glossary \emph{sibi} and treats \emph{sife} as a short neuter i-stem
(Campbell 1959, secs. 444, 609). The normalized Old English target is
therefore \emph{sife}, while \emph{sibi} is an attested earlier spelling
rather than a separate lexeme.
...
(Kroonen 2013, 465). Orel similarly preserves \emph{*sparēnan} (Orel
2003, 362). Ringe and Taylor, however, reconstruct \emph{*sparai-}
\textasciitilde{} \emph{*sparja-} for the English branch and derive the
citation verb from a class-II line (Ringe and Taylor 2014, 162, 191).
The selected input \emph{*spárōjaną} therefore represents the
refashioned class-II formation behind Old English \emph{sparian}, while
the citation reconstruction \emph{*sparēną} remains the inherited
comparative headword.
...
without back vowels (Campbell 1959, sec. 764). Brunner likewise records
Northumbrian \emph{spæria}, \emph{spærede} beside common Old English
\emph{sparian} and Vespasian Psalter \emph{spearad} (Sievers 1965, sec.
...
continuation of inherited \emph{*spárē} (Sievers 1965, sec. 150).
...
reconstructs an a-stem \emph{*staba-} (Kroonen 2013, 471). Orel writes
\emph{*stabiz} \textasciitilde{} \emph{*stabaz} (Orel 2003, 368). That
disagreement matters because a direct i-stem input in \emph{*-iz} would
predict i-mutation in Old English, whereas the attested noun keeps
\emph{æ}.
...
Luick lists \emph{stæf} among closed monosyllables with \emph{æ} (Luick
1914-\/-1940, 176). Ringe and Taylor pair singular \emph{stæf} with
plural \emph{stafas} (Ringe and Taylor 2014, 193). The normalized form
here is therefore \emph{stæf}; later English \emph{staff} with \emph{a}
...
(Ringe and Taylor 2014, 330). Orel likewise gives \emph{*stebnō}
\textasciitilde{} \emph{*stemnō}, whereas Kroonen prefers
\emph{*stimnō-}, and Fulk describes the etymology of \emph{stefn, stemn}
as insecure (Orel 2003, 374; Kroonen 2013, 480; Fulk 2018, sec. 6.11
n.~6).
...
cross-refers \emph{stemn} to the same word (Clark Hall 1960, 276). Ringe
and Taylor give the OE chronology directly as \emph{stebn}
\textgreater{} \emph{stefn} \textgreater{} \emph{stemn} (Ringe and
Taylor 2014, 330).
...
only after the earlier period of nasal influence on \emph{e} (Bülbring
1902, sec. 62 Anm. 3, 445; Luick 1914-\/-1940, sec. 75 Anm. 1). The
...
this stage (Ringe and Taylor 2014, 330; Bülbring 1902, sec. 445).
...
\emph{*swanaz} (Orel 2003, 367). The selected input \emph{*swánas} is
not a competing lexeme reconstruction. It is the genitive singular of
the same paradigm.
...
\emph{swanes feðre} (Bright 1971, 441).
...
Old English \emph{þūsend} among its continuations (Kroonen 2013, 554).
The selected input \emph{*θūs-èndi} is not the same claim. It is an
...
High German keep \emph{u} in the second syllable? (Kroonen 2013, 554).
...
normal case forms (Campbell 1959, sec. 689). The problem lies in the
internal history of the word, not in its lexical status.
...
\emph{*þȳsend} type rather than attested \emph{þūsend} (Campbell 1959,
sec. 203). Preserved root \emph{ū} therefore argues that the
...
\emph{thousand} with forms reshaped on that pattern (Luick 1914-\/-1940,
secs. 198, 492). Viredaz is more cautious, arguing that Old English
\emph{e} in this weak position may simply write schwa and so need not
prove a unique \emph{ærende}-type analogy (Viredaz 2025, sec. 2.1.4).
...
\emph{timber} among its continuations (Kroonen 2013, 517). Ringe and
Taylor instead state the history from PGmc \emph{*timra} through West
Germanic \emph{*timbr} to Old English \emph{timber} (Ringe and Taylor
2014, 327).
...
\emph{timbor} as a variant spelling (Clark Hall 1960, 294). The Old
English form is thus an ordinary citation noun, not a selected oblique
cell or a reconstructed convenience form.
...
of this epenthetic vowel point to the same Old English result (Ringe and
Taylor 2014, 327; Campbell 1959, secs. 463--464).
...
\emph{wacan} (Kroonen 2013, 568). Ringe and Taylor separately derive Old
English \emph{wacian} from weak \emph{*wakai-} \textasciitilde{}
\emph{*wakja-} (Ringe and Taylor 2014, sec. 3.3.2).
...
(Clark Hall 1960, 338). Bosworth-Toller adds an important caution under
\emph{wacan}: the simplex infinitive itself does not occur, its place
seeming to be taken by \emph{wæcnan} (Bosworth and Toller 1898, 226).
...
material points to \emph{*watōr}, \emph{*watenaz} (Kroonen 2013, 616).
Ringe and Taylor likewise start from singular \emph{*wator} before the
Old English branch (Ringe and Taylor 2014, sec. 3.1.4).
...
\emph{wæterum} (Bright 1971, 29). Ringe and Taylor add the dialectal
contrast between West Saxon \emph{weeter} and Mercian \emph{weter}
(Ringe and Taylor 2014, sec. 6.5.2).
...
\emph{r}, yielding \emph{*watar} (Ringe and Taylor 2014, sec. 3.1.4).
Anglo-Frisian brightening then gives \emph{*wætær}, and merger of
unstressed \emph{æ}/\emph{e} yields \emph{wæter}.
...
notes some mixed \emph{*xwaliz} evidence (Orel 2003, 197). Kroonen
instead cites \emph{*hwali-} (Kroonen 2013, 262).
...
preserves the plural \emph{hwalas} (Clark Hall 1960, 170; Bosworth and
Toller 1898, 326). The comparison form is normalized here as \emph{hwæl}
for the singular citation form with Anglo- Frisian fronting.
...
\emph{*hwinan-} (Kroonen 2013, 267). Orel likewise distinguishes strong
\emph{*xwinanan} from weak \emph{*wainōjanan} (Orel 2003, 201). Ringe
and Taylor make the same split at the Northwest Germanic level, linking
Old Norse \emph{hvina} and Old English \emph{hwinan} to the same strong
verb (Ringe and Taylor 2014, 130).
...
selected input \emph{*xwī́naną} therefore represents a competing
...
whistle' (Clark Hall 1960, 171). Seebold keeps the verb among the strong
verbs and notes that only a present-tense attestation is directly
preserved (Seebold 1970, 280).
...
\emph{*wīþja}/\emph{ō-} or \emph{*wiþ-} type (Orel 2003, 503). That
material is useful for the cognate set, but it does not by itself
explain the Old English suffix of \emph{wīþiġ}.
...
\emph{*-ij-} (Campbell 1959, secs. 275, 376). The selected input
...
Clark Hall records the noun as \emph{wiðig} (Clark Hall 1960, 358). The
form used here, \emph{wīþiġ}, is a normalized Old English spelling with
...
(Campbell 1959, secs. 275, 376). Palatalization supplies the final
\emph{ġ}, and the full development reaches \emph{wīþiġ}.
...
(Orel 2003, 501; Ringe and Taylor 2014, 341). Kluge-Seebold likewise
gives the compound \emph{*wira-aldō} and explicitly includes Old English
\emph{weorold} (Kluge 2011, 981).
...
(Ringe and Taylor 2014, 341). The early analogical step lies in that
stem-class reassignment; the later phonological developments can then
run regularly.
...
(Ringe and Taylor 2014, 341). Sievers-Brunner and Bright present the
same wider set, including the syncopated \emph{world} and later rounded
\emph{wurold} (Sievers 1965, sec. 113; Bright 1971, 465).
...
selected comparative label (Ringe and Taylor 2014, 341). The modeled Old
English form therefore stands at the meeting point of an early
stem-class reshaping and later regular sound change.
...
word as \emph{*ju(w)unþi-} (Kroonen 2013, 316). The selected comparative
label \emph{*júgunθiz} already stands at a later Germanic stage with
\emph{g}, and the chosen input \emph{*júgunθ} is later again: it
...
\emph{geoguþ} \textasciitilde{} \emph{iuguþ} (Ringe and Taylor 2014,
141). The selected input therefore differs from the broader comparative
...
Taylor cite \emph{geoguþ} \textasciitilde{} \emph{iuguþ} (Ringe and
Taylor 2014, 141). The form is normalized here as \emph{ġeoguþ}: the
initial palatal is written with \emph{ġ}, and the attested spelling
...
forms such as \emph{duguþ} and \emph{munuc} (Campbell 1959, sec. 374).
Brunner likewise cites \emph{iuzuð} \emph{Jugend} and \emph{munuc}
\emph{Mönch} in the same environment (Sievers 1965, sec. 150.3).
...
\emph{gebann} as the noun reflex (Orel 2003, 35; Seebold 1970, 89). The
citation reconstruction \emph{*bánną} therefore names the lexeme, but
the selected input \emph{*bánnas} is a specific genitive singular cell.
...
Bosworth-Toller records \emph{ge-bann} with oblique usage such as
\emph{gebanne} (Bosworth and Toller 1898, 303). The exact unprefixed
genitive \emph{bannes} is less directly cited in the reviewed material,
so it is best treated here as the selected regular genitive comparison
form rather than as a dictionary headword.
...
\emph{*bazja-} (Kroonen 2013, 54). The selected input \emph{*bázjas} is
therefore not a rival lexeme headword, but a specific genitive singular
cell drawn from that paradigm.
...
gemination repair (Ringe and Taylor 2014, 181).
...
is retained after \emph{r} in this type (Campbell 1959, 250). The
reviewed evidence therefore supports the citation form more directly
than the exact genitive \emph{berġes}, which is best read here as the
...
\emph{herges} beside \emph{here} (Ringe and Taylor 2014, 181). This
...
\emph{*béuganą} (Ringe and Taylor 2014, 55). Within that paradigm,
however, the infinitive and the singular preterite continue different
ablaut grades. The selected input \emph{*báug} is the singular preterite
cell, whereas the citation form \emph{*béuganą} is the infinitive.
...
phonological path relevant for \emph{bēag}, whereas the analogical
\emph{ū} of the present stem belongs to the separate history behind
\emph{būgan} (Ringe and Taylor 2014, 55).
...
form of \emph{būgan} (Bosworth and Toller 1898, 122; Clark Hall 1960,
...
diphthong produce \emph{bēag} (Campbell 1959, 53). No special analogical
...
obl. \emph{*kū-} (Kroonen 2013, 299). The citation form therefore
belongs to the nominative singular, whereas the selected input
\emph{*kūi} belongs to the oblique stem.

Ringe and Taylor also posit a later PNWGmc nominative \emph{*kūaz}
\textgreater{} \emph{*kūz} behind Old English \emph{cū} (Ringe and
Taylor 2014, sec. 3.1.3). That nominative history matters for the
headword, but the form \emph{cȳ} depends on the oblique \emph{*kū-} stem
and a following \emph{*i}.
...
\emph{cȳ}, and dat.pl. \emph{cūm} (Clark Hall 1960, 67). Ringe and
Taylor likewise state that the root-noun \emph{cū} exhibits dat.sg.,
nom.-acc.pl. \emph{cȳ} \textless{} \emph{*cūi}, dat.pl. \emph{cūm}
\textless{} \emph{*cūm}, and apparently gen.sg. \emph{cā} \textless{}
\emph{*cūiz} (Ringe and Taylor 2014, sec. 6.6.1).
...
paradigm statement (Ringe and Taylor 2014, sec. 6.6.1). It also explains
why \emph{cȳ} is the cleanest comparison form for the present entry,
even though the ordinary headword is \emph{cū}.
...
English \emph{findan} (Ringe and Taylor 2014, 344). The selected input
\emph{*fúnðanǭ}, however, belongs to the past-participial paradigm
rather than to the infinitive. It represents an oblique singular form of
the participle.

That distinction matters because the familiar dictionary form
\emph{funden} is not the selected comparison form for this entry. The
selected input instead models an attested participial cell directly,
rather than treating the infinitive or the ordinary dictionary headword
as the privileged target. It therefore targets the comparison form
\emph{fundene} in the cell where the trace and the attested form match
directly.
...
form in \emph{Beón} \emph{þá} herigeata swa fundene (Bosworth and Toller
1898, 219). Clark Hall likewise preserves the participial stem in forms
such as \emph{funden} and \emph{tō-fundennes} (Clark Hall 1960, 124).
...
whereas the selected oblique form \emph{fundene} is the attested
comparison cell used here.
...
participle from the more familiar nominative participial forms. It is
the project's cell-based comparison based on the cited Old English forms
and the documented trace output.
...
selected input and target directly, while the nominative participial
headword remains a different presentation cell within the same paradigm.
...
same family as Gothic \emph{faurhtei} (Orel 2003, 120). The selected
input \emph{*fúrxtīnaz} is not a different lexeme but an oblique
singular cell within that in-stem paradigm.

Ringe and Taylor treat the later nominative forms with \emph{-u} or
\emph{-o} as analogically remodeled (Ringe and Taylor 2014, 395--96). In
project terms, the oblique in-stem forms therefore preserve the older
history more directly. The selected input thus differs from the citation
form because the oblique cell keeps the inherited pathway more clearly
than the better-known lemma forms do.
...
(Bosworth and Toller 1898, 160). Clark Hall separately preserves
adjective and verb material under \emph{fyrht} / \emph{fyrhtan}, which
helps keep the noun distinct from the adjectival family (Clark Hall
1960, 141).
...
evidence, but the selected target of this entry is the oblique cell.

\paragraph{Development to Old
English}\label{development-to-old-english-44}

From \emph{*fúrxtīnaz}, the oblique in-stem develops through the loss
and weakening of the final ending, yielding \emph{fyrhte}. In project
terms, the selected form follows the ordinary Old English reduction of
...
(Kroonen 2013, 206; Orel 2003, 197; Clark Hall 1960, 160). The selected
input \emph{*xámaras} is the genitive singular of that same noun rather
...
genitival phrase (Bosworth and Toller 1898, 78). Clark Hall preserves
the simplex headword as \emph{hamer} / \emph{hamor} (Clark Hall 1960,
160).
...
(Sievers 1965, sec. 245). The relevant comparison form here is the
attested genitive singular \emph{hameres}.
...
under \emph{*xabēną} and Old English \emph{habban} (Kroonen 2013, 237;
Ringe and Taylor 2014, 93). Within that paradigm, however, the
infinitive and the singular present indicative do not continue the same
stem. Ringe and Taylor distinguish a \emph{-ja-} stem in the infinitive
from a non-geminating -ai- / \emph{-ē-} stem in the 2sg and 3sg present
forms (Ringe and Taylor 2014, 93).

The selected input \emph{*xábēθi} is therefore the 3sg present cell
rather than a rephrasing of the infinitive. For the present analysis,
that finite cell is the cleaner comparator for the inherited
non-geminating stem.

\paragraph{Old English evidence}\label{old-english-evidence-46}

The ordinary Old English headword is \emph{habban} (Clark Hall 1960,
157). Campbell's Anglian paradigm includes unsyncopated 3sg forms of the
\emph{hæfed} type, and the present paradigm therefore shows forms of the
\emph{hæf-} type that support the normalized target \emph{hæfeþ}
(Campbell 1959, sec. 762).
...
\emph{habban} (Ringe and Taylor 2014, 364). Campbell's Anglian paradigms
include unsyncopated 3sg forms of the \emph{hæfeþ} / \emph{hæfed} type
(Campbell 1959, sec. 762).
...
\emph{*hemina-} \textasciitilde{} \emph{*hemna-} (Kroonen 2013, 220).
The selected input \emph{*xémonų} is an oblique singular form within
that paradigm rather than the lexeme-level citation form
\emph{*xémenaz}.
...
\emph{heofon}, Mercian \emph{heofen} (Ringe and Taylor 2014, 324).
Campbell likewise gives \emph{heofon} beside \emph{hefen} in the same
West-Saxon \emph{u}-umlaut environment (Campbell 1959, sec. 210.1).
...
\emph{heofon}, alongside Anglian or Mercian \emph{hefen} material (Clark
Hall 1960, 188; Bosworth and Toller 1898, 43). Campbell also cites an
earlier stage \emph{hefzen} in the history of the word (Campbell 1959,
sec. 381).
...
type reflected in northern WGmc \emph{*hebun} (Ringe and Taylor 2014,
324). Campbell's \emph{heofon} beside \emph{hefen} and earlier
\emph{hefzen} show the later West-Saxon back-mutation and suffix
reshaping behind \emph{heofon} (Campbell 1959, sec. 210.1; 1959, sec.
381).
...
under \emph{*libēn-}, reflected in Old English \emph{libban} (Kroonen
2013, 336). Ringe and Taylor show that the paradigm also contained a
separate 3sg present stem, continued in late Northumbrian \emph{lifed},
which they treat as an archaism (Ringe and Taylor 2014, 364).
...
here normalized as \emph{lifeþ} (Ringe and Taylor 2014, 364; Campbell
1959, sec. 762).
...
Northumbrian orthography (Campbell 1959, sec. 762; Ringe and Taylor
2014, 364).
...
\emph{*mannan-}, and Orel has \emph{*mannz} (Kroonen 2013, 354; Orel
2003, 299). The selected input \emph{*mánnas} belongs to a different
level: it is the genitive-singular cell chosen for the Old English
comparison.
...
\emph{menn} (Campbell 1959, sec. 621). Sievers-Brunner likewise cites
\emph{man mannes} (Sievers 1965, sec. 226). He also explains that
word-final simplification underlies forms such as \emph{man} beside
inflected \emph{monnes} (Sievers 1965, sec. 231). Clark Hall keeps the
dictionary headword under \emph{mann} (Clark Hall 1960, 197).
...
confirms the selected genitive singular \emph{mannes} (Campbell 1959,
sec. 621). In the present analysis, \emph{*mánnas} develops through
...
Old English (Sievers 1965, sec. 231).
...
lengthening (Kroonen 2013, 410; Fulk 2018, 69), while Orel keeps a
doublet analysis (Orel 2003, 311). The comparison here concerns the
attested oblique line \emph{meorde}.
...
singular and \emph{meorda} as a genitive plural (Bright 1971, 328;
Bosworth and Toller 1898, 647). Lexicographers reconstruct a bare
nominative \emph{meord} from those obliques, while West Saxon prose more
commonly shows the competing doublet \emph{mēd} (Clark Hall 1960, 214;
Bosworth and Toller 1898, 647).
...
\textasciitilde{} \emph{méd} (Ringe and Taylor 2014, 99). The fuller
oblique-cell path modeled here spells out the intermediate rhotacism,
monophthongization, lowering, breaking, and unstressed-shortening steps
needed for the selected dative-singular comparison.

This is therefore project framing built on a source-backed broader
development. The entry depends on the attested oblique line rather than
on a full decision about the history of the competing \emph{mēd}
tradition.
...
derive West Saxon \emph{niht} from that side (Ringe and Taylor 2014,
240). The citation reconstruction \emph{*náxtz} therefore belongs to the
...
\emph{*nahti} (Ringe and Taylor 2014, 380).
...
\emph{neaht}, \emph{neht}, and \emph{nieht} (Clark Hall 1960, 215).
Campbell likewise preserves the fluctuation between \emph{neaht} and
\emph{niht}, giving genitive \emph{nihte, nihtes}, dative \emph{niht,
nihte}, nominative plural \emph{niht}, and the contrasting plural-side
forms represented by \emph{neahtas} (Campbell 1959, sec. 628.3).
...
\emph{*nehti} and \emph{*neahti} (Ringe and Taylor 2014, 240). Campbell
and Brunner preserve the contrasting non-umlauted \emph{neaht}-type
forms elsewhere in the paradigm (Campbell 1959, sec. 628.3; Sievers
1965, sec. 284).
...
by Old English \emph{ræst} (Kroonen 2013, 445). The selected input
\emph{*rástōz} therefore does not replace the lexeme-level headword. It
identifies one oblique singular cell on the side of the paradigm that
yields \emph{ræste}.
...
The ordinary Old English citation form is \emph{ræst} (Kroonen 2013,
445). Clark Hall likewise gives \emph{ræst} (Clark Hall 1960, 239).
...
\emph{ræste} (Bosworth and Toller 1898, 121).
...
\emph{scultra, scultirra} (Orel 2003, 345). Kroonen reconstructs
\emph{*skuldra-}, a masculine a-stem, and derives the Old High German
feminine forms from \emph{*skuldrjōn-} (Kroonen 2013, 478). Ringe and
Taylor cite PWGmc \emph{*skuldru} for the Old English branch (Ringe and
Taylor 2014, 142).
...
lemmatizes \emph{sculdor} as the normal dictionary form (Clark Hall
1960, 257). Bosworth-Toller also preserves the dative plural
\emph{sculdrum} (Bosworth and Toller 1898, 85).

Bosworth-Toller's Supplement records a weak-feminine \emph{sculdra, an}
(Bosworth and Toller 1898, 699), so \emph{sculdra} belongs to the Old
English record beside the stronger masculine paradigm headed by
\emph{sculdor}. Brunner and Luick also record later spellings such as
\emph{sceoldor} and the i-mutated dative plural \emph{scyldrum}, which
reflect secondary phonological and analogical reshaping within Old
English (Sievers 1965, sec. 92.2.a; Luick 1914-\/-1940, 230).
...
for the dative plural inflexion (Campbell 1959, sec. 373; Hogg 1992,
sec. 3.3.1.3). Brunner points in the same direction by excluding
\emph{m} from the environments in which medial \emph{o} became general
in West Saxon (Sievers 1965, sec. 44 Anm. 7).
...
\emph{scēofan}, \emph{scūfan} (Kroonen 2013, 444). Ringe and Taylor also
show that the English present system belongs to a wider class-II split
that is not identical with the preterite grade (Ringe and Taylor 2014).
...
\emph{scufan, sceaf, scufon, scofen} (Bright 1971, 347). Sweet gives the
same paradigm (Sweet 1953, 29). The normalized form here is
...
Seebold gives Old English \emph{spann} under this noun family (Seebold
1970, 450). The selected input \emph{*spánnai} is therefore not a rival
headword, but a specific dative singular cell chosen on the model of the
feminine ō-stem paradigm (Sievers 1965, sec. 252; 1965, sec. 255.2).
...
\emph{spann} (Clark Hall 1960, 286), and \emph{spanne} is accordingly
treated as the selected regular dative singular comparison form rather
than as a dictionary headword.
...
Orel prints \emph{*þe(x)stilaz} for the lexeme (Orel 2003, 458). The
...
singular \emph{þistles}, which preserves the same stem in an oblique
...
(Campbell 1959, 151). The selected genitive singular \emph{*θístilas}
therefore supplies the conservative comparison form: the cluster is
medial and the regular development yields \emph{þistles}, while the
simplex nominative belongs to the broken headword tradition
\emph{þistel}.
...
cites \emph{lapian, macian} among verbs with restored \emph{a} (Campbell
1959, sec. 159). Ringe and Taylor place the Germanic verb in the same
...
(Ringe and Taylor 2014, 314).

\paragraph{Old English evidence}\label{old-english-evidence-57}

The dictionary headword is \emph{macian} (Clark Hall 1960, 193). The
selected form in this entry is therefore not the lemma but the
imperative singular \emph{maca}, chosen as a paradigm form beside the
...
\emph{a} (Campbell 1959, sec. 159). Ringe and Taylor's class-II weak
imperative singular -a \textless{} \emph{*-ō} then supports the later
finite ending (Ringe and Taylor 2014, 314).
...
make-family base \emph{*maka-} (Kroonen 2013, 350). Ringe and Taylor
likewise derive Old English \emph{macian} from PWGmc \emph{*makon}
through \emph{*mekojan} (Ringe and Taylor 2014, 191).
...
Clark Hall lemmatizes the verb as \emph{macian} (Clark Hall 1960, 193).
The relevant comparison form here is the normalized
present-third-singular \emph{macaþ}, set beside the dictionary headword
and the related imperative singular \emph{maca}.
...
traditions (Campbell 1959, sec. 356.4; 1959, sec. 757). \emph{Macaþ} is
...
\textgreater{} \emph{macaþ} (Campbell 1959, sec. 356.4; Ringe and Taylor
2014, 80).
...
Old English \emph{borian} among its continuants (Kroonen 2013, 85).
Ringe and Taylor give the class-II weak imperative singular as -a
\textless{} \emph{*-ō} (Ringe and Taylor 2014, 314).
...
Clark Hall lemmatizes the verb as \emph{borian} (Clark Hall 1960, 48).
The comparison form here is the normalized imperative singular
\emph{bora}, used beside the headword and the related 3sg form
\emph{boraþ}.
...
\emph{*-ō} points to exactly this type of outcome (Ringe and Taylor
2014, 314). The selected form therefore isolates the regular finite-cell
development more cleanly than the remodelled infinitive does.
...
Kroonen reconstructs the bore-family verb as \emph{*burojan-} and cites
Old English \emph{borian} among its reflexes (Kroonen 2013, 85). The
selected form isolates the finite 3sg cell \emph{*búrōθi} rather than
the infinitive.
...
for the ending (Campbell 1959, sec. 356.4; Ringe and Taylor 2014, 80).

\paragraph{Old English evidence}\label{old-english-evidence-60}

Clark Hall lemmatizes the verb as \emph{borian} (Clark Hall 1960, 48).
The relevant comparison form here is the normalized
present-third-singular \emph{boraþ}, used beside the headword and the
imperative singular \emph{bora}.
...
\emph{-aþ} (Campbell 1959, sec. 757). \emph{Boraþ} is therefore the
regular comparison form for this non-\emph{j} 3sg cell.
...
sequence (Campbell 1959, sec. 356.4; Ringe and Taylor 2014, 80).
...
(Ringe and Taylor 2014, 38), and Kroonen likewise keeps the weak verb as
\emph{*liznōn-} (Kroonen 2013, 380). Fulk cites the same Old English
family from \emph{*liznō-} (Fulk 2018, 127).
...
Clark Hall gives the ordinary headword as \emph{leornian} (Clark Hall
1960, 186). Brunner, however, explicitly records \emph{leornian, nordh.
auch liorna}, and Campbell notes that beside \emph{leornian}
Northumbrian forms with \emph{io} occur where original \emph{eo} and
\emph{io} remain distinct (Sievers 1965, sec. 417 Anm. 10; Campbell
1959, sec. 123 n.~2).
...
i-umlauting environment support this stem shape (Campbell 1959, sec. 123
n.~2; Ringe and Taylor 2014, 247). The West-Saxon-looking \emph{eo}
forms belong to a different dialectal presentation of the same family.
...
\emph{leornian} from a learn-family base of the \emph{*lizn-} type
(Ringe and Taylor 2014, 38), and Kroonen likewise keeps the weak verb as
\emph{*liznōn-} (Kroonen 2013, 380). The selected input \emph{*líznōθi}
is the finite 3sg cell of that family, not the citation form of the
verb.
...
comparison point (Campbell 1959, sec. 356.4; Ringe and Taylor 2014, 80).

\paragraph{Old English evidence}\label{old-english-evidence-62}

Clark Hall gives the ordinary headword as \emph{leornian} (Clark Hall
1960, 186). Brunner records Northumbrian finite forms in \emph{liorn-},
including \emph{liorna} and the 3sg \emph{liornes}, beside the
West-Saxon-looking \emph{leornian} tradition (Sievers 1965, sec. 417
Anm. 10). Campbell likewise notes Northumbrian forms with \emph{io}
beside \emph{leornian} (Campbell 1959, sec. 123 n.~2).
...
supports the final \emph{-aþ} (Campbell 1959, sec. 123 n.~2; Campbell
1959, sec. 356.4; Ringe and Taylor 2014, 247).
...
\emph{lecchon} (Ringe and Taylor 2014, 50). Orel gives the fuller
weak-verb reconstruction \emph{*likkōjanan} with the same Old English
continuation (Orel 2003, 285).

Campbell's weak class-II discussion gives present forms such as lufas,
\emph{-aþ} (\textless{} \emph{-ōsi}, \emph{-ōþi)} (Campbell 1959, sec.
356.4). Ringe and Taylor likewise note that class-II weak present 2sg.
\emph{-as(t)} and 3sg. \emph{-aþ} have stable \emph{a} (Ringe and Taylor
2014, 80). The form treated here is therefore not that remodeled
infinitive but a finite cell in bare trimoric \emph{*-ō}.

\paragraph{Old English evidence}\label{old-english-evidence-63}

Bosworth-Toller lemmatizes the verb as \emph{liccian} (Bosworth and
Toller 1898, 614). Campbell cites \emph{liccian} among Old English forms
with preserved geminate \emph{cc} (Campbell 1959, sec. 398.1). Brunner
likewise cites \emph{liccian} (Sievers 1965, sec. 45 Anm. 3). The Old
English evidence therefore establishes the verbal headword and its
...
finite cell (Campbell 1959, sec. 356.4). Ringe and Taylor likewise note
stable \emph{a} in the class-II 2sg and 3sg (Ringe and Taylor 2014, 80).
The same stem consonantism that appears in \emph{liccian} is preserved
here, giving \emph{cc} throughout the finite form.
...
\emph{-ōsi}, \emph{-ōþi)} (Campbell 1959, sec. 356.4). Ringe and Taylor
likewise note that class-II weak present 2sg. \emph{-as(t)} and 3sg.
\emph{-aþ} have stable \emph{a} (Ringe and Taylor 2014, 80).

\paragraph{Old English evidence}\label{old-english-evidence-64}

Bosworth-Toller lemmatizes the verb as \emph{liccian} (Bosworth and
Toller 1898, 614). The same consonantal frame appears in Campbell's and
Brunner's grammatical citations of \emph{liccian} (Campbell 1959, sec.
398.1; Sievers 1965, sec. 45 Anm. 3). The Old English headword is
therefore clear even though the entry here is not about the citation
form.
...
(Campbell 1959, sec. 356.4). Ringe and Taylor likewise note stable
\emph{a} in the class-II 2sg and 3sg (Ringe and Taylor 2014, 80).
...
\emph{skawōn}, and Old High German \emph{scouwōn} (Orel 2003, 337). The
selected input in this entry is not that infinitive but the imperative
singular \emph{*skáwô}, a finite class-II cell with imperative -a
\textless{} \emph{*-ō} (Ringe and Taylor 2014, 314).
...
singular \emph{scēawa} under that headword (Bright 1971, 346). The form
...
developments (Campbell 1959, sec. 120). Ringe and Taylor's class-II weak
imperative singular -a \textless{} \emph{*-ō} supports the late finite
ending that yields \emph{sċēawa} (Ringe and Taylor 2014, 314). The
result is therefore the expected finite singular form of the
...
\emph{sceáwian} beside Old Frisian \emph{skawia}, Old Saxon
\emph{skawōn}, and Old High German \emph{scouwōn} (Orel 2003, 337). The
selected input in this entry is the present third singular
\emph{*skáwōθi}, a finite class-II cell with stable \emph{a} in the 3sg
ending (Ringe and Taylor 2014, 80).

Campbell states the class-II present endings as lufas, \emph{-aþ}
(\textless{} \emph{-ōsi}, \emph{-ōþi)} (Campbell 1959, sec. 356.4).
Ringe and Taylor likewise note that class-II weak present 2sg.
\emph{-as(t)} and 3sg. \emph{-aþ} have stable \emph{a} (Ringe and Taylor
2014, 80). The relevant comparison is therefore the 3sg cell itself, not
an i-umlauted alternative.
...
singular \emph{-sceawað} (Bright 1971, 383). The evidence thus
establishes the \emph{scēaw-} / \emph{-awað} finite-cell pattern
directly.
...
development (Campbell 1959, sec. 120). \emph{*skáwōθi} therefore belongs
to the \emph{scēaw-} family before the class-II 3sg ending is applied.
...
(Campbell 1959, sec. 356.4; Ringe and Taylor 2014, 80). Because the
...
this group (Kroonen 2011, 297). The citation reconstruction
\emph{*knúppaz} represents the broader cognate-set headword, while the
selected input \emph{*knúbbô} represents the voiced weak-noun branch
treated here.
...
(Clark Hall 1960, 79). Those forms are genuine Old English evidence, but
they belong to the voiceless branch of the family.
...
comparative dictionaries (Kroonen 2013, 446; Orel 2003, 338). The
selected input \emph{*ráukaz} is therefore not the lexeme-level
headword, but the form used here for the Old English derivation.
...
family (Clark Hall 1960, 255; Kroonen 2013, 446). Clark Hall and Seebold
also record verbal \emph{rēac} as the preterite of \emph{rēocan}, but
that verbal form is separate from the noun treated here (Clark Hall
1960, 254; Seebold 1970, 380).
...
English \emph{streowian} as its dictionary continuation (Kroonen 2013,
483). Ringe and Taylor make the split within Old English explicit: the
inherited class-I verb is continued by Anglian \emph{strēgan}, while
West Saxon \emph{streowian} is a remodelled class-II verb (Ringe and
Taylor 2014, sec. 6.1 n.~27).
...
of the \emph{hīez}, ciezan type (Luick 1914-\/-1940, sec. 98). Fulk
likewise allows an early West Saxon \emph{*striegan} directly from
Proto-Germanic \emph{*straujana} (Fulk 2018, sec. 4.10 n.~1).
...
class-II branch (Ringe and Taylor 2014, sec. 6.1 n.~27; Campbell 1959,
sec. 753.7).
...
remodelled into class II (Fulk 2018, sec. 4.10 n.~1; Campbell 1959, sec.
753.7).
...
(Kroonen 2013, 151). The selected input \emph{*fūri} therefore does not
function as an arbitrary substitute for the headword: it represents the
specific inherited cell that supplies the \emph{i} needed for i-umlaut.
...
oblique \emph{fyre} in the Old English record (Bosworth and Toller 1898,
288). The first is the regular inherited outcome of the phonological
development from the selected input; the second shows the later
restoration of a final \emph{-e} within the paradigm.
...
From \emph{*fūri}, i-umlaut changes \emph{ū} to \emph{ȳ} (Hogg 1992,
sec. 3.3.3.1). Subsequent loss of the final high vowel after a heavy
syllable yields \emph{fȳr} (Campbell 1959, sec. 345). The inherited
...
Old English \emph{tæppa} (Orel 2003, 402). The selected input is
therefore the inherited noun itself; the entry does not depend on a
different lexeme-level proto or a different inherited noun cell.
...
\emph{tæppere} and \emph{tæppestre} (Orel 2003, 402; Clark Hall 1960,
305). The target is therefore a real Old English noun form, not a
reconstructed convenience spelling.
...
\emph{*bukkaz} (Kroonen 2013, 121; Orel 2003, 61). Orel also preserves
parallel n-stem material behind Old English \emph{bucca} (Orel 2003,
62). The selected input therefore remains identical with the lexeme
label: no alternative inherited cell accounts for the form.
...
(Campbell 1959, sec. 115). Clark Hall and Bosworth-Toller show that Old
English has both \emph{bucca} and \emph{bucc} (Clark Hall 1960, 53;
Bosworth and Toller 1898, 122). The a-stem citation form \emph{bucc} is
the target treated here, with \emph{bucca} kept as genuine philological
...
(Ringe and Taylor 2014, 42--43; Campbell 1959, 43).
...
spelling \emph{fugel} (Bosworth and Toller 1898, 282; Clark Hall 1960,
...
history predicts \emph{fo-} (Campbell 1959, 150).

\paragraph{Development to Old
English}\label{development-to-old-english-73}

From \emph{*fúglaz}, the regular cascade yields \emph{fogol}: the root
vowel lowers before the following non-high vowel (Ringe and Taylor 2014,
42--43; Campbell 1959, 43), final \emph{z} is lost, and the cluster is
resolved by the usual medial vowel (Ringe and Taylor 2014, 345; Campbell
1959, 150). That is the expected inherited outcome.
...
inherited residue, not as a categorical sound law (Luick 1914-\/-1940,
148; Ringe and Taylor 2014, 47). The item therefore remains a genuine
lexical exception rather than the output of a recoverable regular
mechanism.
...
English \emph{rust} and Old Saxon and Old High German \emph{rost} (Orel
2003, 308). The form \emph{*rústō} therefore stands here as a competing
citation reconstruction rather than as the best-supported inherited
headword.
...
\emph{rūst} m. (Clark Hall 1960, 245), and Bosworth-Toller records
\emph{rúst} (? and rust) (Bosworth and Toller 1898, 677). The form is
normalized here as \emph{rust} from that attested record.
...
not \emph{rust} (Campbell 1959, sec. 115). The same lowering would also
affect comparative citation-form reconstructions such as \emph{*rustaz}.
...
(Kroonen 2013, 638; Ringe and Taylor 2014, 47). Campbell accordingly
names Old English \emph{wulf} as an exception to the regular lowering of
stressed \emph{u} before a following non-high vowel (Campbell 1959, sec.
115).
...
in comparable forms such as \emph{folc} and \emph{bolt} (Bülbring 1902,
sec. 116). Luick rejects a categorical labial blocker on the same
grounds and prefers a lexical or analogical account instead (Luick
1914-\/-1940, 148).
...
(Campbell 1959, sec. 115). Sievers-Brunner notes that oblique
\emph{wulfe} continues \emph{wulfi} or older \emph{wulfai} (Sievers
1965, sec. 160).
...
toward \emph{o}-vocalism (Ringe and Taylor 2014, 42--44). The compact
trace shows exactly that path: \emph{*wúlfaz} \textgreater{}
\emph{*wólfaz} \textgreater{} \emph{*wólfa} \textgreater{} \emph{wolf}.

A high-vowel oblique input would behave differently. In project terms,
the following high vowel would block the lowering of \emph{u}, but the
same environment would also trigger i-umlaut, so the regular control
result would be \emph{wylf} or \emph{wylfe}, not bare \emph{wulf}. The
...
place where the rule itself is doubtful (Campbell 1959, sec. 115).

Bülbring likewise lists \emph{wulle} among the traditional
\emph{u}-preserving exceptions (Bülbring 1902, sec. 116). The
comparative evidence therefore establishes two things at once: the
regular result should be \emph{woll}, and Old English still has a
lexical exception of the \emph{wull} / \emph{wulle} type.
...
form of the noun (Bülbring 1902, sec. 116). Both point to the same
lexical item and to the same exceptional preservation of root \emph{u}.
...
know why \emph{*u} failed to lower in forms of this sort (Luick
1914-\/-1940, 148; Ringe and Taylor 2014, sec. 2.3.1).
...
Bammesberger, Alfred. 1997. {``Die Vorform von Altenglisch
{h{æ}rfest}.''} \emph{Anglia: Zeitschrift f{ü}r Englische Philologie}
115 (2): 223--30. \url{https://doi.org/10.1515/angl.1997.115.2.223}.
...
3rd ed. Edited by Frederic G. Cassidy and Richard N. Ringler. Holt,
Rinehart; Winston.
...
Sprache}. 25th ed. Edited by Elmar Seebold. De Gruyter.
...
Lloyd, Albert L. 1966. {``Is There an {a-}Umlaut of *{i} in
{Germanic}?''} \emph{Language} 42 (4): 738--45.
\url{https://doi.org/10.2307/411829}.
...
Sievers, Eduard. 1965. \emph{Altenglische Grammatik}. 3rd ed. Edited by
Karl Brunner. Niemeyer.

\bibitem[\citeproctext]{ref-Sweet1953}
Sweet, Henry. 1953. \emph{An {Anglo-Saxon} Primer}. 9th ed. Clarendon
Press.
