...
The distinction is concrete in Old English [*sċuldrum*]{.iv lang=oe sort=sculdrum} ‘shoulders, dative plural’. Its inflectional history requires an input of the type [*skúldramiz*]{.iv lang=pgmc display=*skúldramiz sort=skuldramiz}; the singular headword represents a different paradigm cell.
