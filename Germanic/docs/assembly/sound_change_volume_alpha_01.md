...
| 031-034 | SC031; SC032; SC033; SC034 | scaffold | strong local chain with one broader outlier | literature and book dossiers drafted | grouped corridor scaffold |
...
This scaffold unit groups `SC031`, `SC032`, `SC033`, and `SC034` as the West
Saxon diphthong chain. The aim is to keep the local corridor visible now
without pretending that the final prose architecture is settled.

#### Current evidence

Chronology cards and FOMA implementations exist for all four member changes.
Dedicated literature dossiers have not yet been promoted for the grouped chain,
so the current entry remains a scaffold placeholder.

#### Place in the cascade

The unit spans current cascade orders 31-34, from *OEWwSimplification* through
*OEAwLongDiphthong*. It follows the glide/fronting entry and leads into the
prefix and compound adjustments.

#### Order evidence

`SC031` and `SC034` already behave like a local reciprocal pair, while `SC032`
and `SC033` keep the chain from collapsing into a single two-change note. The
result is stronger than a miscellaneous cluster but still short of finished
chapter prose.

#### Status

This is a scaffold placeholder, not finished book prose. It still needs
literature dossiers and human judgement about whether the eventual book keeps
the full four-change chain together or promotes one narrower subsection first.
