#!/usr/bin/env python3
"""Annotate old_english_sandbox_results entries with stage-by-stage info."""
...
def load_trace_helper():
    tracer_path = SCRIPT_DIR / "trace_old_english_sandbox.py"
    spec = importlib.util.spec_from_file_location("trace_old_english_sandbox", tracer_path)
    if not spec or not spec.loader:
        raise RuntimeError("Unable to load trace_old_english_sandbox module")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Attach tracer stage info to old_english_sandbox_results JSON",
    )
    parser.add_argument(
        "--input",
        default="tmp/old_english_sandbox_results_current.json",
        help="Path to the base sandbox results JSON (default: %(default)s)",
    )
    parser.add_argument(
        "--output",
        default="tmp/old_english_sandbox_results_with_stages.json",
