#!/usr/bin/env python3
"""Repair the ``fst_identifier`` column of the historical staging map.
...
def repair_lines(staging_text: str, sc_to_foma: dict[str, str]) -> tuple[list[str], int]:
...
def main() -> int:
    ...
    repaired_lines, changed = repair_lines(original, sc_to_foma)
    repaired_text = "\n".join(repaired_lines) + "\n"
