...
local function is_notation_only(form)
  ...
  -- (e.g., *u, *z, *ō, *ai, *lþ, *éa). Non-asterisked 2-char forms like cū,
  -- bā may be real OE words; allow them through.
  ...
  -- (e.g., ll, ld, eo, ie, ai, pp, xs, rr) — real OE words nearly always
  -- contain at least one diacritic or non-ASCII character, or are 3+ chars.
  if bare_len == 2 then
    local has_non_ascii = bare:byte(1) > 127 or bare:byte(2) > 127
    if not has_non_ascii then return true end
    if bare:match("^[áéíóúýǣāēīōūȳæ]+$") then return true end
  end
  return false
end
