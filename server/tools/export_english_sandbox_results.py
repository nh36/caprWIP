#!/usr/bin/env python3
"""Export English sandbox analyzer outputs to JSON."""
...
DEFAULT_OUTPUT = SCRIPT_DIR.parent / "tmp" / "english_sandbox_results_current.json"
ANALYZER = "english_brace_sandbox.bin"
...
def load_rows(tsv_path: Path) -> List[Dict[str, str]]:
    with tsv_path.open(encoding="utf-8") as handle:
        ...
        rows = [row for row in reader if row.get("DOCULECT") == "English"]
    return rows


def main() -> None:
    parser = argparse.ArgumentParser(description="Export English sandbox analyzer results to JSON")
    ...
    parser.add_argument(
        ...
        help="Directory containing english_brace_sandbox.bin (default: %(default)s)",
    )
    ...
    for row in rows:
        ...
        if not ipa:
            continue
        outputs = analyze_surface(ipa, bin_dir)
        if outputs:
            success += 1
        results.append(
            {
